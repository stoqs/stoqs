...
class MPQuerySet(object):
            ...
            try:
                ...
                for mp in self.mp_query[:ITER_HARD_LIMIT]:
                    ...
                    row = {
                        ...
                        "measurement__instantpoint__activity__name": mp[
                            "measurement__instantpoint__activity__name"
                        ],
    ...
    def count(self):
        ...
        try:
            ...
            logger.debug(
                "self._count = %d as retreived from self.mp_query.count()", self._count
            )
        except AttributeError:
            try:
                self._count = sum(1 for mp in self.mp_query)
                logger.debug(
                    "self._count = %d as retreived from sum(1 for mp in self.mp_query)",
                    self._count,
                )
...
class SPQuerySet(object):
            ...
            try:
                ...
                for mp in self.sp_query[:ITER_HARD_LIMIT]:
                    row = {
                        ...
                        "sample__instantpoint__timevalue": mp[
                            "sample__instantpoint__timevalue"
                        ],
                        "sample__instantpoint__activity__name": mp[
                            "sample__instantpoint__activity__name"
                        ],
                        "datavalue": mp["datavalue"],
                    }
            ...
            try:
                ...
                for mp in self.sp_query[:ITER_HARD_LIMIT]:
                    row = {
                        ...
                        "sample__instantpoint__activity__platform__name": mp[
                            "sample__instantpoint__activity__platform__name"
                        ],
                        # If .values(...) are requested in the query string then json serialization of the point geometry does not work right
    ...
    def count(self):
        logger.debug(
            "Counting records in self.sp_query which is of type = %s",
            type(self.sp_query),
        )
        ...
        try:
            ...
            try:
                ...
                logger.debug(
                    "c = %d as retreived from sum(1 for mp in self.sp_query)", c
                )
            except DatabaseError:
                return 0
        return c
...
class MPQuery(object):
    ...
    def buildMPQuerySet(self, *args, **kwargs):
        ...
        if self.qs_mp is None:
            ...
            if self.parameterID is not None:
                ...
                parameter = Parameter.objects.using(self.request.META["dbAlias"]).get(
                    id=self.parameterID
                )
            ...
            if SAMPLED in parameterGroups:
                ...
                if self.kwargs["showparameterplatformdata"]:
                    logger.debug(
                        "Building qs_sp_no_order with values_list = %s",
                        SPQuerySet.ui_timedepth_columns,
                    )
                    self.qs_sp_no_order = self.getSampledParametersQS(
                        SPQuerySet.ui_timedepth_columns, orderedFlag=False
                    )
                else:
                ...
                if self.kwargs["showparameterplatformdata"]:
                    logger.debug(
                        "Building qs_mp_no_order with values_list = %s",
                        MPQuerySet.ui_timedepth_columns,
                    )
                    self.qs_mp_no_order = self.getMeasuredParametersQS(
                        MPQuerySet.ui_timedepth_columns, orderedFlag=False
                    )
                else:
                    self.qs_mp_no_order = self.getMeasuredParametersQS(
                        orderedFlag=False
        ...
        if group == SAMPLED:
            ...
            if "platforms" in self.kwargs:
                if self.kwargs["platforms"]:
                    qparams["sample__instantpoint__activity__platform__name__in"] = (
                        self.kwargs["platforms"]
                    )
            if "time" in self.kwargs:
                if self.kwargs["time"][0] is not None:
                    qparams["sample__instantpoint__timevalue__gte"] = self.kwargs[
                        "time"
                    ][0]
                if self.kwargs["time"][1] is not None:
                    qparams["sample__instantpoint__timevalue__lte"] = self.kwargs[
                        "time"
                    ][1]
            ...
            if "activitynames" in self.kwargs:
                if self.kwargs["activitynames"]:
                    qparams["sample__instantpoint__activity__name__in"] = self.kwargs[
                        "activitynames"
                    ]
            ...
            if "measuredparametersgroup" in self.kwargs:
                if self.kwargs["measuredparametersgroup"]:
                    qparams["parameter__id__in"] = self.kwargs[
            ...
            if "platforms" in self.kwargs:
                if self.kwargs["platforms"]:
                    qparams[
                        "measurement__instantpoint__activity__platform__name__in"
                    ] = self.kwargs["platforms"]
            if "time" in self.kwargs:
                if self.kwargs["time"][0] is not None:
            ...
            if "activitynames" in self.kwargs:
                if self.kwargs["activitynames"]:
                    qparams["measurement__instantpoint__activity__name__in"] = (
                        self.kwargs["activitynames"]
                    )
                    logger.debug(
                        f"qparams['measurement__instantpoint__activity__name__in'] = {qparams['measurement__instantpoint__activity__name__in']}"
                    )
    ...
    def getMeasuredParametersQS(self, values_list=[], orderedFlag=True):
        ...
        if self.parameterID:
            logger.debug(
                "Adding parameter__id=%d filter to qs_mp", int(self.parameterID)
            )
        ...
        if "parametervalues" in self.kwargs:
            if self.kwargs["parametervalues"]:
                ...
                logger.debug(
                    "Building MPQuerySet with qs_mpquery = %s", str(qs_mp.query)
                )
                qs_mpq = MPQuerySet(
                    self.request.META["dbAlias"], None, values_list, qs_mp=qs_mp
                )
        else:
        ...
        if qs_mpq is None:
            ...
            logger.debug(
                'Strings may be split on spaces in the following "pretty print"'
            )
            logger.debug(
    ...
    def getSampledParametersQS(self, values_list=[], orderedFlag=True):
        ...
        if self.parameterID:
            logger.debug(
                "Adding parameter__id=%d filter to qs_sp", int(self.parameterID)
            )
            qs_sp = qs_sp.filter(parameter__id=int(self.parameterID))

        if orderedFlag:
            qs_sp = qs_sp.order_by(
                "sample__instantpoint__activity__name",
                "sample__instantpoint__timevalue",
                "parameter__name",
            )
        ...
        if "parametervalues" in self.kwargs:
            if self.kwargs["parametervalues"]:
                ...
                qs_sp = (
                    SampledParameter.objects.using(self.request.META["dbAlias"])
                    .select_related("sample__instantpoint__activity__platform")
                    .filter(**qparams)
                )
                if orderedFlag:
                    qs_sp = qs_sp.order_by(
                ...
                sql = pq.addParameterValuesSelfJoins(
                    sql,
                    self.kwargs["parametervalues"],
                    select_items=self.sampled_rest_select_items,
                )
                logger.debug("\n\nsql after parametervalue query = %s\n\n", sql)
                ...
                qs_spq = SPQuerySet(
                    self.request.META["dbAlias"], None, values_list, qs_sp=qs_sp
                )
        else:
            logger.debug("Building SPQuerySet for SampledParameter...")
            qs_spq = SPQuerySet(
    ...
    def getMPCount(self):
        ...
        if not self._count:
            ...
            if self.initialQuery:
                ...
                self._count = MeasuredParameter.objects.using(
                    self.request.META["dbAlias"]
                ).count()
            else:
                try:
