...
from loaders import X3D_MODEL
from loaders import X3DPLATFORMMODEL
from loaders.SampleLoaders import ESP_FILTERING
from loaders.SampleLoaders import NETTOW
from loaders.SampleLoaders import PLANKTONPUMP
from loaders.SampleLoaders import SAMPLE_TYPES
from loaders.SampleLoaders import SAMPLED
from loaders.SampleLoaders import sample_simplify_crit
...
from .utils import EPOCH_DATETIME
from .utils import EPOCH_STRING
from .utils import getGet_Actual_Count
from .utils import getParameterGroups
from .utils import getShow_All_Parameter_Values
from .utils import getShow_Parameter_Platform_Data
from .utils import getShow_Sigmat_Parameter_Values
from .utils import getShow_StandardName_Parameter_Values
from .utils import postgresifySQL
from .utils import round_to_n
from .utils import simplify_points
from .Viz import MeasuredParameter
from .Viz import ParameterParameter
from .Viz import PlatformAnimation
from .Viz import PPDatabaseException
...
class STOQSQManager(object):
    ...
    def _buildQuerySet(self, *args):
        ...
        if "qs" in args:
            logger.debug(
                "Using query string passed in to make a non-activity based query"
            )
            qs = args["qs"]
        else:
            ...
            if fromTable == "Activity":
                ...
                qs = models.Activity.objects.using(
                    self.dbname
                ).all()  # To receive filters constructed below from kwargs
                qs_platform = qs
            elif fromTable == "Sample":
                logger.debug("Making %s based query", fromTable)
                qs = models.Sample.objects.using(
                    self.dbname
                ).all()  # To receive filters constructed below from kwargs
                ...
                logger.debug("Making %s based query", fromTable)
                qs = models.ActivityParameter.objects.using(
                    self.dbname
                ).all()  # To receive filters constructed below from kwargs
            elif fromTable == "ActivityParameterHistogram":
                logger.debug("Making %s based query", fromTable)
                qs = models.ActivityParameterHistogram.objects.using(
                    self.dbname
                ).all()  # To receive filters constructed below from kwargs
            else:
        ...
        for k, v in list(self.kwargs.items()):
            ...
            if k == "depth" or k == "time":
                ...
                if v:
                    logger.debug(
                        "Setting self.initialQuery = False because %s = %s", k, v
                    )
                    self.initialQuery = False
        ...
        for k, v in list(self.kwargs.items()):
            ...
            if hasattr(self, "_%sQ" % (k,)):
                ...
                logger.debug(
                    "fromTable = %s, k = %s, v = %s, q = %s", fromTable, k, v, q
                )
                qs = qs.filter(q)
    ...
    def generateOptions(self):
        ...
        for k, v in list(self.options_functions.items()):
            ...
            logger.info(
                f"Built in {1000 * (time.time() - start_time):6.1f} ms {k} with {str(v).split('.')[1].split(' ')[0]}()"
            )
    ...
    def getCounts(self):
        ...
        logger.debug(
            "str(self.getActivityParametersQS(forCount=True).query) = %s",
            str(self.getActivityParametersQS(forCount=True).query),
        )
        approximate_count = self.getActivityParametersQS(forCount=True).aggregate(
            Sum("number")
        )["number__sum"]
        ...
        try:
            approximate_count_localized = locale.format(
                "%d", approximate_count, grouping=True
            )
        except TypeError:
            logger.warn(
                "Failed to format approximate_count = %s into a number, setting to None",
                approximate_count,
            )
        ...
        if actual_count:
            try:
                actual_count_localized = locale.format(
                    "%d", actual_count, grouping=True
                )
            except TypeError:
                logger.exception(
                    "Failed to format actual_count = %s into a number", actual_count
                )
    ...
    def getAPCount(self):
        ...
        """
        qs_ap = (
            self.getActivityParametersQS()
        )  # Approximate count from ActivityParameter
        if qs_ap:
            return qs_ap.count()
        else:
            return 0
    ...
    def getActivityParametersQS(self, forCount=False):
        ...
        if forCount:
            if self.kwargs["measuredparametersgroup"]:
                logger.debug(
                    "Adding Q object for parameter__id__in = %s",
                    self.kwargs["measuredparametersgroup"],
                )
                return self.activityparameter_qs.filter(
                    Q(parameter__id__in=self.kwargs["measuredparametersgroup"])
                )
            else:
    ...
    def getParameters(self, groupName=""):
        ...
        p_qs = models.Parameter.objects.using(self.dbname).filter(
            Q(activityparameter__activity__in=qs_acts)
        )
        ...
        if "mplabels" in self.kwargs:
            if self.kwargs["mplabels"]:
                ...
                commonMeasurements = (
                    models.MeasuredParameterResource.objects.using(self.dbname)
                    .filter(resource__id__in=self.kwargs["mplabels"])
                    .values_list("measuredparameter__measurement__id", flat=True)
                )
                p_qs = p_qs.filter(
                    Q(
                        id__in=models.MeasuredParameter.objects.using(self.dbname)
                        .filter(Q(measurement__id__in=commonMeasurements))
        ...
        p_qs = p_qs.values(
            "name", "standard_name", "id", "units", "long_name", "description"
        ).distinct()
        ...
        for row in p_qs.order_by("name"):
            ...
            if not long_name:
                long_name_q = (
                    models.ParameterResource.objects.using(self.dbname)
                    .filter(parameter__id=id, resource__name="long_name")
                    .values("resource__value")
                )
                if long_name_q:
            ...
            comment_q = (
                models.ParameterResource.objects.using(self.dbname)
                .filter(parameter__id=id, resource__name="comment")
                .values("resource__value", "parameter__name")
            )
            if not comment_q:
                pass
            else:
            ...
            if name is not None:
                results.append(
                    (name, standard_name, id, units, long_name, comment, description)
                )
        ...
        if pid:
            try:
                if percentileAggregateType == "extrema":
                    logger.debug(
                        "self.getActivityParametersQS().filter(parameter__id=%s) = %s",
                        pid,
                        str(
                            self.getActivityParametersQS()
                            .filter(parameter__id=pid)
                            .query
                        ),
                    )
                    qs = (
                        self.getActivityParametersQS()
                        .filter(parameter__id=pid)
                        .aggregate(Min("p010"), Max("p990"), Avg("median"))
                    )
                    logger.debug("qs = %s", qs)
                    try:
                        plot_results = [
                            pid,
                            round_to_n(qs["p010__min"], 4),
                            round_to_n(qs["p990__max"], 4),
                        ]
                    except TypeError:
                    ...
                    qs = self.getActivityParametersQS().filter(
                        parameter__id=pid, number__gt=4
                    )
                    if qs.count() == 0:
                    ...
                    try:
                        plot_results = [
                            pid,
                            round_to_n(qs["p025__avg"], 4),
                            round_to_n(qs["p975__avg"], 4),
                        ]
                        if plot_results[1] == plot_results[2]:
                            logger.debug(
                                "Standard min and max for for pid %s are the same. Getting the overall min and max values.",
                                pid,
                            )
                            qs = (
                                self.getActivityParametersQS()
                                .filter(parameter__id=pid)
                                .aggregate(Min("p025"), Max("p975"))
                            )
                            plot_results = [
                                pid,
                                round_to_n(qs["p025__min"], 4),
                                round_to_n(qs["p975__max"], 4),
                            ]
                    except TypeError:
                ...
                if pid in ("longitude", "latitude"):
                    ...
                    if pid == "longitude":
                        plot_results = [
                            "longitude",
                            round_to_n(extent[0][0], 4),
                            round_to_n(extent[1][0], 4),
                        ]
                    if pid == "latitude":
                        plot_results = [
                            "latitude",
                            round_to_n(extent[0][1], 4),
                            round_to_n(extent[1][1], 4),
                        ]
                elif pid == "depth":
                    dminmax = self.qs.aggregate(Min("mindepth"), Max("maxdepth"))
                    plot_results = [
                        "depth",
                        round_to_n(dminmax["mindepth__min"], 4),
                        round_to_n(dminmax["maxdepth__max"], 4),
                    ]
                elif pid == "time":
                    epoch = EPOCH_DATETIME
                    tminmax = self.qs.aggregate(Min("startdate"), Max("enddate"))
                    tmin = (tminmax["startdate__min"] - epoch).days + (
                        tminmax["startdate__min"] - epoch
                    ).seconds / 86400.0
                    tmax = (tminmax["enddate__max"] - epoch).days + (
                        tminmax["enddate__max"] - epoch
                    ).seconds / 86400.0
                ...
                logger.warn(f"{e}")
                logger.warn(
                    f"Setting pid = {pid} in plot_results to min/max to limits of single-precision floats"
                )
                plot_results = [
                    pid,
                    round_to_n(np.finfo("f").min, 4),
                    round_to_n(np.finfo("f").max, 4),
                ]
            ...
            if self.kwargs["parameterplot"][0]:
                ...
                try:
                    if percentileAggregateType == "extrema":
                        qs = (
                            self.getActivityParametersQS()
                            .filter(parameter__id=parameterID)
                            .aggregate(Min("p025"), Max("p975"))
                        )
                        plot_results = [
                            parameterID,
                        ...
                        # Eliminate Gulper Activities which skew the result badly resulting in a bad color lookup table
                        qs = self.getActivityParametersQS().filter(
                            parameter__id=parameterID, number__gt=4
                        )
                        if qs.count() == 0:
                            # Relax the constraint eliminating Gulper Activities that have a number of 4 or less
                            qs = self.getActivityParametersQS().filter(
        ...
        if "measuredparametersgroup" in self.kwargs:
            if len(self.kwargs["measuredparametersgroup"]) == 1:
                ...
                try:
                    ...
                    logger.warn(f"{e}")
                    logger.warn(
                        f"Setting pid = {pid} in da_results to min/max to limits of single-precision floats"
                    )
                    da_results = [
                        pid,
                        round_to_n(np.finfo("f").min, 4),
            ...
            if len(self.kwargs["parameterstandardname"]) == 1:
                ...
                try:
                    if percentileAggregateType == "extrema":
                        qs = (
                            self.getActivityParametersQS()
                            .filter(parameter__standard_name=sname)
                            .aggregate(Min("p025"), Max("p975"))
                        )
                        da_results = [
                            sname,
        ...
        def _innerGetPlatformModel(self, platform):
            ...
            if pModel:
                ...
                geom_list = list(
                    [
                        _f
                        for _f in self.qs.filter(platform__name=platformName)
                ...
                if len(geom_list) > 1:
                    logger.debug(
                        "More than one location for %s returned."
                ...
                depth_list = (
                    self.qs.filter(platform__name=platformName)
                    .values_list("nominallocation__depth", flat=True)
                    .distinct()
                )
                if len(depth_list) > 1:
                    logger.debug(
                        "More than one depth for %s returned. Checking "
                        "Resource for nominaldepth",
                        platformName,
                    )
                    try:
                        depth = float(
                        ...
                        logger.debug(
                            "Got depth = %s from X3D_MODEL_nominaldepth in "
                            "PlatformResource",
                            depth,
                        )
                    except (IndexError, ObjectDoesNotExist):
                        logger.warn(
                            "Resource name X3D_MODEL_nominaldepth not found for "
                            "for platform %s. Using a nominaldepth of 0.0",
                            platformName,
                        )
                        depth = 0.0
                else:
    ...
    def getPlatforms(self):
        ...
        qs = (
            self.qs_platform.filter(~Q(activitytype__name=LRAUV_MISSION))
            .values(
                "platform__uuid",
                "platform__name",
                "platform__color",
                "platform__platformtype__name",
            )
            .distinct()
        ...
        for row in qs:
            ...
            if name is not None and id is not None:
                ...
                fts = (
                    models.ActivityResource.objects.using(self.dbname)
                    .filter(resource__name="featureType", activity__platform__name=name)
                ...
                if len(fts) > 1:
                    logger.warn(
                        "More than one featureType returned for platform %s: %s.",
                        name,
                        fts,
                    )
                    logger.warn(
                        f"Using '{fts[0]}'.  Consider using a different Platform name for the other featureType(s)."
                    )
                try:
                    featureType = fts[0]
                except IndexError:
                    logger.warn(
                        'No featureType returned for platform name = %s.  Setting it to "trajectory".',
                        name,
                    )
                    featureType = "trajectory"

                if "trajectory" in featureType:
                    platformTypeHash[platformType].append(
                        (
                            name,
                            id,
                            color,
                    ...
                    if name in self.qs.values_list("platform__name", flat=True):
                        ...
                        logger.debug(
                            f"Seeing if Platform {name} has roll, pitch, and yaw Parameters..."
                        )
                        pr_qs = models.ActivityParameter.objects.using(
                            self.dbname
                        ).filter(activity__platform__name=name)
                        has_roll = pr_qs.filter(
                            parameter__standard_name="platform_roll_angle"
                        )
                        has_pitch = pr_qs.filter(
                            parameter__standard_name="platform_pitch_angle"
                        )
                        has_yaw = pr_qs.filter(
                            parameter__standard_name="platform_yaw_angle"
                        )
                        if has_roll or has_pitch or has_yaw:
                            ...
                            platformTypeHash[platformType].append(
                                (name, id, color, featureType, x3dModel, x, y, z)
                            )
    ...
    def getTime(self):
        ...
        try:
            times = (
                time.mktime(qs["startdate__min"].timetuple()) * 1000,
                time.mktime(qs["enddate__max"].timetuple()) * 1000,
            )
    ...
    def getDepth(self):
        ...
        try:
            ...
            logger.exception(
                "Failed to convert mindepth__min and/or maxdepth__max to float from qs = %s",
                qs,
            )
            return
        else:
    ...
    def _add_ts_tsp_to_sdt(self, p, plq, timeSeriesQ, timeSeriesProfileQ, sdt):
        ...
        qs_tsp = (
            self.qs.filter(plq & (timeSeriesQ | timeSeriesProfileQ))
            .select_related()
            .values(
        ...
        if "time" in self.kwargs:
            if (
                self.kwargs["time"][0] is not None
                and self.kwargs["time"][1] is not None
            ):
                logger.debug(f"Querying beween {self.kwargs['time']}")
                qs_tsp = qs_tsp.filter(
                    Q(instantpoint__timevalue__gte=self.kwargs["time"][0])
        ...
        for sd in qs_tsp:
            an_nd = "%s_%s" % (
                sd["name"],
                sd["simpledepthtime__nominallocation__depth"],
            )
    ...
    def getSimpleDepthTime(self):
        ...
        for at in other_activitytypes:
            acts = models.Activity.objects.using(self.request.META["dbAlias"]).filter(
                activitytype__name=at
            )
            for act, c in zip(
                acts, gist_ncar(np.linspace(0, gist_ncar.N, len(acts), dtype=int))
            ):
                ...
                for p in plats:
                    ...
                    if p[3].lower() == "trajectory":
                        ...
                        if act_type == "default":
                            ...
                            qs_traj = qs_traj.filter(
                                ~Q(activitytype__name__in=other_activitytypes)
                            )
                        else:
                        ...
                        logger.debug(
                            f"-trajectory, filling sdt_groups['{act_type}']['sdt']['{p[0]}'][]"
                        )
                        # Better to use 'exclude' to get remaining Activities so as to include those Activities
                        # (dorado_Gulper, daphne_Sipper, makai_ESP, etc.) that aren't in the checkbox list
                        if self.kwargs.get("exclude_ans"):
                            logger.debug(
                                "exclude_ans = %s", self.kwargs.get("exclude_ans")
                            )
                            qs_traj = qs_traj.exclude(
                                name__in=self.kwargs.get("exclude_ans")
                            )
                        for s in qs_traj:
                            if s[1] is not None:
                                sdt_groups[act_type]["sdt"][p[0]][s[2]].append(
                                    [s[0], "%.2f" % s[1]]
                                )
                        if act_type != "default":
                            for number, act_mission in enumerate(
                                sdt_groups[act_type]["sdt"][p[0]].keys()
                            ):
                                sdt_groups[act_type]["colors"][p[0]][act_mission] = (
                                    at_colors[act_type][act_mission]
                                )
                        logger.debug(
                            f" Done filling sdt_groups['{act_type}']['sdt']['{p[0]}'][]"
                        )

                    elif (
                        p[3].lower() == "timeseries"
                        or p[3].lower() == "timeseriesprofile"
                    ) and act_type == "default":
                        self._add_ts_tsp_to_sdt(
                            p,
                            plq,
                            timeSeriesQ,
                            timeSeriesProfileQ,
                            sdt_groups[act_type]["sdt"],
                        )

                    elif (
                        p[3].lower() == "trajectoryprofile" and act_type == "default"
                    ):  # pragma: no cover
                        iptvq = Q()
                        qs_tp = None
                        if "time" in self.kwargs:
                            if (
                                ...
                                s_ems = (
                                    time.mktime(
                                        datetime.strptime(
                                            self.kwargs["time"][0], "%Y-%m-%d %H:%M:%S"
                                        ).timetuple()
                                    )
                                    * 1000
                                )
                                e_ems = (
                                    time.mktime(
                                        datetime.strptime(
                                            self.kwargs["time"][1], "%Y-%m-%d %H:%M:%S"
                                        ).timetuple()
                                    )
                                    * 1000
                                )
                                iptvq = Q(
                                    simpledepthtime__epochmilliseconds__gte=s_ems
                                ) & Q(simpledepthtime__epochmilliseconds__lte=e_ems)
                                qs_tp = (
                                    self.qs.filter(plq & trajectoryProfileQ & iptvq)
                                    .select_related()
                                    .values(
                                        "name",
                                        "simpledepthtime__depth",
                        ...
                        if not qs_tp:
                            qs_tp = (
                                self.qs.filter(plq & trajectoryProfileQ)
                                .select_related()
                                .values(
                                    "name",
                                    "simpledepthtime__depth",
    ...
    def getSimpleBottomDepthTime(self):
        ...
        for plats in list(self.getPlatforms().values()):
            for p in plats:
                ...
                if p[3].lower() == "trajectory":
                    qs_traj = (
                        self.qs.filter(plq & trajectoryQ)
                        .values_list(
                            "simplebottomdepthtime__epochmilliseconds",
                            "simplebottomdepthtime__bottomdepth",
                            "name",
                        )
                        .order_by("simplebottomdepthtime__epochmilliseconds")
                    )
                    ...
                    for s in qs_traj:
                        try:
                            ...
                            sbdt[p[0]][
                                s[2]
                            ] = []  # First time seeing activity__name, make it a list
                            if s[1] is not None:
                                sbdt[p[0]][s[2]].append(
                                    [s[0], "%.2f" % s[1], maxDepth]
                                )  # Append first value, even if it is 0.0
                        except TypeError:
    ...
    def _collectParameters(
        self, platform, pt, pa_units, is_standard_name, ndCounts, strides, colors
    ):
        """
        ...
        # Get parameters for this platform and collect units in a parameter name hash, use standard_name if set and repair bad names
        p_qs = models.Parameter.objects.using(self.dbname).filter(
            Q(activityparameter__activity__in=self.qs)
        )
        logger.debug(
            "self.kwargs['parametertimeplotid'] = %s",
            self.kwargs["parametertimeplotid"],
        )
        ...
        for parameter in p_qs:
            ...
            nds = (
                models.NominalLocation.objects.using(self.dbname)
                .filter(
                    Q(activity__in=self.qs),
            ...
            plotTimeSeriesDepths = (
                models.ParameterResource.objects.using(self.dbname)
                .filter(parameter__name=parameter, resource__name="plotTimeSeriesDepth")
    ...
    def _get_activity_nominaldepths(self, p):
        ...
        pr_qs = models.ParameterResource.objects.using(self.dbname).filter(
            parameter__name=p, resource__name="plotTimeSeriesDepth"
        )
        if not pr_qs:
            # See if there is one for standard_name
            pr_qs = models.ParameterResource.objects.using(self.dbname).filter(
                parameter__standard_name=p, resource__name="plotTimeSeriesDepth"
            )
        try:
            for pr in pr_qs:
                logger.debug(
                    "pr.parameter.name, pr.resource.value = {}, {}".format(
                        pr.parameter.name, pr.resource.value
                    )
                )
    ...
    def _getParameterTimeFromAP(self, pt, pa_units, a, p):
        ...
        aps = (
            models.ActivityParameter.objects.using(self.dbname)
            .filter(activity=a, parameter__name=p)
            .values("min", "max")
        )
        if not aps:
            aps = (
                models.ActivityParameter.objects.using(self.dbname)
                .filter(activity=a, parameter__standard_name=p)
                .values("min", "max")
            )
    ...
    def _buildParameterTime(
        self, pa_units, is_standard_name, ndCounts, pt, strides, pt_qs_mp
    ):
        """
        ...
        for pcount, (p, u) in enumerate(list(pa_units.items())):
            logger.debug(
                "is_standard_name = %s.  p, u = %s, %s", is_standard_name, p, u
            )
            ...
            if is_standard_name[p]:
                ...
                logger.info(
                    f"Only coords selected, using {dummy_parm} to go through MPs to get coords"
                )
            ...
            qs_awp = qs_awp.filter(
                Q(activityresource__resource__value__icontains="timeseries")
                | Q(
                    activityparameter__parameter__parameterresource__resource__name__icontains="plotTimeSeriesDepth"
                )
            ).distinct()
            ...
            if not ndCounts[p]:
                ndCounts[p] = (
                    1  # Trajectories with plotTimeSeriesDepth will not have a nominal depth, set to 1 for calculation below
                )
            ...
            for acount, a in enumerate(
                qs_awp.distinct("startdate", "name").order_by("startdate")
            ):
                ...
                logger.debug(
                    "a.name = %s, a.startdate = %s, a.enddate %s, aseconds = %s, secondsperpixel = %s",
                    a.name,
                    a.startdate,
                    a.enddate,
                    aseconds,
                ...
                if (
                    float(aseconds) > float(secondsperpixel)
                    or len(self.kwargs.get("platforms")) == 1
                ):
                    # Multiple points of this activity can be displayed in the flot, get an appropriate stride
                    logger.debug(
                        "PIXELS_WIDE = %s, ndCounts[p] = %s", PIXELS_WIDE, ndCounts[p]
                    )
                    stride = int(round(qs_mp_a.count() / PIXELS_WIDE / ndCounts[p]))
                    if stride < 1:
                        stride = 1
                    logger.debug(
                        "Getting timeseries from MeasuredParameter table with stride = %s",
                        stride,
                    )
                    strides[p][a.name] = stride
                    logger.debug("Adding timeseries for p = %s, a = %s", p, a)
                    pt, units, strides = self._getParameterTimeFromMP(
                        qs_mp_a,
                        pt,
                        pa_units,
                        a,
                    ...
                    if (
                        self.kwargs["parametertimeplotcoord"]
                        and acount == 0
                        and pcount == 0
                    ):
                        pt, units, strides = self._append_coords_to_pt(
                            qs_mp, pt, pa_units, a, stride, units, strides
                        )
    ...
    def getParameterTime(self):
        ...
        for plats in list(self.getPlatforms().values()):
            for platform in plats:
                ...
                if (
                    platform[3].lower() == "timeseriesprofile"
                    or platform[3].lower() == "timeseries"
                ):
                    # Do cheap query to count the number of timeseriesprofile or timeseries parameters
                    timeSeriesParmCount = (
                        models.Parameter.objects.using(self.dbname)
                        .filter(
                            activityparameter__activity__activityresource__resource__name__iexact="featureType",
                            activityparameter__activity__activityresource__resource__value__iexact=platform[
                                3
                            ].lower(),
                            activityparameter__activity__platform__name=platform[0],
                        )
                ...
                if counts and (
                    self.kwargs.get("parametertimeplotid")
                    or "parametertimeplotcoord" in self.kwargs
                ):
                    if (
                        "parametertime" in self.kwargs["only"]
                        or self.kwargs["parametertab"]
                    ):
                        # Initialize structure organized by units for parameters left in the selection
                        logger.debug(
                            "Calling self._collectParameters() with platform = %s",
                            platform,
                        )
                        pa_units, is_standard_name, ndCounts, pt, colors, strides = (
                            self._collectParameters(
                                platform,
                                pt,
                                pa_units,
                                is_standard_name,
        ...
        if pa_units:
            ...
            pt, units, strides = self._buildParameterTime(
                pa_units, is_standard_name, ndCounts, pt, strides, pt_qs_mp
            )
    ...
    def getSampleDepthTime(self):
        ...
        if self.getSampleQS():
            ...
            if self.kwargs.get("exclude_ans"):
                qs = qs.exclude(
                    instantpoint__activity__name__in=self.kwargs.get("exclude_ans")
                )
            ...
            for s in qs:
                ...
                if s[2].find("_decim") != -1:
                    label = "%s %s" % (
                        s[2].split("_decim")[0],
                        s[3],
                    )  # Lop off '_decim.nc (stride=xxx)' part of name
                elif s[2].find(" ") != -1:
                    label = "%s %s" % (
                        s[2].split(" ")[0],
                        s[3],
                    )  # Lop off everything after a space in the activity name
                else:
                    label = "%s %s" % (
                        s[2],
                        s[3],
                    )  # Show entire Activity name & sample name
    ...
    def getSampleDurationDepthTime(self):
        ...
        sample_durations = []
        try:
            nettow = models.SampleType.objects.using(self.dbname).get(
                name__contains=NETTOW
            )
        except models.SampleType.DoesNotExist:
            nettow = None
        try:
            planktonpump = models.SampleType.objects.using(self.dbname).get(
                name__contains=PLANKTONPUMP
            )
        except models.SampleType.DoesNotExist:
            planktonpump = None
        try:
            esp_archive = models.SampleType.objects.using(self.dbname).get(
                name__contains=ESP_FILTERING
            )
        except models.SampleType.DoesNotExist:
            esp_archive = None
        try:
            esp_archive_at = models.ActivityType.objects.using(self.dbname).get(
                name__contains=ESP_FILTERING
            )
        ...
        if self.getSampleQS() and (nettow or planktonpump):
            qs = (
                self.getSampleQS()
                .filter(Q(sampletype=nettow) | Q(sampletype=planktonpump))
                .values_list(
                    "instantpoint__timevalue",
                    "depth",
                    "instantpoint__activity__name",
            ...
            for s in qs:
                ...
                try:
                    rec = {
                        "label": label,
                        "data": [[s_ems, "%.2f" % s[7]], [e_ems, "%.2f" % s[6]]],
                    }
                except TypeError:
            ...
            for samp in samples:
                ...
                if " " in samp.name:
                    label = "%s %s" % (
                        samp.name.split(" ")[0],
                        sample_number,
                    )  # Lop off everything after first space in the activity name
                else:
                    label = "%s %s" % (
                        samp.name,
                        sample_number,
                ...
                sample_durations.append(
                    {"label": label, "data": samp_depth_time_series}
                )
    ...
    def getActivityParameterHistograms(self):
        ...
        showStandardnameParameterValuesFlag = getShow_StandardName_Parameter_Values(
            self.kwargs
        )
        for pa in models.Parameter.objects.using(self.dbname).order_by("name").all():
            ...
            for aph in (
                self.getActivityParameterHistogramsQS()
                .select_related()
                .filter(activityparameter__parameter=pa)
                .values(
                    "activityparameter__activity__name",
                    "activityparameter__activity__platform__name",
                    "binlo",
                    "binhi",
                    "bincount",
                )
                ...
                if np.isnan(aph["binlo"]) or np.isnan(aph["binhi"]):
                    continue
                try:
                    histList[aph["activityparameter__activity__name"]].append(
                        [aph["binlo"], aph["bincount"]]
                    )
                except KeyError:
                    ...
                    platformList[aph["activityparameter__activity__name"]].append(
                        aph["activityparameter__activity__platform__name"]
                    )
    ...
    def _build_mpq_queryset(self):
        ...
        if "parameterplot" in self.kwargs:
            if self.kwargs["parameterplot"][0]:
                parameterID = self.kwargs["parameterplot"][0]
                parameter = models.Parameter.objects.using(
                    self.request.META["dbAlias"]
                ).get(id=parameterID)
                parameterGroups = getParameterGroups(
                    self.request.META["dbAlias"], parameter
                )
        ...
        if "parametercontourplot" in self.kwargs:
            if self.kwargs["parametercontourplot"][0]:
                ...
                contourparameter = models.Parameter.objects.using(
                    self.request.META["dbAlias"]
                ).get(id=contourparameterID)
                contourparameterGroups = getParameterGroups(
                    self.request.META["dbAlias"], contourparameter
                )
    ...
    def getParameterDatavaluePNG(self):
        ...
        if SAMPLED in parameterGroups:
            ...
            cp = MeasuredParameter(
                self.kwargs,
                self.request,
                self.qs,
                self.mpq.qs_sp_no_order,
                self.contour_mpq.qs_sp_no_order,
                min_max,
                self.getSampleQS(),
                platformName,
                parameterID,
            ...
            cp = MeasuredParameter(
                ...
                self.request,
                self.qs,
                self.mpq.qs_mp_no_order,
                self.contour_mpq.qs_mp_no_order,
                min_max,
                self.getSampleQS(),
                platformName,
                parameterID,
    ...
    def getPDV_IFSs(self):
        ...
        if contourFlag and self.kwargs.get("showgeox3dmeasurement"):
            ...
            for pns in self._combine_sample_platforms(platformName):
                ...
                for act in self.qs.filter(
                    Q(platform__name=platform_single)
                    & ~Q(activitytype__name=LRAUV_MISSION)
                ):
                    ...
                    logger.info(
                        f"Rendering image for pns='{pns}', act.name='{act.name}'"
                    )
                    cp = MeasuredParameter(
                        self.kwargs,
                        self.request,
                    ...
                    x3d_items, shape_id_dict = cp.curtainX3D(
                        pns,
            ...
            if x3d_dict:
                x3d_dict["speedup"] = self._get_speedup(
                    {act.platform for act in self.qs}
                )
                cycInt = (
                    self.max_end_time - self.min_start_time
                ).total_seconds() / x3d_dict["speedup"]
                x3d_dict["timesensor"] = PlatformAnimation.timesensor_template.format(
                    cycInt=cycInt
                )

                sec_interval = (cp.x[2] - cp.x[1]) * cp.scale_factor
                spaced_ts = np.arange(
                    self.min_start_time.timestamp(),
                    self.max_end_time.timestamp(),
                    sec_interval,
                )
    ...
    def getParameterParameterPNG(self):
        ...
        if "parameterparameter" in self.kwargs:
            ...
            if px and py:
                ...
                pMinMax = {
                    "x": self.getParameterMinMax(px, percentileAggregateType="extrema")[
                        "plot"
                    ],
                ...
                if not pMinMax["x"] or not pMinMax["y"]:
                    return (
                        "",
                        "Selected x and y axis parameters are not in filtered selection.",
                    )
                self.pp = ParameterParameter(
                    self.kwargs,
                    self.request,
                    {"x": px, "y": py, "c": pc},
                    self.mpq,
        ...
        if "parameterparameter" in self.kwargs:
            ...
            if px and py and pz:
                ...
                if not pMinMax["x"] or not pMinMax["y"] or not pMinMax["z"]:
                    return (
                        "",
                        "Selected x, y, z, c Parameters not in filtered selection.",
                    )

                logger.debug(
                    "Instantiating Viz.PropertyPropertyPlots for X3D............................................"
                )
                self.pp = ParameterParameter(
                    self.kwargs,
                    self.request,
    ...
    def getX3DSimulations(self):
        ...
        for similation_url in (
            ...
            x3d_dict[similation_url]["directory"] = (
                models.Resource.objects.using(self.request.META["dbAlias"])
                .filter(
                    resourcetype__name="x3dsimulation",
                    name="directory",
                    uristring=similation_url,
                )
                .values_list("value", flat=True)
            )[0]
            x3d_dict[similation_url]["half_time_step_secs"] = float(
                (
                    models.Resource.objects.using(self.request.META["dbAlias"])
                    .filter(
                        resourcetype__name="x3dsimulation",
                        name="half_time_step_secs",
                        uristring=similation_url,
                    )
                    .values_list("value", flat=True)
                )[0]
            )
            x3d_dict[similation_url]["variable"] = (
                models.Resource.objects.using(self.request.META["dbAlias"])
                .filter(
                    resourcetype__name="x3dsimulation",
                    name="variable",
            ...
            x3d_dict[similation_url]["data_range"] = (
                models.Resource.objects.using(self.request.META["dbAlias"])
                .filter(
                    resourcetype__name="x3dsimulation",
                    name="data_range",
            ...
            # Apply selected vertical exageration to the simulation geocoords and dimensions, default ve is 10
            geocoords = (
                models.Resource.objects.using(self.request.META["dbAlias"])
                .filter(
                    resourcetype__name="x3dsimulation",
            ...
            x3d_dict[similation_url]["dimensions"] = (
                f"{dimensions.split()[0]} {(int(self.request.GET.get('ve', 100)) * float(dimensions.split()[1])):.1f} {dimensions.split()[2]}"
            )
            x3d_dict[similation_url]["geocoords"] = (
                f"{geocoords.split()[0]} {geocoords.split()[1]} {(int(self.request.GET.get('ve', 100)) * float(geocoords.split()[2])):.1f}"
            )
            ...
            (
                x3d_dict[similation_url]["slicesOverX"],
                x3d_dict[similation_url]["slicesOverY"],
            ) = [int(n) for n in tile_dims.split("x")]
            x3d_dict[similation_url]["numberOfSlices"] = (
                x3d_dict[similation_url]["slicesOverX"]
                * x3d_dict[similation_url]["slicesOverY"]
            )
            ...
            try:
                ...
                if parameterplotid := self.request.GET.get("parameterplotid"):
                    logger.info(
                        f"cmincmax not set. Trying to get plot limits for {parameterplotid=}"
                    )
                    cmin, cmax = self.getParameterMinMax(int(parameterplotid))["plot"][
                        1:
                    ]
            logger.info(f"{cmin =}, {cmax =}")
            if cmin is not None and cmax is not None:
                dmin, dmax = [
                    float(d) for d in x3d_dict[similation_url]["data_range"].split()
                ]
                dr_cm_file = os.path.join(
                    settings.MEDIA_ROOT,
                    "simulations",
                    f"{x3d_dict[similation_url]['variable']}_{self.request.session['sessionID']}.png",
                )
                ...
                cm.data_range_colormap(
                    selected_colormap,
                    dmin,
                    dmax,
                    cmin,
                    cmax,
                    int(selected_num_colors),
                    dr_cm_file,
                )
                x3d_dict[similation_url]["colormap"] = (
                    f"{x3d_dict[similation_url]['variable']}_{self.request.session['sessionID']}.png"
                )
            else:
                logger.info(
                    "Not creating data_range colormap image as cmin and cmax not set."
                )
            ...
            for file in sorted(
                glob.glob(
                    os.path.join(
                        settings.MEDIA_ROOT, "simulations", similation_dir, "sim_*.png"
                    )
                )
            ):
                try:
                ...
                if start_esec < esec and esec < end_esec:
                    x3d_dict[similation_url]["image_atlases"].append(
        ...
        if self.kwargs.get("showgeox3dmeasurement") and "parameterplot" in self.kwargs:
            ...
            for pns in self._combine_sample_platforms(platformName):
                ...
                for act in self.qs.filter(
                    ...
                    logger.info(
                        f"Getting dataValues for pns='{pns}', act.name='{act.name}'"
                    )
                    cp = MeasuredParameter(
                        self.kwargs,
                        self.request,
                    ...
                    x3d_items, shape_id_dict = cp.dataValuesX3D(
                        platform_single,
                        float(self.request.GET.get("ve", 10)),
    ...
    def getPlatformAnimation(self):
        ...
        if self.request.GET.get("showplatforms", False):
            ...
            platformsHavingModels = {
                pr.platform
                for pr in models.PlatformResource.objects.using(self.dbname).filter(
                    resource__resourcetype__name=X3DPLATFORMMODEL,
                    platform__in=[a.platform for a in self.qs],
                )
            }
            platforms_trajectories = {
                ar.activity.platform
                for ar in models.ActivityResource.objects.using(self.dbname).filter(
                    resource__name="featureType",
                    resource__value="trajectory",
            ...
            platforms_rotations = {
                ar.activity.platform
                for ar in models.ActivityResource.objects.using(self.dbname).filter(
                    activity__activityparameter__parameter__name="AXIS_X",
                    activity__platform__in=[a.platform for a in self.qs],
                )
            }

            platforms_to_animate = platformsHavingModels & (
                platforms_trajectories | platforms_rotations
            )
            if platforms_to_animate:
                # Use qs_mp_no_parm QuerySet as it contains roll, pitch, and yaw values
                mppa = PlatformAnimation(
                    platforms_to_animate,
                    self.kwargs,
                    self.request,
                    self.qs,
                    self.mpq.qs_mp_no_parm,
                )
    ...
    def getX3DTerrains(self):
        ...
        try:
            for r in (
                models.Resource.objects.using(self.dbname)
                .filter(resourcetype__name="x3dterrain")
                .all()
            ):
                try:
    ...
    def getX3DPlaybacks(self):
        ...
        try:
            for r in (
                models.Resource.objects.using(self.dbname)
                .filter(resourcetype__name="x3dplayback")
                .values(
                    "uristring", "name", "value", "activityresource__activity__name"
                )
            ):
                ms = models.Measurement.objects.using(self.dbname).filter(
                    instantpoint__activity__name=r["activityresource__activity__name"]
                )
                try:
                    x3dpHash[r["uristring"]][r["name"]] = r["value"]
                    x3dpHash[r["uristring"]]["startGeoCoords"] = "%s %s %s" % (
                        ms[0].geom.y,
                        ms[0].geom.x,
                        -ms[0].depth,
                    )
            ...
            logger.warn(
                "No resourcetype__name of x3dplayback in %s: %s", self.dbname, e
            )
    ...
    def getResources(self):
        ...
        logger.debug(
            "Begining to loop though ActivityResource query to build netcdfHash..."
        )
        # Original way of loading netCDF nc_global attributes
        nc_global_names = ["title", "summary", "comment"]
        ars = (
            models.ActivityResource.objects.using(self.dbname)
            .filter(activity__in=self.qs, resource__name__in=nc_global_names)
            .values(
                "activity__platform__name",
                "activity__name",
                "activity__comment",
                "resource__name",
                "resource__value",
        ...
        for ar in (
            ...
            try:
                ...
                netcdfHash[ar["activity__platform__name"]][ar["activity__name"]][
                    "comment"
                ] = ar["activity__comment"]
                netcdfHash[ar["activity__platform__name"]][ar["activity__name"]][
                    "loaded_date"
                ] = ar["activity__loaded_date"]
            except KeyError:
                try:
        ...
        logger.debug(
            "Begining to loop though ActivityResource query to build qlHash..."
        )
        for ar in (
            models.ActivityResource.objects.using(self.dbname)
            .filter(activity__in=self.qs, resource__resourcetype__name="quick_look")
            .values(
                "activity__platform__name",
                "activity__name",
                "resource__name",
            ...
            qlHash[ar["activity__platform__name"]][ar["activity__name"]][
                ar["resource__name"]
            ] = ar["resource__uristring"]
        logger.debug("Done building qlHash.")

        # Campaign information
        c_hash = {}
        logger.debug(
            "Begining to loop though ActivityResource query to build c_hash..."
        )
    ...
    def getAttributes(self):
        ...
        sources = (
            models.ResourceResource.objects.using(self.dbname)
            .filter(toresource__name=COMMANDLINE)
        ...
        for mpr in (
            models.MeasuredParameterResource.objects.using(self.dbname)
            .filter(activity__in=self.qs, resource__name__in=[LABEL, LRAUV_MISSION])
            .values("resource__resourcetype__name", "resource__value", "resource__id")
            .distinct()
            .order_by("resource__value")
        ):
            # Include all description resources associated with this label
            descriptions = " ".join(
                models.ResourceResource.objects.using(self.dbname)
                .filter(
                    fromresource__id=mpr["resource__id"], toresource__name=DESCRIPTION
                )
                .values_list("toresource__value", flat=True)
            )
            try:
                measurementHash[mpr["resource__resourcetype__name"]].append(
                    (mpr["resource__id"], mpr["resource__value"], descriptions)
                )
            except KeyError:
        ...
        logger.debug(
            "Returning from getAttributes with  measurementHash = %s", measurementHash
        )
    ...
    def _parameterstandardnameQ(self, parameterstandardname, fromTable="Activity"):
        ...
        if parameterstandardname is None:
            ...
            if fromTable == "Activity":
                q = Q(
                    activityparameter__parameter__standard_name__in=parameterstandardname
                )
            elif fromTable == "Sample":
                # Use sub-query to find all Samples from Activities that are in the existing Activity queryset
                q = Q(instantpoint__activity__in=self.qs)
    ...
    def _mplabelsQ(self, resourceids, fromTable="Activity"):
        ...
        if not resourceids:
            ...
            if fromTable == "Activity":
                q = Q(
                    id__in=models.MeasuredParameterResource.objects.using(self.dbname)
                    .filter(resource__id__in=resourceids)
                    .values_list("activity__id")
                    .distinct()
                )
            elif fromTable == "ActivityParameter":
                q = Q(
                    activity__id__in=models.MeasuredParameterResource.objects.using(
                        self.dbname
                    )
                    .filter(resource__id__in=resourceids)
                    .values_list("activity__id")
    ...
    def _trajectoryQ(self):
        ...
        udcc_q1 = Q(activityresource__resource__name__iexact="thredds_data_type") & Q(
            activityresource__resource__value__iexact="Trajectory"
        )
        udcc_q2 = Q(activityresource__resource__name__iexact="cdm_data_type") & Q(
            activityresource__resource__value__iexact="trajectory"
        )
        udcc_q3 = Q(activityresource__resource__name__iexact="CF%3afeatureType") & Q(
            activityresource__resource__value__iexact="trajectory"
        )
        udcc_q4 = Q(activityresource__resource__name__iexact="CF_featureType") & Q(
            activityresource__resource__value__iexact="trajectory"
        )
        cf16_q = Q(activityresource__resource__name__iexact="featureType") & Q(
            activityresource__resource__value__iexact="trajectory"
        )
    ...
    def _timeSeriesQ(self):
        ...
        # Restrict selection to Activities that are trajectories.  Can have pre CF-1.6 UCDD and CF-1.6 and later metadata.
        udcc_q1 = Q(activityresource__resource__name__iexact="thredds_data_type") & Q(
            activityresource__resource__value__iexact="station"
        )
        udcc_q2 = Q(activityresource__resource__name__iexact="cdm_data_type") & Q(
            activityresource__resource__value__iexact="station"
        )
        cf16_q = Q(activityresource__resource__name__iexact="featureType") & Q(
            activityresource__resource__value__iexact="timeSeries"
        )
    ...
    def _timeSeriesProfileQ(self):
        ...
        cf16_q = Q(activityresource__resource__name__iexact="featureType") & Q(
            activityresource__resource__value__iexact="timeSeriesProfile"
        )
    ...
    def _trajectoryProfileQ(self):
        ...
        cf16_q = Q(activityresource__resource__name__iexact="featureType") & Q(
            activityresource__resource__value__iexact="trajectoryProfile"
        )
    ...
    def getActivityGeoQuery(self, Q_object=None, pointFlag=False):
        ...
        # Query for mapserver
        geo_query = (
            "geom from (%s) as subquery using unique gid using srid=4326"
            % postgresifySQL(qs.query, pointFlag).rstrip()
        )
    ...
    def getSampleGeoQuery(self, Q_object=None):
        ...
        geo_query = (
            "geom from (%s) as subquery using unique gid using srid=4326"
            % postgresifySQL(qs.query, sampleFlag=True)
        )
    ...
    def getExtent(self, srid=4326, outputSRID=spherical_mercator_srid):
        ...
        for geom_field in ("maptrack", "mappoint", "plannedtrack"):
            try:
                ...
                logger.warn(
                    "Database %s does not have field %s", self.dbname, geom_field
                )
            except TypeError:
                pass
        ...
        logger.debug(
            "Collected %d geometry extents from Activities and Samples", len(extentList)
        )
        geom_union = None
        if extentList:
            ...
            for extent in extentList[index:]:
                if extent is not None:
                    if extent[0] == extent[2] and extent[1] == extent[3]:
                        ...
                        geom_union = geom_union.union(
                            fromstr("LINESTRING (%s %s, %s %s)" % extent, srid=srid)
                        )
            ...
            if geom_union:
                ...
                logger.exception(
                    "geom_union could not be set from extentList = %s", extentList
                )
                return ([], None, None)
            ...
            try:
                ...
                logger.exception(
                    "Tried to get extent for self.qs.query =  %s, but failed. Check the database loader and make sure a geometry type (maptrack or mappoint) is assigned for each activity.",
                    str(self.qs.query),
                )
            except ValueError:
                logger.exception(
            ...
            try:
                ...
                logger.exception(
                    "Could not get extent for geomstr = %s, srid = %d", geomstr, srid
                )
            ...
            try:
                ...
                logger.exception(
                    "Cannot get transorm to %s for geomstr = %s, srid = %d",
                    outputSRID,
                    geomstr,
                    srid,
                )
