...
class PlatformAnimation(object):
    ...
    def getX3DPlatformModel(self, pName):
        ...
        return (
            models.PlatformResource.objects.using(self.request.META["dbAlias"])
            .get(
                platform__name=pName,
    ...
    def getX3DPlatformModelScale(self, pName):
        ...
        try:
            factor = float(
                ...
                .get(
                    platform__name=pName,
                    resource__name=X3D_MODEL_SCALEFACTOR,
                    resource__resourcetype__name=X3DPLATFORMMODEL,
                )
                .resource.value
            )
            ...
            factor = 1.0
            error_msg = (
                "Platform {} has more than one Resource named {}."
                " Setting to default factor of {:.1f}'"
            ).format(pName, X3D_MODEL_SCALEFACTOR, factor)
    ...
    def loadData(self, platform):
        ...
        for mp in pqs.filter(parameter__standard_name="platform_yaw_angle"):
            self.lon_by_plat.setdefault(platform.name, []).append(
                mp["measurement__geom"].x
            )
            self.lat_by_plat.setdefault(platform.name, []).append(
                mp["measurement__geom"].y
            )
            self.depth_by_plat.setdefault(platform.name, []).append(
                mp["measurement__depth"]
            )
            ...
            self.time_by_plat.setdefault(platform.name, []).append(
                int((time.mktime(dt.timetuple()) + dt.microsecond / 1.0e6) * 1000.0)
            )
    ...
    def _assemble_platforms(
        self, platforms, vert_ex, geoOrigin, scale, speedup, force_overlap
    ):
            ...
            try:
                time_ranges[p] = Range(
                    start=datetime.utcfromtimestamp(
                        self.time_by_plat[p.name][0] / 1000.0
                    ),
                    end=datetime.utcfromtimestamp(
                        self.time_by_plat[p.name][-1] / 1000.0
                    ),
                )
            except KeyError:
                error_msg = "Cannot animate {}. Make sure it has standard_name of 'platform_yaw_angle'".format(
                    p
                )
                self.logger.warn(error_msg)
        ...
        for p, r in list(time_ranges.items()):
            if r.start < min_start_time:
                ...
                st_ems = int(
                    (
                        time.mktime(min_start_time.timetuple())
                        + min_start_time.microsecond / 1.0e6
                    )
                    * 1000.0
                )
                earliest_platform = p
            if r.end > max_end_time:
                ...
                et_ems = int(
                    (
                        time.mktime(max_end_time.timetuple())
                        + max_end_time.microsecond / 1.0e6
                    )
                    * 1000.0
                )
        ...
        for p, r in list(time_ranges.items()):
            if force_overlap:
                ...
                if self.overlap_time(time_ranges[earliest_platform], r) > 0:
                    x3d_dict[p.name] = self._animationX3D_for_platform(
                        p, vert_ex, geoOrigin, scale, st_ems, et_ems
                    )
        ...
        platforms_not_shown = set(p.name for p in platforms) - set(
            p.name for p in assembled_platforms
        )
        ...
        if self.time_by_plat:
            equal_times = np.arange(
                st_ems,
                et_ems,
                self.time_by_plat[earliest_platform.name][2]
                - self.time_by_plat[earliest_platform.name][1],
            )

        return self.x3d_info(
            x3d=x3d_dict,
            timesensor_x3d=timesensor_x3d,
            platforms=assembled_platforms,
            times=equal_times,
            limits=(0, len(equal_times)),
    ...
    def _deg2rad(self, angle):
        ...
        try:
            ...
            axis = (
                [el for el in traceback.format_stack() if "_deg2rad" in el][0]
                .split("_deg2rad(")[1]
                .split(")")[0]
            )
    ...
    def _append_animation_values(
        ...
        if (
            ...
            if vert_ex != 1:
                self.logger.warn(
                    "angle_axis orientation does not allow for vertical exaggeration"
                )
        ...
        try:
            ...
            if pad_beginning:
                for t in (st_ems, self.time_by_plat[pName][0]):
                    ...
                    self.axisValues += self.axisValuesFmt.format(
                        -0.5 * a_x, -0.5 * a_y, -0.5 * a_z, a_x, a_y, a_z
                    )
            else:
                for a_x, a_y, a_z in zip(
                    self.rot_x_by_plat[pName],
                    self.rot_y_by_plat[pName],
    ...
    def _animationX3D_for_platform(
        self, platform, vert_ex, geoOrigin, scale, st_ems, et_ems
    ):
        ...
        if self.aaRotValues:
            x3d = self.position_orientation_template_aa.format(
                pName=pName,
                plat_scale=self.getX3DPlatformModelScale(pName),
                pURL=self.getX3DPlatformModel(pName),
                pKeys=self.keys[:-1],
                posValues=self.points,
                oKeys=self.keys[:-1],
                aaRotValues=self.aaRotValues,
                axisValues=self.axisValues,
                pColor=pColor,
                scale=scale,
                geoOriginStr=geoorigin_use,
                bed_transform_beg=bed_transform_beg,
                bed_transform_end=bed_transform_end,
            )
        elif self.xRotValues and self.yRotValues and self.zRotValues:
            x3d = self.position_orientation_template.format(
                pName=pName,
                plat_scale=self.getX3DPlatformModelScale(pName),
                pURL=self.getX3DPlatformModel(pName),
                pKeys=self.keys[:-1],
                posValues=self.points,
                oKeys=self.keys[:-1],
                xRotValues=self.xRotValues,
                yRotValues=self.yRotValues,
                zRotValues=self.zRotValues,
                pColor=pColor,
                scale=scale,
                geoOriginStr=geoorigin_use,
            )
        else:
            x3d = self.position_template.format(
                pName=pName,
                plat_scale=self.getX3DPlatformModelScale(pName),
                pURL=self.getX3DPlatformModel(pName),
                pKeys=self.keys[:-1],
    ...
    def platformAnimationDataValuesForX3D(
        self, vert_ex=10.0, geoOrigin="", scale=1, speedup=1, force_overlap=False
    ):
        ...
        if len(info.times) > PA_MAX_POINTS:
            ...
            return {
                "x3d": "",
                "message": "{} are too many values to animate. Filter to get below {}.".format(
            ...
            return {
                "x3d": info.x3d,
                "timesensor": info.timesensor_x3d,
                "limits": info.limits,
                "time": info.times,
                "platforms_not_shown": info.platforms_not_shown,
                "speedup": speedup,
                "scale": scale,
