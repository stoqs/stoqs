...
class KML(BaseParameter):
    ...
    def kmlResponse(self):
        ...
        if "parameter__id" in self.qparams:
            ...
            pName = (
                m.Parameter.objects.using(self.request.META["dbAlias"])
                .get(id=int(self.qparams["parameter__id"]))
                .name
            )
        ...
        if not pName:
            raise ValueError(
                "parameter__name, parameter__standard_name, or parameter__id is not specified"
            )
        ...
        if self.stoqs_object_name == "measured_parameter":
            try:
                ...
                data = [
                    (
                        mp["measurement__instantpoint__timevalue"],
                ...
                data = [
                    (
                        mp.measurement.instantpoint.timevalue,
                        mp.measurement.geom.x,
                        mp.measurement.geom.y,
                        mp.measurement.depth,
                        mp.parameter.name,
                        mp.datavalue,
            ...
            try:
                folderName = "%s_%.1f_%.1f" % (
                    pName,
                    float(self.qparams["measurement__depth__gte"]),
                    float(self.qparams["measurement__depth__lte"]),
                )
            except KeyError:
                folderName = "%s_" % (pName,)
            ...
            try:
                ...
                data = [
                    (
                        mp["sample__instantpoint__timevalue"],
                        mp["sample__geom"].x,
                        mp["sample__geom"].y,
                        mp["sample__depth"],
                        mp["parameter__name"],
                        mp["datavalue"],
                        mp["sample__instantpoint__activity__platform__name"],
                    )
                    for mp in self.qs_mp[:: self.stride]
                ]
            except TypeError:
                ...
                data = [
                    (
                        mp.sample.instantpoint.timevalue,
                        mp.sample.geom.x,
                        mp.sample.geom.y,
                        mp.sample.depth,
                        mp.parameter.name,
                        mp.datavalue,
                        mp.sample.instantpoint.activity.platform.name,
                    )
                    for mp in self.qs_mp[:: self.stride]
                ]
            try:
                folderName = "%s_%.1f_%.1f" % (
                    pName,
                    float(self.qparams["sample__depth__gte"]),
                    float(self.qparams["sample__depth__lte"]),
                )
            except KeyError:
                folderName = "%s_" % (pName,)
        ...
        if not dataHash:
            logger.exception(
                "No data collected for making KML within the constraints provided"
            )
            return response
    ...
    def makeKML(self, dbAlias, dataHash, pName, title, desc, cmin=None, cmax=None):
        ...
        for p in m.Parameter.objects.using(dbAlias).all().values_list("name"):
            ...
            try:
                qs = (
                    m.ActivityParameter.objects.using(dbAlias)
                    .filter(parameter__name=pn)
        ...
        if cmin and cmax:
            try:
                ...
                raise InvalidLimits(
                    "Cannot make KML with specified cmin, cmax of %s, %s" % (cmin, cmax)
                )
        else:
            try:
                clim = climHash[pName]
                ...
                logger.warn(
                    'Parameter "%s" not in Parameter table in database %s',
                    pName,
                    dbAlias,
                )
...
%s
                        ...
                        time.strftime("%Y-%m-%dT%H:%M:%SZ", dt.timetuple()),
...
%s
    ...
    def _buildKMLpoints(self, data, clim):
        ...
        for c in self.clt:
            ge_color = "ff%02x%02x%02x" % (
                (round(c[2] * 255), round(c[1] * 255), round(c[0] * 255))
            )
            if _debug:
                logger.debug("c = %s", c)
...
""" % (ge_color, ge_color, os.path.join(baseURL, "images", "colordots", ge_color))
        ...
        for row in data:
            ...
            try:
                clt_index = int(
                    round(
                        (float(datavalue) - clim[0])
                        * ((len(self.clt) - 1) / float(numpy.diff(clim)))
                    )
                )
            ...
            ge_color_val = "ff%02x%02x%02x" % (
                (
                    round(self.clt[clt_index][2] * 255),
                    round(self.clt[clt_index][1] * 255),
                    round(self.clt[clt_index][0] * 255),
                )
            )
