...
from loaders import X3D_MODEL_SCALEFACTOR
...
from loaders.SampleLoaders import VERTICALNETTOW
...
from utils.utils import EPOCH_STRING
from utils.utils import pearsonr
...
MP_MAX_POINTS = (
    10000  # Set by visually examing high-res Tethys data for what looks good
)
...
class BaseParameter(object):
            ...
            if self.request.GET.get("sn_colormap"):
                if self.standard_name in cmocean_lookup.keys():
                    ...
                    if self.cm_name in ("balance", "curl", "delta", "diff", "tarn"):
                        ...
                        bminmax = round_to_n(
                            min(abs(self.pMinMax[1]), abs(self.pMinMax[2])), 2
                        )
                        self.cmin = -bminmax
                        self.cmax = bminmax
    ...
    def set_ticks_bounds_norm(
        self, parm_info, use_ui_cmincmax=True, use_ui_num_colors=True
    ):
    ...
    def makeColorBar(
        self, colorbarPngFileFullPath, parm_info, orientation="horizontal"
    ):
        """
        ...
        if parm_info[1] == parm_info[2]:
            raise Exception(
                ("Parameter has same min and max value: {}").format(parm_info)
            )
        ...
        if orientation == "horizontal":
            ...
            try:
                cp = models.Parameter.objects.using(self.request.META["dbAlias"]).get(
                    id=int(parm_info[0])
                )
                if cp.units:
...
class MeasuredParameter(BaseParameter):
    ...
    def __init__(
        ...
        contourParameterID=None,
        contourParameterGroups=(MEASUREDINSITU),
    ):
        """
        ...
        if self.parameterID:
            ...
            self.colorbarPngFile = (
                self.kwargs["measuredparametersgroup"][0]
                + "_"
                + self.platformName
                + "_colorbar_"
                + self.imageID
        ...
        if self.colorbarPngFile:
            self.colorbarPngFileFullPath = os.path.join(
                settings.MEDIA_ROOT, "sections", self.colorbarPngFile
            )
        else:
    ...
    def _fillXYZ(self, mp, sampled=False, spanned=False, activitytype=None):
        ...
        if sampled:
            if self.scale_factor:
                self.x.append(
                    mp["sample__instantpoint__timevalue"].timestamp()
                    / self.scale_factor
                )
            else:
                self.x.append(mp["sample__instantpoint__timevalue"].timestamp())
            self.y.append(mp["sample__depth"])
            self.depth_by_act.setdefault(
                mp["sample__instantpoint__activity__name"], []
            ).append(float(mp["sample__depth"]))
            self.z.append(mp["datavalue"])
            self.value_by_act.setdefault(
                mp["sample__instantpoint__activity__name"], []
            ).append(float(mp["datavalue"]))

            if "sample__geom" in list(mp.keys()):
                self.lon.append(mp["sample__geom"].x)
                self.lon_by_act.setdefault(
                    mp["sample__instantpoint__activity__name"], []
                ).append(mp["sample__geom"].x)
                self.lat.append(mp["sample__geom"].y)
                self.lat_by_act.setdefault(
                    mp["sample__instantpoint__activity__name"], []
                ).append(mp["sample__geom"].y)
            ...
            if spanned and activitytype == VERTICALNETTOW:
                ...
                self.depth_by_act_span.setdefault(
                    mp["sample__instantpoint__activity__name"], []
                ).append(
                    (
                        mp["sample__instantpoint__activity__maxdepth"],
                ...
                self.value_by_act_span.setdefault(
                    mp["sample__instantpoint__activity__name"], []
                ).append(float(mp["datavalue"]))
                ...
                if "sample__geom" in list(mp.keys()):
                    ...
                    self.lon_by_act_span.setdefault(
                        mp["sample__instantpoint__activity__name"], []
                    ).append((mp["sample__geom"].x, mp["sample__geom"].x))
                    self.latspan.append((mp["sample__geom"].y, mp["sample__geom"].y))
                    self.lat_by_act_span.setdefault(
                        mp["sample__instantpoint__activity__name"], []
                    ).append((mp["sample__geom"].y, mp["sample__geom"].y))
            ...
            if self.scale_factor:
                self.x.append(
                    mp["measurement__instantpoint__timevalue"].timestamp()
                    / self.scale_factor
                )
            else:
            ...
            self.depth_by_act.setdefault(
                mp["measurement__instantpoint__activity__name"], []
            ).append(mp["measurement__depth"])
            self.z.append(mp["datavalue"])
            self.value_by_act.setdefault(
                mp["measurement__instantpoint__activity__name"], []
            ).append(mp["datavalue"])
            ...
            if "measurement__geom" in list(mp.keys()):
                ...
                self.lon_by_act.setdefault(
                    mp["measurement__instantpoint__activity__name"], []
                ).append(mp["measurement__geom"].x)
                self.lat.append(mp["measurement__geom"].y)
                self.lat_by_act.setdefault(
                    mp["measurement__instantpoint__activity__name"], []
                ).append(mp["measurement__geom"].y)
    ...
    def loadData(self, qs_mp):
        ...
        if SAMPLED in self.parameterGroups:
            for i, mp in enumerate(qs_mp):
                ...
                if (i % 10) == 0:
                    self.logger.debug(
                        "Appended %i samples to self.x, self.y, and self.z", i
                    )

            # Build span data members for VERTICALNETTOW activity types
            # TODO: Implement other types as they are needed
            qs = qs_mp.filter(
                sample__instantpoint__activity__activitytype__name__contains=VERTICALNETTOW
            )
            for i, mp in enumerate(qs):
                self._fillXYZ(
                    mp, sampled=True, spanned=True, activitytype=VERTICALNETTOW
                )
                if (i % 10) == 0:
                    self.logger.debug(
                        "Appended %i samples to self.xspan, self.yspan, and self.zspan",
                        i,
                    )
        else:
            ...
            if qs_mp.isRawQuerySet:
                ...
                for counter, mp in enumerate(qs_mp):
                    ...
                    if counter % stride == 0:
                        ...
                        if (i % 1000) == 0:
                            self.logger.debug(
                                "Appended %i measurements to self.x, self.y, and self.z",
                                i,
                            )
            else:
                ...
                for i, mp in enumerate(qs_mp[::stride]):
                    ...
                    if (i % 1000) == 0:
                        self.logger.debug(
                            "Appended %i measurements to self.x, self.y, and self.z", i
                        )

        self.depth = self.y
        self.value = self.z

    def _get_samples_for_markers(
        self, act_type_name=None, spanned=False, exclude_act_type_name=None
    ):
        """
        ...
        ysamp = []
        sname = []
        qs = self.sampleQS.values(
            "instantpoint__timevalue", "instantpoint__activity__name", "depth", "name"
        )
        if act_type_name:
            qs = qs.filter(
                instantpoint__activity__activitytype__name__contains=act_type_name
            )
        else:
            if exclude_act_type_name:
                qs = qs.exclude(
                    instantpoint__activity__activitytype__name__contains=exclude_act_type_name
                )

        for s in qs:
            if self.scale_factor:
                xsamp.append(
                    s["instantpoint__timevalue"].timestamp() / self.scale_factor
                )
            else:
        ...
        if spanned and (
            act_type_name == VERTICALNETTOW or act_type_name == PLANKTONPUMP
        ):
            xsamp = []
            ysamp = []
            sname = []
            ...
            for s in qs:
                if self.scale_factor:
                    xsamp.append(
                        (
                            s["instantpoint__activity__startdate"].timestamp()
                            / self.scale_factor,
                ...
                ysamp.append(
                    (
                        s["instantpoint__activity__maxdepth"],
                        s["instantpoint__activity__mindepth"],
                    )
                )
                sname.append(s["instantpoint__activity__name"])
        ...
        if spanned and (act_type_name == ESP_FILTERING):
            ...
            for sample in self.sampleQS.select_related("instantpoint__activity").filter(
                instantpoint__activity__activitytype__name=act_type_name
            ):
                ...
                for td in (
                    self.qs.filter(instantpoint__sample=sample)
                    .values_list(
                        "simpledepthtime__epochmilliseconds", "simpledepthtime__depth"
                    )
    ...
    def _get_color(self, datavalue, cmin, cmax, clt=None):
        ...
        indx = int(
            round(
                (float(datavalue) - cmin) * ((len(clt.colors) - 1) / float(cmax - cmin))
            )
        )
        if indx < 0:
            indx = 0
    ...
    def _make_image(
        ...
        if self.parameterID or self.contourParameterID:
            if self.kwargs.get("activitynames"):
                sectionPngFile = "{}_{}_{}_{}_{}.png".format(
                    self.parameterID,
                    self.contourParameterID,
                    self.platformName,
                    self.kwargs["activitynames"][0].split(".nc")[0],
                    self.imageID,
                )
            else:
                sectionPngFile = "{}_{}_{}_{}.png".format(
                    self.parameterID,
                    self.contourParameterID,
                    self.platformName,
                    self.imageID,
        ...
        sectionPngFileFullPath = os.path.join(
            settings.MEDIA_ROOT, "sections", sectionPngFile
        )
        try:
        ...
        if len(cz) == 0 and len(clz) == 0:
            return (
                None,
                None,
                "No data returned from selection",
                self.cm_name,
                cmocean_lookup_str,
                self.standard_name,
            )
        ...
        if contourFlag:
            try:
                self.logger.debug(
                    "Gridding data with self.sdt_count = %d, and self.y_count = %d",
                    self.sdt_count,
                    self.y_count,
                )
                ...
                return (
                    None,
                    None,
                    "Got KeyError. Could not grid the data",
                    self.cm_name,
                    cmocean_lookup_str,
                    self.standard_name,
                )
        ...
        try:
            ...
            if self.parameterID is not None:
                if contourFlag:
                    ax.contourf(
                        ...
                        extend="both",
                        levels=np.linspace(
                            parm_info[1], parm_info[2], self.num_colors + 1
                        ),
                    )
                    ...
                    self.logger.debug(
                        f"Drawing spanned data: {len(self.xspan)} samples"
                    )
                    for xs, ys, z in zip(self.xspan, self.yspan, self.zspan):
                        try:
                            ax.plot(
                                xs,
                                ys,
                                c=self._get_color(z, parm_info[1], parm_info[2]),
                                lw=3,
                            )
                            ...
                            return (
                                None,
                                None,
                                "Can't plot identical data values of %f" % z,
                                self.cm_name,
                                cmocean_lookup_str,
                                self.standard_name,
                            )
            ...
            if self.sampleQS and SAMPLED not in self.parameterGroups:
                ...
                xsamp, ysamp, sname = self._get_samples_for_markers(
                    act_type_name=NETTOW
                )
                self.logger.debug(
                    f"Annotating scatter of Net Tows: {len(xsamp)} samples"
                )
                ax.scatter(
                    xsamp,
                    np.float64(ysamp),
                ...
                xspan, yspan, sname = self._get_samples_for_markers(
                    act_type_name=VERTICALNETTOW, spanned=True
                )
                self.logger.debug(f"Plotting Vertical Net Tows: {len(xsamp)} samples")
                for xs, ys in zip(xspan, yspan):
                    ax.plot(xs, ys, c="k", lw=2)
                    ax.scatter(
                        [xs[1]], [0], marker="o", c="w", s=15, zorder=10, edgecolors="k"
                    )

                # Sample markers for Plankton Pumps - lines
                xspan, yspan, sname = self._get_samples_for_markers(
                    act_type_name=PLANKTONPUMP, spanned=True
                )
                self.logger.debug(
                    f"Sample markers for Plankton Pumps: {len(xspan)} samples"
                )
                for xs, ys in zip(xspan, yspan):
                    ax.plot(xs, ys, c="k", lw=2)
                    ax.scatter(
                ...
                xspan, yspan, sname = self._get_samples_for_markers(
                    act_type_name=ESP_FILTERING, spanned=True
                )
                self.logger.debug(
                    f"Sample markers for ESP Archives: {len(xspan)} samples"
                )
                for xs, ys in zip(xspan, yspan):
            ...
            if (
                self.kwargs.get("showgeox3dmeasurement")
                and contourFlag
                and self.kwargs.get("activitynames")
            ):
                self.logger.debug(
                    f"Writing curtain X3D file {sectionPngFileFullPath} with dpi=480"
                )
                fig.savefig(sectionPngFileFullPath, dpi=480, transparent=True)
            elif full_screen:
                self.logger.debug(
                    f"Writing full_screen file {sectionPngFileFullPath} with dpi=240"
                )
    ...
    def _plot_limits(self):
        ...
        if not tmin and not tmax:
            if (
                self.kwargs["flotlimits"][0] is not None
                and self.kwargs["flotlimits"][1] is not None
            ):
        ...
        if "depth" in self.kwargs:
            if (
                self.kwargs["depth"][0] is not None
                and self.kwargs["depth"][1] is not None
            ):
        ...
        if not dmin and not dmax:
            if (
                self.kwargs["flotlimits"][2] is not None
                and self.kwargs["flotlimits"][3] is not None
            ):
    ...
    def renderDatavaluesNoAxes(
        ...
        if "sessionID" in self.request.session:
            ...
            sessionID = "".join(
                random.choice(string.ascii_uppercase + string.digits) for x in range(7)
            )
        ...
        if tmin and tmax:
            self.sdt_count = (
                self.qs.filter(platform__name__in=self.platformName.split(","))
                .values_list("simpledepthtime__depth")
                .count()
            )
            self.sdt_count = int(
                self.sdt_count / 2
        ...
        if xi is not None and yi is not None:
            ...
            try:
                ...
                self.logger.warn(e)
                self.logger.debug(
                    "Not setting self.scale_factor.  Scatter plots will still work."
                )
            else:
            ...
            cz = list(self.z)
            self.logger.debug(
                "Number of cx, cy, cz data values retrieved from database = %d", len(cz)
            )
            ...
            if self.contourParameterID is not None:
                ...
                clz = list(self.z)
                self.logger.debug(
                    "Number of clx, cly, clz data values retrieved from database = %d",
                    len(clz),
                )
            ...
            self.logger.warn(
                "xi and yi are None.  tmin, tmax, dmin, dmax = %s, %s, %s, %s",
                tmin,
                tmax,
                dmin,
                dmax,
    ...
    def _get_ils(
        self, act, istart, iend, vert_ex, lon_attr, lat_attr, depth_attr, value_attr
    ):
        ...
        indices = ""
        index = 0
        for lon, lat, depth, value in zip(
            lon_sliced, lat_sliced, depth_sliced, value_sliced
        ):
            try:
                cindx = int(
                    round(
                        (value - float(self.pMinMax[1]))
                        * (len(self.clt) - 1)
                ...
                logger.error(
                    "Can't make color lookup table with min and max being the same, self.pMinMax = %s",
                    self.pMinMax,
                )
                raise e
            ...
            if lon_attr.endswith("_span"):
                points = points + "%.5f %.5f %.1f %.5f %.5f %.1f " % (
                    lats[0],
                    lons[0],
                    -depths[0] * vert_ex,
                    lats[1],
                ...
                colors = colors + "%.3f %.3f %.3f %.3f %.3f %.3f " % (
                    self.clt[cindx][0],
                    self.clt[cindx][1],
                    self.clt[cindx][2],
                    self.clt[cindx][0],
    ...
    def dataValuesX3D(self, platform_name, vert_ex=10.0, slice_minutes=10):
        ...
        try:
            for act in list(self.value_by_act.keys()):
                ...
                self.logger.debug(
                    f"Slicing pairwise {len(self.lon_by_act[act])} lat & lon points at indices {slice_indices} for {slice_minutes} minute intervals"
                )
                for istart, iend, end_esecs in zip(
                    slice_indices, slice_indices[1:], slice_esecs[1:]
                ):
                    shape_id = f"ils_{platform_name}_{int(end_esecs)}"
                    shape_id_dict[int(end_esecs)] = [shape_id]
                    self.logger.debug(
                        f"Getting IndexedLineSet data for shape_id: {shape_id}"
                    )
                    iendp1 = iend + 1
                    if iendp1 > len(self.lon_by_act[act]) - 1:
                        iendp1 = iend
                    points, colors, indices = self._get_ils(
                        act,
                        istart,
                        iendp1,
                        vert_ex,
                    ...
                    x3d_results[shape_id] = {
                        "colors": colors.rstrip(),
                        "points": points.rstrip(),
                        "index": indices.rstrip(),
                    }
    ...
    def curtainX3D(self, platform_names, vert_ex=10.0, slice_minutes=10):
        ...
        sectionPngFile, colorbarPngFile, _, _, _, _ = self.renderDatavaluesNoAxes(
            forFlot=False, measurement_markers=False
        )
        ...
        sectionPngFileTrimmedFullPath = os.path.join(
            settings.MEDIA_ROOT, "sections", sectionPngFileTrimmed
        )
        ...
        if len(self.kwargs["platforms"]) > 1:
            ...
            self.logger.debug(
                f"renderDatavaluesNoAxes(loadDataOnly=True) for self.kwargs['platforms'] = {self.kwargs['platforms']}"
            )
            sectionPngFile, colorbarPngFile, _, _, _, _ = self.renderDatavaluesNoAxes(
                forFlot=False, loadDataOnly=True, measurement_markers=False
            )
            if not sectionPngFile:
                self.kwargs["platforms"] = saved_platforms
                return x3d_results, shape_id_dict
        ...
        slice_indices, slice_esecs = self._get_slices(self.x, slice_minutes)
        self.logger.debug(
            f"Slicing {len(self.lon)} lat & lon points at indices {slice_indices} for {slice_minutes} minute intervals"
        )
        last_frac = 0.0
        for sindex, eindex, end_esecs in zip(
            slice_indices, slice_indices[1:], slice_esecs[1:]
        ):
            ...
            points = ""
            points += "{:.5f} {:.5f} {:.1f} ".format(
                self.lat[sindex], self.lon[sindex], -self.dmax * vert_ex
            )
            points += "{:.5f} {:.5f} {:.1f} ".format(
                self.lat[eindex], self.lon[eindex], -self.dmax * vert_ex
            )
            points += "{:.5f} {:.5f} {:.1f} ".format(
                self.lat[eindex], self.lon[eindex], -self.dmin * vert_ex
            )
            points += "{:.5f} {:.5f} {:.1f} ".format(
                self.lat[sindex], self.lon[sindex], -self.dmin * vert_ex
            )
            ...
            tc_points += "{last_frac:.5f} 0 {frac:.5f} 0 ".format(
                last_frac=last_frac, frac=fraction
            )
            tc_points += "{frac:.5f} 1 {last_frac:.5f} 1 ".format(
                last_frac=last_frac, frac=fraction
            )
            last_frac = fraction

            self.logger.debug(
                f"len(points.strip().split(' '))/3 = {len(points.strip().split(' ')) / 3}"
            )
            self.logger.debug(
                f"len(tc_points.strip().split(' '))/2 = {len(tc_points.strip().split(' ')) / 2}"
            )
            self.logger.debug(
                f"len(indices.strip().split(' '))/1 = {len(indices.strip().split(' ')) / 1}"
            )
            self.logger.debug(
                f"Faces: len(ifs_tcindex.strip().split(' '))/5 = {len(ifs_tcindex.strip().split(' ')) / 5}"
            )

            x3d_results[shape_id] = {
                "points": points.rstrip(),
                "ifs_cindex": ifs_cindex.rstrip(),
                "ifs_tcindex": ifs_tcindex.rstrip(),
                "tc_points": tc_points.rstrip(),
                "info": "",
                "image": sectionPngFileTrimmed,
            }
...
class ParameterParameter(BaseParameter):
    ...
    def _getXYCData(
        self, strideFlag=True, latlonFlag=False, returnIDs=False, sampleFlag=True
    ):
        ...
        def inner_getXYCData(self, strideFlag, latlonFlag):
            ...
            if latlonFlag:
                if sql.find("stoqs_measurement") != -1:
                    ...
                    sql = sql.replace(
                        "DISTINCT",
                        "DISTINCT ST_X(stoqs_measurement.geom) AS lon, ST_Y(stoqs_measurement.geom) AS lat,\n",
                    )
                elif sql.find("stoqs_sample") != -1:
                    self.logger.debug("Adding lon lat to SELECT")
                    sql = sql.replace(
                        "DISTINCT",
                        "DISTINCT ST_X(stoqs_sample.geom) AS lon, ST_Y(stoqs_sample.geom) AS lat,\n",
                    )
            ...
            try:
                ...
                self.logger.warn(
                    "Cannot execute sql query for Parameter-Parameter plot: %s", e
                )
                raise PPDatabaseException(infoText, sql)
            ...
            if sampleFlag:
                ...
                try:
                    ...
                    self.logger.warn(
                        "Cannot execute sample_sql query for Parameter-Parameter plot: %s",
                        e,
                    )
            ...
            counter = 0
            self.logger.debug(
                "Looping through rows in cursor with a stride of %d...", stride_val
            )
            for row in cursor:
                ...
                if counter % 1000 == 0:
                    self.logger.debug(
                        "Made it through %d of %d points", counter, pp_count
                    )
    ...
    def make2DPlot(self):
        ...
        try:
            ...
            ppPngFile = "%s_%s_%s_%s.png" % (
                self.pDict["x"],
                self.pDict["y"],
                self.pDict["c"],
                imageID,
            )
            ppPngFileFullPath = os.path.join(
                settings.MEDIA_ROOT, "parameterparameter", ppPngFile
            )
            if not os.path.exists(os.path.dirname(ppPngFileFullPath)):
                try:
                    ...
                    self.logger.exception(
                        "Failed to create path for " + "parameterparameter (%s) file",
                        ppPngFile,
                    )
                    return (
                        None,
            ...
            if self.c:
                ...
                self.logger.debug(
                    "Making colored scatter plot of %d points", len(self.x)
                )
                self.set_ticks_bounds_norm(
                    self.pMinMax["c"], use_ui_cmincmax=False, use_ui_num_colors=False
                )
                try:
                    if (
                        self.kwargs["parameterparameter"][3]
                        == self.kwargs["parameterplot"][0]
                    ):
                ...
                try:
                    cp = models.Parameter.objects.using(
                        self.request.META["dbAlias"]
                    ).get(id=int(self.pDict["c"]))
                    if cp.units:
                        if cp.units in cp.name:
                            cb.set_label(cp.name)
                        else:
                ...
                warnings.filterwarnings(
                    "ignore",
                    message="omni_normtest is not valid with less than 8 observations; 6 samples were given",
                )
                ax.scatter(self.x, self.y, marker=".", s=10, c="k", lw=0, clip_on=False)

            # Label the axes
            try:
                xp = models.Parameter.objects.using(self.request.META["dbAlias"]).get(
                    id=int(self.pDict["x"])
                )
            ...
            try:
                yp = models.Parameter.objects.using(self.request.META["dbAlias"]).get(
                    id=int(self.pDict["y"])
                )
            ...
            if stride_val > 1:
                infoText += (
                    " (of %d, stride = %d - Check No stride for unstrided plot)"
                    % (pp_count, stride_val)
                )
            infoText += (
                "<br>%s ranges: fixed [%f, %f], actual [%f, %f]<br>%s ranges: fixed [%f, %f], actual [%f, %f]"
                % (
                    xp.name,
                    self.pMinMax["x"][1],
                    self.pMinMax["x"][2],
            ...
            if (
                xp.standard_name == "sea_water_salinity"
                and yp.standard_name == "sea_water_temperature"
            ):
                X, Y, Z = self.computeSigmat(ax.axis(), xaxis_name="sea_water_salinity")
            if (
                xp.standard_name == "sea_water_temperature"
                and yp.standard_name == "sea_water_salinity"
            ):
                X, Y, Z = self.computeSigmat(
                    ax.axis(), xaxis_name="sea_water_temperature"
                )
            if Z is not None:
            ...
            if pplrFlag:
                ...
                infoText += "<br><br>OLS linear regression: {} = {} * {} + {}".format(
                    yp.name,
                    round_to_n(results.params[1], 4),
                    xp.name,
                    round_to_n(results.params[0], 4),
                )
                    ...
                    if self.c:
                        try:
                            ax.scatter(
                                ...
                                s=25,
                                cmap=self.cm,
                                vmin=self.pMinMax["c"][1],
                                vmax=self.pMinMax["c"][2],
                                clip_on=False,
                                edgecolors="k",
                            )
                        except ValueError as e:
            ...
            try:
                self.logger.debug(
                    "Saving to file ppPngFileFullPath = %s", ppPngFileFullPath
                )
    ...
    def makeX3D(self):
        ...
        def inner_makeX3D(self):
            ...
            if (
                (
                    models.Parameter.objects.using(self.request.META["dbAlias"])
                    .get(id=int(self.pDict["x"]))
                    .name
                    == "AXIS_X"
                    and models.Parameter.objects.using(self.request.META["dbAlias"])
                    .get(id=int(self.pDict["y"]))
                    .name
                    == "AXIS_Y"
                    and models.Parameter.objects.using(self.request.META["dbAlias"])
                    .get(id=int(self.pDict["z"]))
                    .name
                    == "AXIS_Z"
                )
                or (
            ...
            try:
                ...
                for x, y, z in zip(self.x, self.y, self.z):
                    ...
                    xs = (
                        10000
                        * (x - float(self.pMinMax["x"][1]))
                        / (float(self.pMinMax["x"][2]) - float(self.pMinMax["x"][1]))
                    )
                    ys = (
                        10000
                        * (y - float(self.pMinMax["y"][1]))
                        / (float(self.pMinMax["y"][2]) - float(self.pMinMax["y"][1]))
                    )
                    zs = (
                        10000
                        * (z - float(self.pMinMax["z"][1]))
                        / (float(self.pMinMax["z"][2]) - float(self.pMinMax["z"][1]))
                    )
                    points = points + "%d %d %d " % (int(xs), int(ys), int(zs))
                    if self.c:
                        cindx = int(
                            round(
                                (self.c.pop() - float(self.pMinMax["c"][1]))
                                * (len(self.clt) - 1)
                                / (
                                    float(self.pMinMax["c"][2])
                                    - float(self.pMinMax["c"][1])
                                )
                            )
                        )
                        if cindx < 0:
                ...
                # Label the axes
                try:
                    xp = models.Parameter.objects.using(
                        self.request.META["dbAlias"]
                    ).get(id=int(self.pDict["x"]))
                    if xp.units:
                        if xp.units in xp.name:
                ...
                try:
                    yp = models.Parameter.objects.using(
                        self.request.META["dbAlias"]
                    ).get(id=int(self.pDict["y"]))
                    if yp.units:
                        if yp.units in yp.name:
                ...
                try:
                    zp = models.Parameter.objects.using(
                        self.request.META["dbAlias"]
                    ).get(id=int(self.pDict["z"]))
                    if zp.units:
                ...
                if self.pDict["c"]:
                    ...
                    try:
                        ...
                        colorbarPngFile = "%s_%s_%s_%s_3dcolorbar_%s.png" % (
                            self.pDict["x"],
                            self.pDict["y"],
                            self.pDict["z"],
                            self.pDict["c"],
                        ...
                        colorbarPngFileFullPath = os.path.join(
                            settings.MEDIA_ROOT, "parameterparameter", colorbarPngFile
                        )
