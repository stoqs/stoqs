...
from stoqs.models import MeasuredParameterResource
...
class PQuerySet(object):
            ...
            try:
                ...
                for mp in self.mp_query[:ITER_HARD_LIMIT]:
                    row = {
                        ...
                        "measurement__instantpoint__timevalue": mp[
                            "measurement__instantpoint__timevalue"
                        ],
            ...
            try:
                ...
                for mp in self.mp_query[:ITER_HARD_LIMIT]:
                    row = {
                        ...
                        "measurement__instantpoint__activity__platform__name": mp[
                            "measurement__instantpoint__activity__platform__name"
                        ],
    ...
    def __getitem__(self, k):  # pragma: no cover
        ...
        if isinstance(k, slice):
            ...
            klass_idx = pk_idx + 1
            mp_pks = [
                row[pk_idx] for row in rows if row[klass_idx] is MeasuredParameter
            ]
    ...
    def count(self):
        logger.debug(
            "Counting records in self.mp_query which is of type = %s",
            type(self.mp_query),
        )
        try:
...
class PQuery(object):
    ...
    def isParameterSampled(self, id):
        ...
        """
        value = id in ParameterGroupParameter.objects.using(
            self.request.META["dbAlias"]
        ).filter(parametergroup__name=SAMPLED).values_list("parameter__id", flat=True)
        if not value:
            value = id in ParameterGroupParameter.objects.using(
                self.request.META["dbAlias"]
            ).filter(parametergroup__name=SAMPLED).values_list(
    ...
    def isParameterMeasured(self, id):
        ...
        value = id in ParameterGroupParameter.objects.using(
            self.request.META["dbAlias"]
        ).filter(parametergroup__name=MEASUREDINSITU).values_list(
    ...
    def _getQueryParms(self):
        ...
        if "platforms" in self.kwargs:
            if self.kwargs["platforms"]:
                qparams["measurement__instantpoint__activity__platform__name__in"] = (
                    self.kwargs["platforms"]
                )
        if "time" in self.kwargs:
            if self.kwargs["time"][0] is not None:
                qparams["measurement__instantpoint__timevalue__gte"] = self.kwargs[
                    "time"
                ][0]
            if self.kwargs["time"][1] is not None:
                qparams["measurement__instantpoint__timevalue__lte"] = self.kwargs[
                    "time"
                ][1]
        ...
        if "mplabels" in self.kwargs:
            if self.kwargs["mplabels"]:
                qparams["id__in"] = (
                    MeasuredParameterResource.objects.using(
                        self.request.META["dbAlias"]
                    )
                    .filter(resource__id__in=self.kwargs["mplabels"])
                    .values_list("measuredparameter__id", flat=True)
                )
    ...
    def getMeasuredParametersQS(self, values_list=[]):
        ...
        if qs_mpq:
            ...
            self.logger.debug(
                "No queryset returned for qparams = %s", pprint.pformat(qparams)
            )
    ...
    def _pvSQLfragments(self, pvDict):
        ...
        for pminmax in pvDict:
            ...
            for k, v in list(pminmax.items()):
                if k in Parameter.objects.using(
                    self.request.META["dbAlias"]
                    ...
                    if p.search(v[0]) or p.search(v[1]):
                        ...
                        raise Exception(
                            "Invalid ParameterValue constraint expression: %s, %s"
                            % (k, v)
                        )
                    ...
                    if self.isParameterMeasured(k):
                        ...
                        from_sql += (
                            "on m"
                            + str(i)
                            + ".instantpoint_id = stoqs_instantpoint.id "
                        )
                        from_sql += (
                            "INNER JOIN stoqs_measuredparameter mp" + str(i) + " "
                        )
                        from_sql += (
                            "on mp" + str(i) + ".measurement_id = m" + str(i) + ".id "
                        )
                        from_sql += "INNER JOIN stoqs_parameter p" + str(i) + " "
                        from_sql += (
                            "on mp" + str(i) + ".parameter_id = p" + str(i) + ".id "
                        )

                        where_sql += "(p" + str(i) + ".name = '" + k + "') AND "
                        if v[0]:
                            where_sql += (
                                "(mp" + str(i) + ".datavalue > " + str(v[0]) + ") AND "
                            )
                        if v[1]:
                            where_sql += (
                                "(mp" + str(i) + ".datavalue < " + str(v[1]) + ") AND "
                            )
                        where_sql += (
                            "(mp" + str(i) + ".parameter_id = p" + str(i) + ".id) AND "
                        )
                        ...
                        from_sql += (
                            "INNER JOIN stoqs_sampledparameter sp" + str(i) + " "
                        )
                        from_sql += (
                            "on sp" + str(i) + ".sample_id = s" + str(i) + ".id "
                        )
                        from_sql += "INNER JOIN stoqs_parameter p" + str(i) + " "
                        from_sql += (
                            "on sp" + str(i) + ".parameter_id = p" + str(i) + ".id "
                        )

                        where_sql += "(p" + str(i) + ".name = '" + k + "') AND "
                        if v[0]:
                            where_sql += (
                                "(sp" + str(i) + ".datavalue > " + str(v[0]) + ") AND "
                            )
                        if v[1]:
                            where_sql += (
                                "(sp" + str(i) + ".datavalue < " + str(v[1]) + ") AND "
                            )
                        where_sql += (
                            "(sp" + str(i) + ".parameter_id = p" + str(i) + ".id) AND "
                        )
    ...
    def addParameterValuesSelfJoins(
        self,
        query,
        pvDict,
        select_items="""stoqs_instantpoint.timevalue as measurement__instantpoint__timevalue, 
                                                                          stoqs_measurement.depth as measurement__depth,
                                                                          stoqs_measurement.geom as measurement__geom,
                                                                          stoqs_measuredparameter.datavalue as datavalue""",
    ):
        ...
        if add_to_from or from_sql or where_sql:
            ...
            q = q.replace(
                "SELECT FROM stoqs_measuredparameter",
                "FROM " + add_to_from + "stoqs_measuredparameter",
            )
            q = q.replace(
    ...
    def addParameterParameterSelfJoins(self, query, pDict):
        ...
        containsMeasuredFlag, containsSampleFlag = self._getContainsFlags(
            select_order, pDict
        )
                    ...
                    try:
                        ...
                        if pDict[axis] == "longitude":
                            if containsSampleFlag and not containsMeasuredFlag:
                                ...
                                xyzc_items = (
                                    xyzc_items
                                    + "ST_X(stoqs_measurement.geom) as "
                                    + axis
                                    + ", "
                                )
                            ...
                            if containsSampleFlag and not containsMeasuredFlag:
                                ...
                                xyzc_items = (
                                    xyzc_items
                                    + "ST_Y(stoqs_measurement.geom) as "
                                    + axis
                                    + ", "
                                )
                            ...
                            if containsSampleFlag and not containsMeasuredFlag:
                                xyzc_items = (
                                    xyzc_items + "stoqs_sample.depth as " + axis + ", "
                                )
                            else:
                                xyzc_items = (
                                    xyzc_items
                                    + "stoqs_measurement.depth as "
                                    + axis
                                    + ", "
                                )
                        elif pDict[axis] == "time":
                            xyzc_items += (
                                "(DATE_PART('day', stoqs_instantpoint.timevalue - timestamp '%s') * 86400 + "
                                % EPOCH_STRING
                            )
                            xyzc_items += (
                                "DATE_PART('hour', stoqs_instantpoint.timevalue - timestamp '%s') * 3600 + "
                                % EPOCH_STRING
                            )
                            xyzc_items += (
                                "DATE_PART('minute', stoqs_instantpoint.timevalue - timestamp '%s') * 60 + "
                                % EPOCH_STRING
                            )
                            xyzc_items += (
                                "DATE_PART('second', stoqs_instantpoint.timevalue - timestamp '%s') ) / 86400 "
                                % EPOCH_STRING
                            )
                            xyzc_items += " as " + axis + ", "
                        else:
                            logger.error(
                                "%s, but axis = %s is not a coordinate", e, pDict[axis]
                            )
            ...
            if pid:
                ...
                try:
                    if self.isParameterMeasured(int(pid)):
                        ...
                        self.logger.warn(
                            "Encountered parameter (id=%s) that is not in the Measured nor in the Sampled ParameterGroup",
                            pid,
                        )
                    where_sql = (
                        where_sql + "(p_" + axis + ".id = " + str(pid) + ") AND "
                    )
                except ValueError:
        ...
        q = (
            "SELECT " + select_items + q[q.find("FROM") :]
        )  # Override original select items, finds first FROM which is what we want
        ...
        if q.find("WHERE") == -1 and where_sql:
            ...
            q = q.replace(
                " WHERE ", " WHERE " + where_sql, 1
            )  # Replace only first occurance to preserve subquery
    ...
    def addSampleConstraint(self, query):
        ...
        q = q.replace(
            "FROM", ", stoqs_sample.name FROM", 1
        )  # Replace just first occurance
