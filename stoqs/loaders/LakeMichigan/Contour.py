        ...
        if self.animate: 
            ...
            try:
                ...
                while end_datetime <= data_end :
                    data_end_local = end_datetime.astimezone(pytz.timezone('America/New_York'))
                    data_start_local = start_datetime.astimezone(pytz.timezone('America/New_York'))
            ...
            try:
                if self.data is not None:
                    data_end_local = data_end.astimezone(pytz.timezone('America/New_York'))
                    data_start_local = data_start.astimezone(pytz.timezone('America/New_York'))
                    logger.debug('Plotting data')
                    self.subtitle1 = '%s  to  %s EDT' % (data_start_local.strftime('%Y-%m-%d %H:%M'), data_end_local.strftime('%Y-%m-%d %H:%M'))
