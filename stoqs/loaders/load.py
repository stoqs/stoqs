...
from stoqs.models import CampaignResource
...
class Loader(object):
    ...
    def _create_db(self, db):
        ...
        create_ext = (
            'psql -p {port} -c "CREATE EXTENSION postgis;" -d {db} -U postgres && '
        ).format(**{"port": settings.DATABASES[db]["PORT"], "db": db})
        create_ext += (
            'psql -p {port} -c "CREATE EXTENSION postgis_topology;" -d {db} -U postgres'
        ).format(**{"port": settings.DATABASES[db]["PORT"], "db": db})
    ...
    def _copy_log_file(self, log_file):
        ...
        try:
            ...
            self.prov["load_logfile"] = os.path.join(
                settings.MEDIA_URL, "loadlogs", log_file_url
            )
        except IOError as e:
        ...
        if not self.args.background:
            if os.path.isfile(log_file):
                ...
                if "minutes_to_load" not in self.prov:
                    self.logger.warning(
                        f"Did not find line starting with {MINUTES} in {log_file}"
                    )
                try:
                    # Inserted after the log_file has been written with --updateprovenance
                    self.prov["real_exection_time"] = (
                        tail(log_file, 3).split("\n")[0].split("\t")[1]
                    )
                    self.prov["user_exection_time"] = (
                        tail(log_file, 3).split("\n")[1].split("\t")[1]
                    )
                    self.prov["sys_exection_time"] = (
                        tail(log_file, 3).split("\n")[2].split("\t")[1]
                    )
                except IndexError:
            ...
            # Counts
            self.prov["MeasuredParameter_count"] = MeasuredParameter.objects.using(
                db
            ).count()
            self.prov["SampledParameter_count"] = SampledParameter.objects.using(
                db
            ).count()
    ...
    def _has_no_t_option(self, db, load_command):
        return (
            (db.endswith("_o") and "-o" in load_command)
            or "ROVCTD" in load_command
            or load_command.endswith(".sh")
            or "&&" in load_command
        )
    ...
    def _drop_indexes(self):
        ...
        model_files = (
            os.path.join(app_dir, "stoqs/migrations/0001_initial.py"),
    ...
    def checks(self):
        ...
        try:
            ...
            print(
                "The stoqs/campaigns.py could not be loaded. "
                'Create a symbolic link named "campaigns.py" '
                "pointing to the file for your site."
            )
            print(
                "Use stoqs/mbari_campaigns.py or stoqs/mbari_lrauv_campaigns.py as"
        ...
        cmd = ('psql -p {} -c "\q" -U postgres').format(
        ...
        if ret != 0:
            self.logger.warning(
                "Cannot connect to the database server as user postgres. Either run as user postgres or alter your pg_hba.conf file."
            )
            suggestion = """
...
"""
        ...
        if self.args.clobber and not self.args.test and not self.args.restore:
            print(
                (
                    "On the server running on port =",
                    settings.DATABASES["default"]["PORT"],
                )
            )
            print(
                "You are about to drop all database(s) in the list below and reload them:"
            )
            ...
            for db, load_command in list(campaigns.campaigns.items()):
                ...
                try:
                    with transaction.atomic(using=db):
                        minutes_to_load = (
                            CampaignResource.objects.using(db)
                            .get(resource__name="minutes_to_load")
                            .resource.value
                        )
                    print(f"{db:30s} {minutes_to_load:>20}")
                    nothing_printed = False
                except (
                    CampaignResource.DoesNotExist,
                    CampaignResource.MultipleObjectsReturned,
                    OperationalError,
                    ProgrammingError,
                ) as e:
        ...
        if not self.args.db and not self.args.list and not self.args.updateprovenance:
            ...
            if self.args.restore:
                ans = (
                    input("\nAre you sure you want restore all these databases? [y/N] ")
                    or "N"
                )
            else:
                ans = (
        ...
        if self.args.db and self.args.test:
            for db in self.args.db:
                if self._has_no_t_option(db, campaigns.campaigns[db]):
                    print(
                        f"{campaigns.campaigns[db]} does not support the --test argument"
                    )
                    sys.exit(-1)
    ...
    def _assign_rt_campaign(self, db, rt_name="provenance"):
        ...
        try:
            self.rt, _ = ResourceType.objects.using(db).get_or_create(
                name=rt_name, description="Information about the source of data"
            )
        except (ConnectionDoesNotExist, OperationalError, ProgrammingError) as e:
            self.logger.warning(
                'Could not open database "%s" for updating provenance.', db
            )
            self.logger.warning(e)
        ...
        while not self.campaign:
            try:
                self.logger.debug(
                    "Looking in database %s for first Campaign record", db
                )
                ...
                if self.args.background:
                    ...
                    if i > max_iter:
                        raise DatabaseLoadError(
                            (
                                "No campaign created after {:d} seconds. "
                                "Check log_file for errors: {}"
                            ).format(sec_wait * max_iter, log_file)
                        )
                else:
                    self.logger.error(
                        f"Could not find Campaign record for {db} in the database."
                    )
                    raise DatabaseLoadError()
    ...
    def recordprovenance(self, db, load_command, log_file):
        ...
        self.logger.debug(
            "Recording provenance for %s using log_file = %s", db, log_file
        )
        ...
        for name, value in list(self.prov.items()):
            ...
            cr, loaded_flag = CampaignResource.objects.using(db).get_or_create(
                campaign=self.campaign, resource=r
            )
            self.logger.info(
                'Resource uristring="%s", name="%s", value="%s"', "", name, value
            )
    ...
    def add_resource(self):
        ...
        for db, load_command in list(campaigns.campaigns.items()):
            ...
            for url in (
                Resource.objects.using(db)
                .filter(resourcetype__name="x3dterrain")
                .values_list("uristring", flat=True)
                .distinct()
            ):
                self.logger.info(
                    f"{url}: Adding default clipping plane zNear, zFar values: 100.0, 300000.0"
                )
    ...
    def pg_dump(self):
        ...
        for db, load_command in list(campaigns.campaigns.items()):
            ...
            for name, value in {
                ...
                "pg_dump_date_gmt": datetime.datetime.utcnow(),
            }.items():
                r, _ = Resource.objects.using(db).get_or_create(
    ...
    def _restore_db(self, db):
        ...
        if ret != 0:
            raise DatabaseCreationError(
                f"Could not create database: {db}. Perhaps use the --clobber option?"
            )
    ...
    def removetest(self):
        self.logger.info(
            "Removing test databases from sever running on port %s",
            settings.DATABASES["default"]["PORT"],
        )
        campaigns = importlib.import_module(self.args.campaigns)
        for db, load_command in list(campaigns.campaigns.items()):
    ...
    def load(self, campaigns=None, create_only=False, cl_args=None):
        ...
        for db, load_command in list(campaigns.campaigns.items()):
            ...
            if self.args.test:
                ...
                settings.MAPSERVER_DATABASES[campaign] = (
                    settings.MAPSERVER_DATABASES.get("default").copy()
                )
            ...
            if not appending and not self.args.restore:
                if (
                    self._db_exists(db)
                    and self.args.clobber
                    and (self.args.noinput or self.args.test)
                ):
                    self._dropdb(db)
                ...
                try:
                    ...
                    self.logger.warning(e)
                    self.logger.warning(
                        "Use the --clobber option, or fix the problem indicated."
                    )
                    if self.args.db and not self.args.test:
                        raise Exception(
                            "Maybe use the --clobber option to recreate the database..."
                        )
                    else:
            ...
            if self.args.restore:
                ...
                if script.endswith(".sh"):
                    ...
                    if appending:
                        cmd = (
                            f"(STOQS_CAMPAIGNS={db} time {script}) >> {log_file} 2>&1;"
                        )
                    else:
...
then
    ...
    tail -20 {log}) | mail -s "{db} load FAILED" {email}
            ...
            if self.args.slack:
                ...
                log_url = (
                    "http://localhost:8008/media/loadlogs/"
            ...
            if ret != 0:
                self.logger.error(
                    f"Non-zero return code from load script. Check {log_file}"
                )
    ...
    def process_command_line(self):
        ...
        examples += (
            "  Reload specific databases from as background jobs with verbose output:\n"
        )
        examples += (
            "    "
            + sys.argv[0]
            + " --db stoqs_september2013 stoqs_may2015 --clobber --background --email mccann@mbari.org -v 1\n"
        )
        examples += "  Drop specific test databases:\n"
        examples += (
            "    "
            + sys.argv[0]
            + " --db stoqs_september2010 stoqs_october2010 --removetest -v 1\n"
        )
...
"""
        ...
        parser.add_argument(
            "--campaigns",
            action="store",
            help="Module containing campaigns dictionary (must also be in campaigns.py)",
            default="campaigns",
        )
        ...
        parser.add_argument(
            ...
            action="store",
            help=(
                "Specify databases from CAMPAIGNS to load"
        ...
        parser.add_argument(
            "--test",
            action="store_true",
            help="Load test databases using -t option of loaders.LoadScript",
        )
        parser.add_argument(
            "--clobber",
            action="store_true",
            help=(
                "Drop databases before creating and loading them."
        ...
        parser.add_argument(
            "--background",
            action="store_true",
            help="Execute each load in the background to parallel process multiple loads",
        )
        parser.add_argument(
            "--removetest",
            action="store_true",
            help="Drop all test databases; the --db option limits the dropping to those in the list",
        )
        parser.add_argument(
            "--list",
            action="store_true",
            help="List the databases that are in --campaigns",
        )
        parser.add_argument(
            "--email",
            action="store",
            help="Address to send mail to when the load finishes. Does not work from Docker, use --slack instead.",
        )
        parser.add_argument(
            "--slack",
            action="store_true",
            help="Post message to stoqs-loads channel on Slack using SLACKTOKEN env variable",
        )
        parser.add_argument(
            "--updateprovenance",
            action="store_true",
            help=(
                "Use after background jobs finish to copy"
                " loadlogs and update provenance information"
            ),
        )
        parser.add_argument(
            "--grant_everyone_select",
            action="store_true",
            help="Grant everyone role select privileges on all relations",
        )
        parser.add_argument(
            "--add_resource",
            action="store_true",
            help="Add a Resource to all databases: e.g. for zNear & zFar",
        )
        parser.add_argument(
            "--drop_indexes",
            action="store_true",
            help="Before load drop indexes and create them following the load",
        )
        parser.add_argument(
            "--pg_dump",
            action="store_true",
            help='Store a pg_dump(1) with "-Fc" option file on the server',
        )
        parser.add_argument(
            "--noinput",
            action="store_true",
            help="Execute without asking for a response, e.g. for --clobber",
        )
        parser.add_argument(
            "--drop_if_fail",
            action="store_true",
            help="Drop database if fail to load data",
        )
        parser.add_argument(
            "--append", action="store_true", help="Append data to existing database"
        )
        parser.add_argument(
            "--startdate", help="Startdate in YYYYMMDD format for appending data"
        )
        parser.add_argument(
            "--current_day",
            action="store_true",
            help="Set startdate to current UTC day - useful for running directly from cron",
        )
        parser.add_argument(
            "--create_only",
            action="store_true",
            help="Just create the database and do not load the data",
        )
        parser.add_argument(
            "--restore",
            action="store",
            help="Restore databases from specified server, e.g.: stoqs.shore.mbari.org",
        )
        ...
        parser.add_argument(
            ...
            choices=[1, 2, 3],
            type=int,
            help="Turn on verbose output. If > 2 load is verbose too.",
            const=1,
            default=0,
        )
        ...
        if self.args.slack:
            try:
                ...
                print(
                    "If using --slack must set SLACKTOKEN environment variable. [Never share your token!]"
                )
                sys.exit(-1)
