...
def get_closest_instantpoint(aName, tv, dbAlias):
    ...
    while tol < 86400:  # Fail if not found within 24 hours
        qs = (
            m.InstantPoint.objects.using(dbAlias)
            .filter(
...
class HABLoader(STOQS_Loader):
    ...
    def load_measurement(self, lat, lon, depth, mtime, parmNameValues):
        ...
        mt = None
        try:
            mt = self.createMeasurement(mtime=mtime, depth=depth, lat=lat, lon=lon)
        ...
        for pn, value in parmNameValues:
            logger.info("pn = %s", pn)
            logger.info(
                "parameter._state.db = %s", self.getParameterByName(pn)._state.db
            )
            mp = m.MeasuredParameter(
                measurement=mt, parameter=self.getParameterByName(pn), datavalue=value
            )
    ...
    def load_sample(self, lon, lat, depth, timevalue, bottleName):
        ...
        try:
            activity = m.Activity.objects.using(self.dbAlias).get(
                name__contains=self.activityName
            )
            logger.debug("Got activity = %s", activity)
        except ObjectDoesNotExist:
            logger.warn(
                "Failed to find Activity with name like %s.  Skipping load.",
                self.activityName,
            )
            return
            ...
            activity = m.Activity.objects.using(self.dbAlias).filter(
                name__contains=self.activityName
            )[0]

        # Get or create SampleType
        (sample_type, created) = m.SampleType.objects.using(self.dbAlias).get_or_create(
            name="Pier"
        )
        logger.debug("sampletype %s, created = %s", sample_type, created)

        # Get or create SamplePurpose
        (sample_purpose, created) = m.SamplePurpose.objects.using(
            self.dbAlias
        ...
        try:
            ip, seconds_diff = get_closest_instantpoint(
                self.activityName, timevalue, self.dbAlias
            )
            point = Point(lon, lat)
            stuple = m.Sample.objects.using(self.dbAlias).get_or_create(
                name=bottleName,
            ...
            rtuple = m.Resource.objects.using(self.dbAlias).get_or_create(
    ...
    def process_csv_file(self, fh):
        ...
        logger.info(
            "runHABLoader(): Updating its comment with newComment = %s", newComment
        )
        aName = location

        num_updated = (
            m.Activity.objects.using(self.dbAlias)
