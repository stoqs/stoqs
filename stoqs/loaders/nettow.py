...
class NetTow:
        ...
        with open(self.args.subsampleFile, encoding="latin-1") as f:
            for r in csv.DictReader(f):
                ...
                try:
                    ...
                    sm["filterporesize"] = float(
                        r.get("Filter Pore Size [um]").split()[0]
                    )
    ...
    def _db_join(self, sm_hash):
        ...
        """
        self.logger.info(
            "Joining subsample information with cast data from the database using subtractMinutes = %d",
            self.args.subtractMinutes,
        )
        new_hash = OrderedDict()
        for a_name, v in list(sm_hash.items()):
            try:
                a = Activity.objects.using(self.args.database).get(
                    name__contains=a_name
                )
                v["longitude"] = a.mappoint.x
                v["latitude"] = a.mappoint.y
                v["datetime_gmt"] = (
                    a.startdate - timedelta(minutes=self.args.subtractMinutes)
                ).isoformat()
                new_hash[a_name] = v
            except Activity.DoesNotExist as e:
                self.logger.warn(
                    'Activity matching "%s" does not exist in database %s',
                    a_name,
                    self.args.database,
                )
                continue
                ...
                self.logger.warn(e)
                acts = (
                    Activity.objects.using(self.args.database)
                    .filter(name__contains=a_name)
                    .order_by("name")
                )
                ...
                self.logger.warn(
                    "Creating a load record for the first one, but make sure that this is what you want!"
                )
                v["longitude"] = acts[0].mappoint.x
                v["latitude"] = acts[0].mappoint.y
                v["datetime_gmt"] = (
                    acts[0].startdate - timedelta(minutes=self.args.subtractMinutes)
                ).isoformat()
                new_hash[a_name] = v
                continue
    ...
    def make_csv(self):
        ...
        try:
            ...
            self.logger.error(
                "Are there more than one net tows per cast or is this an error in file %s?",
                self.args.subsampleFile,
            )
    ...
    def _get_net_tow_platform(self, cast_platform):
        ...
        """
        pt, created = PlatformType.objects.using(self.args.database).get_or_create(
            name="ship"
        )
        ...
        platform, created = Platform.objects.using(self.args.database).get_or_create(
            name=name, platformtype=pt, color=cast_platform.color
        )
        return platform

    def _create_activity_instantpoint_platform(
        self, r, duration_minutes, nettow_number, point
    ):
        ...
        campaign = Campaign.objects.using(self.args.database).filter(
            activity__name__contains=r.get("Cast")
        )[0]
        cast_plt = Platform.objects.using(self.args.database).filter(
            activity__name__contains=r.get("Cast")
        )[0]
        platform = self._get_net_tow_platform(cast_plt)
        at, created = ActivityType.objects.using(self.args.database).get_or_create(
            name=VERTICALNETTOW
        )
        ...
        act, created = Activity.objects.using(self.args.database).get_or_create(
            ...
            comment="Plankton net tow done in conjunction with CTD cast %s"
            % r.get("Cast"),
            platform=platform,
            startdate=timevalue,
        ...
        ip, created = InstantPoint.objects.using(self.args.database).get_or_create(
            activity=act, timevalue=timevalue
        )
    ...
    def load_samples(self):
        ...
        with open(self.args.loadFile) as f:
            for r in csv.DictReader(f):
                ...
                act, ip = self._create_activity_instantpoint_platform(
                    r, duration_minutes=2, nettow_number=nettow_number, point=point
                )

                if r.get("sampletype").lower().find("vertical") != -1:
                    sampletype_name = VERTICALNETTOW
                sampletype, created = SampleType.objects.using(
                    self.args.database
                ).get_or_create(name=sampletype_name)

                samplepurpose = None
                if self.args.purpose:
                    samplepurpose, created = SamplePurpose.objects.using(
                        self.args.database
                    ).get_or_create(name=self.args.purpose)
    ...
    def process_command_line(self):
        ...
        parser.add_argument(
            "-d", "--database", action="store", help="Database alias", required=True
        )
        ...
        parser.add_argument(
            ...
            "--subtractMinutes",
            action="store",
            help="Subtract these number of minutes from start of CTD cast for net tow time",
            type=int,
            default=30,
        )
        parser.add_argument(
        ...
        if self.args.subsampleFile:
            if not self.args.csvFile:
                parser.error(
                    "Must include --csvFile argument with --subsampleFile option"
                )
