...
from loaders import HasMeasurement
from loaders import SkipRecord
from loaders import STOQS_Loader
from loaders.SampleLoaders import ClosestTimeNotFoundException
from loaders.SampleLoaders import get_closest_instantpoint
from psycopg2.errors import UniqueViolation
from pydap.client import open_url
from utils.utils import mode
from utils.utils import simplify_points

from stoqs.models import Activity
from stoqs.models import ActivityResource
from stoqs.models import InstantPoint
from stoqs.models import MeasuredParameter
from stoqs.models import Measurement
from stoqs.models import NominalLocation
from stoqs.models import Parameter
from stoqs.models import Resource
from stoqs.models import ResourceType
...
class Base_Loader(STOQS_Loader):
    ...
    def __init__(
        ...
        self.dataStartDatetime = (
            dataStartDatetime  # For when we append data to an existing Activity
        )
        ...
        try:
            ...
            socket.error,
            pydap.exceptions.ServerError,
            pydap.exceptions.ClientError,
        ):
        ...
        self.ignored_names = list(
            self.global_ignored_names
        )  # Start with copy of list of global ignored names
    ...
    def _getStartAndEndTimeFromDS(self):
        ...
        for v in self.include_names:
            ...
            if (
                self.getFeatureType() == TRAJECTORY
                or self.getFeatureType() == TRAJECTORYPROFILE
            ):
                self.logger.debug("Getting trajectory min and max times for v = %s", v)
                self.logger.debug("self.ds[ac['time']][0] = %s", self.ds[ac["time"]][0])
                try:
                    minDT[v] = num2date(
                        self.ds[ac["time"]].data[0][0],
                        self.ds[ac["time"]].attributes["units"],
                        only_use_cftime_datetimes=False,
                        only_use_python_datetimes=True,
                    )
                    maxDT[v] = num2date(
                        self.ds[ac["time"]].data[-1][0],
                        self.ds[ac["time"]].attributes["units"],
                        only_use_cftime_datetimes=False,
                        only_use_python_datetimes=True,
                    )
                ...
                self.getFeatureType() == TIMESERIES
                or self.getFeatureType() == TIMESERIESPROFILE
            ):  # pragma: no cover
                ...
                if time_units == "true julian day":
                    ...
                    tindx = self.getTimeBegEndIndices(
                        self.ds[list(self.ds[v].keys())[1]]
                    )
                    times = self.ds[list(self.ds[v].maps.keys())[0]].data[
                        tindx[0] : tindx[-1] : self.stride
                    ]
                    ...
                    minDT[v] = num2date(
                        self.ds[v][ac["time"]].data[0][0],
                        self.ds[ac["time"]].attributes["units"],
                        only_use_cftime_datetimes=False,
                        only_use_python_datetimes=True,
                    )
                    maxDT[v] = num2date(
                        self.ds[v][ac["time"]].data[-1][0],
                        self.ds[ac["time"]].attributes["units"],
                        only_use_cftime_datetimes=False,
                        only_use_python_datetimes=True,
                    )
        ...
        self.logger.info(
            "Activity from %s: startDatetime = %s, endDatetime = %s",
            self.url,
            startDatetime,
            endDatetime,
        )
    ...
    def initDB(self):
        ...
        if hasattr(self, "command_line_args"):
            if hasattr(self.command_line_args, "append") and hasattr(
                self.command_line_args, "remove_appended_activities"
            ):
                if (
                    self.command_line_args.append
                    and self.command_line_args.remove_appended_activities
                ):
        ...
        if self.checkForValidData():
            ...
            if hasattr(self, "add_to_activity"):
                ...
                self.logger.info(
                    f"Will add these data to Activity (likely plankton_proxies) {self.add_to_activity}"
                )
            elif hasattr(self, "associatedActivityName"):
                self.logger.info(
                    f"Will associate these data (likely lopc) to Activity {self.associatedActivityName}"
                )
            else:
                ...
                if not self.startDatetime or not self.endDatetime:
                    self.startDatetime, self.endDatetime = (
                        self._getStartAndEndTimeFromDS()
                    )
                self.createActivity()
        else:
    ...
    def getmissing_value(self, var):
        ...
        try:
            ...
            if "nemesis" in self.url and var in ("u", "v"):
                self.logger.debug(
                    "Special fix for nemesis data, return a standard missing_value of -1.e34"
                )
                mv = -1.0e34
            else:
                self.logger.debug(
                    "Cannot get attribute missing_value for variable %s from url %s",
                    var,
                    self.url,
                )
    ...
    def get_FillValue(self, var):
        ...
        try:
            ...
            self.logger.debug(
                "Cannot get attribute _FillValue for variable %s from url %s",
                var,
                self.url,
            )
            try:
                ...
                try:
                    ...
                    self.logger.debug(
                        "Cannot get FillValue for variable %s from url %s: %s",
                        var,
                        self.url,
                        str(e),
                    )
    ...
    def getActivityName(self):
        ...
        if not " starting at " in newName:
            if hasattr(self, "requested_startDatetime") and hasattr(
                self, "requested_endDatetime"
            ):
                if self.requested_startDatetime and self.requested_endDatetime:
                    if "(stride" in self.activityName:
                        first_part = self.activityName[
                            : self.activityName.find("(stride")
                        ]
                        last_part = self.activityName[
                            self.activityName.find("(stride") :
                        ]
                    else:
                        first_part = self.activityName
                        last_part = ""
                    newName = "{} starting at {} {}".format(
                        first_part.strip(), self.requested_startDatetime, last_part
                    )
    ...
    def getFeatureType(self):
        ...
        if hasattr(self, "ds"):
            if hasattr(self.ds, "attributes"):
                try:
                    ...
                    self.logger.warn(
                        'Dataset does not have an NC_GLOBAL attribute! Setting featureType to "trajectory" assuming that this is an old Tethys file'
                    )
                    return TRAJECTORY
            else:
                self.logger.warn(
                    'Loader has no ds attribute. Setting featureType to "trajectory" assuming that this is an ROVCTD Loader.'
                )
                return TRAJECTORY
        else:
            self.logger.warn(
        ...
        if "cf-1.6" in conventions.lower():
            ...
            if "cdm_data_type" in nc_global_keys:
                ...
                "CF:featureType" in nc_global_keys
            ):  # Seen in lrauv/*/realtime/sbdlogs files
        ...
        for k in from_variables:
            try:
                if "standard_name" in self.ds[k].attributes:
                    if self.ds[k].attributes["standard_name"] in (
                        "time",
                        "latitude",
                        "longitude",
                        "depth",
                ...
                self.logger.error(
                    f"Could not find variable {k} in the file. Perhaps there's a problem with the coordinates attribute?"
                )
                raise
    ...
    def getAuxCoordinates(self, variable):
        ...
        if variable not in self.ds:
            raise ParameterNotFound(
                "Variable %s is not in dataset %s" % (variable, self.url)
            )
        ...
        if "coordinates" in self.ds[variable].attributes:
            ...
            try:
                ...
                self.logger.error(
                    f"Could not get coordinates for {variable}. Check its coordinates attribute."
                )
            ...
            for coord in coords:
                ...
                try:
                    ...
                    if coord == "trajectory":
                        self.logger.info(
                            f"Found 'trajectory' in coordinates attribute. Likely a Saildrone or GliderDAC trajectory file."
                        )
                    else:
                        raise AuxCoordMissingStandardName(e)
        else:
            self.logger.debug(
                "Variable %s is missing coordinates attribute, checking if loader has specified it in auxCoords",
                variable,
            )
        ...
        if set(coord_dict.keys()) != reqCoords:
            self.logger.debug(
                "Required coordinate(s) %s missing in NetCDF file.",
                list(reqCoords - set(coord_dict.keys())),
            )
            if not self.auxCoords:
                raise VariableMissingCoordinatesAttribute(
                    "%s: %s missing coordinates attribute"
                    % (
                        self.url,
                        variable,
                    )
        ...
        if not coord_dict or set(coord_dict.keys()) != reqCoords:  # pragma: no cover
            if self.auxCoords:
                if variable in self.auxCoords:
                    ...
                    self.logger.debug(
                        "Returning auxCoords for variable %s that were specified in the constructor: %s",
                        variable,
                        self.auxCoords[variable],
                    )
                    return self.auxCoords[variable]
                else:
                    raise ParameterNotFound(
                        "auxCoords is specified, but variable requested (%s) is not in %s"
                        % (variable, self.auxCoords)
                    )
        else:
            return coord_dict
    ...
    def getNominalLocation(self):
        ...
        for v in self.include_names:
            ...
            if self.getFeatureType() == TIMESERIES:
                self.logger.debug(
                    "Initializing depths list for timeseries, ac = %s", ac
                )
                try:
                    if "depth" in ac:
                    ...
                    self.logger.warn(
                        "No depth coordinate found for %s.  Assuming EPIC scalar and assigning depth from first element",
                        v,
                    )
                    depths[v] = self.ds[ac["depth"]].data[0]
            elif self.getFeatureType() == TIMESERIESPROFILE:
                self.logger.debug(
                    "Initializing depths list for timeseriesprofile, ac = %s", ac
                )
                try:
                    ...
                    if "nominal_sensor_depth" in self.ds.attributes["NC_GLOBAL"]:
                        ...
                        depths[v] = [
                            float(
                                self.ds.attributes["NC_GLOBAL"]["nominal_sensor_depth"]
                            )
                        ]
                    else:
                        self.logger.warning(
                            f"Could not find {ac['depth']} for variable {v} in {self.url}, attempting to hard-code the depth with 'ADCP_DEPTH'"
                        )
                        if v == "SW_FLUX_HR":
                            self.logger.info(
                                f"Attempting to hard-code the depth with the first value from 'HR_DEPTH_0'"
                            )
                            depths[v] = [self.ds["HR_DEPTH_0"].data[:][0]]
                        else:
                            self.logger.info(
                                f"Attempting to hard-code the depth with the first value from 'ADCP_DEPTH'"
                            )
                            depths[v] = [self.ds["ADCP_DEPTH"].data[:][0]]
            elif self.getFeatureType() == TRAJECTORYPROFILE:
                self.logger.debug(
                    "Initializing depths list for trajectoryprofile, ac = %s", ac
                )
                depths[v] = self.ds[v][ac["depth"]].data[:]
            ...
            try:
                ...
                if len(self.ds[ac["longitude"]].data[:]) == 1:
                    ...
                    self.logger.debug(
                        f"Auxillary longitude coordinate {ac['longitude']} is a variable with"
                    ...
                    try:
                        ...
                        self.logger.warn(
                            f"Cannot get nominal longitude coordinate using COARDS rules: self.ds[v].keys() = {self.ds[v].keys()}"
                        )
                else:
                    self.logger.warn(
                        "Variable %s has longitude auxillary coordinate of length %d, expecting it to be 1.",
                        v,
                        len(self.ds[ac["longitude"]].data[:]),
                    )
            ...
            try:
                ...
                if len(self.ds[ac["latitude"]].data[:]) == 1:
                    ...
                    # OASIS ADCP data has GPS_LONGITUDE and GPS_LATITUDE time series in auxillary coordinate
                    self.logger.debug(
                        f"Auxillary latitude coordinate {ac['latitude']} is a variable with"
                    ...
                    try:
                        ...
                        self.logger.warn(
                            f"Cannot get nominal latitude coordinate using COARDS rules: self.ds[v].keys() = {self.ds[v].keys()}"
                        )
                else:
                    self.logger.warn(
                        "Variable %s has latitude auxillary coordidate of length %d, expecting it to be 1.",
                        v,
                        len(self.ds[ac["latitude"]].data[:]),
                    )

        # All variables must have the same nominal location
        if len(set(lats.values())) != 1 or len(set(lons.values())) != 1:
            raise Exception(
                "Invalid file coordinates structure.  All variables must have"
                " identical nominal lat & lon, lats = %s, lons = %s" % lats,
                lons,
            )
    ...
    def getTimeBegEndIndices(self, timeAxis):
        ...
        """
        if not getattr(self, "startDatetime", None) and not getattr(
        ...
        try:
            ...
            isEPIC = (
                "time" in self.ds.keys()
                and "time2" in self.ds.keys()
                and self.ds["time"].attributes["units"] == "True Julian Day"
            )
            if isEPIC:
                self.logger.warn(
                    "%s does not have 'Conventions', yet appears to be EPIC from its time/time2 variables",
                    self.url,
                )
        ...
        if isEPIC:
            ...
            if not t_indx.any():
                raise NoValidData(
                    "No data from %s for time values between %s and %s.  Skipping."
                    % (self.url, self.startDatetime, self.endDatetime)
                )
            ...
            if (
                ...
                or self.startDatetime.minute
                or self.startDatetime.second
            ):
                bms = (
                    self.startDatetime.hour * 3600000
                    + self.startDatetime.minute * 60000
                    + self.startDatetime.second * 1000
                )
            ems = 86400000
            if (
                self.endDatetime.hour
                or self.endDatetime.minute
                or self.endDatetime.second
            ):
                ems = (
                    self.endDatetime.hour * 3600000
                    + self.endDatetime.minute * 60000
                    + self.endDatetime.second * 1000
                )
            ...
            if beg_day_indices.any():
                time2_axis_beg = self.ds["time2"]["time2"][
                    beg_day_indices[0] : beg_day_indices[-1]
                ]
                try:
            ...
            if end_day_indices.any():
                if end_day_indices[0] > 0 and (
                    end_day_indices[0] == end_day_indices[-1]
                ):
                    time2_axis_end = self.ds["time2"]["time2"][
                        int(end_day_indices[0]) - 1 : int(end_day_indices[-1])
                    ]
                else:
                    time2_axis_end = self.ds["time2"]["time2"][
                        int(end_day_indices[0]) : int(end_day_indices[-1])
                    ]
                try:
                    t2_indx_end = (
                        len(time2_axis_end) - np.where(ems >= time2_axis_end)[0][-1]
                    )
                except IndexError:
        ...
        timeAxisUnits = timeAxisUnits.replace(
            "utc", "UTC"
        )  # coards requires UTC to be upper case
        if (
            timeAxis.units == "seconds since 1970-01-01T00:00:00Z"
            or timeAxis.units == "seconds since 1970/01/01 00:00:00Z"
        ):
            timeAxisUnits = (
                "seconds since 1970-01-01 00:00:00"  # coards doesn't like ISO format
            )
        if self.startDatetime:
            self.logger.debug(
                "self.startDatetime, timeAxis.units = %s, %s",
                self.startDatetime,
                timeAxis.units,
            )
            s = date2num(self.startDatetime, timeAxisUnits)
            self.logger.debug(
                "For startDatetime = %s, the udnits value is %f", self.startDatetime, s
            )

        if self.dataStartDatetime:
            # Override s if self.dataStartDatetime is specified
            self.logger.debug(
                "self.dataStartDatetime, timeAxis.units = %s, %s",
                self.dataStartDatetime,
                timeAxis.units,
            )
            s = date2num(self.dataStartDatetime, timeAxisUnits)
            self.logger.debug(
                "For dataStartDatetime = %s, the udnits value is %f",
                self.dataStartDatetime,
                s,
            )
        ...
        if self.requested_endDatetime:
            ...
            self.logger.debug(
                "For endDatetime = %s, the udnits value is %f", self.endDatetime, e
            )
        else:
            e = timeAxis[-1]
            self.logger.debug(
                "requested_endDatetime not given, using the last value of timeAxis = %f",
                e.data[0],
            )
        ...
        if getattr(self, "command_line_args", False):
            if self.command_line_args.append:
                ...
                self.logger.info(
                    f"--append specified. Finding start index where time > {s}"
                )
        ...
        if tIndx.size == 0:
            raise NoValidData(
                f"No time values from {self.url} between time values {s} and {e}"
            )
        elif tIndx.size == 1:
        ...
        try:
            ...
            raise NoValidData(
                "Could not get first and last indexes from tIndex = %s. Skipping."
                % (tIndx)
            )
        ...
        if indices[1] <= indices[0]:
            raise InvalidSliceRequest(
                "Cannot issue DAP temporal constraint expression of non-positive slice: indices = {indices}"
            )
    ...
    def getTotalRecords(self):
        ...
        for name in self.include_names:
            try:
                tIndx = self.getTimeBegEndIndices(
                    self.ds[self.coord_dicts[name]["time"]]
                )
            except KeyError:
            ...
            try:
                if self.getFeatureType() == TRAJECTORY:
                    try:
                        trajSingleParameterCount = np.prod(
                            self.ds[name].shape[1:] + (np.diff(tIndx)[0],)
                        )
                    except AttributeError:
                        # Likely using pydap 3.2+
                        trajSingleParameterCount = np.prod(
                            self.ds[name].array.shape[1:] + (np.diff(tIndx)[0],)
                        )
                try:
                    pcount = (
                        np.prod(self.ds[name].shape[1:] + (np.diff(tIndx)[0],))
                        / self.stride
                    )
                    count += pcount
                except AttributeError:
                    # Likely using pydap 3.2+
                    pcount = (
                        np.prod(self.ds[name].array.shape[1:] + (np.diff(tIndx)[0],))
                        / self.stride
                    )
                    count += pcount
            except KeyError as e:
        ...
        self.logger.debug(
            "Adding %d derived parameters of length %d to the count",
            numDerived,
            trajSingleParameterCount / self.stride,
        )
    ...
    def _equal_coords(self, load_groups, coor_groups):
        ...
        if len(coor_groups) == 1:
            self.logger.info(
                f"Single set of coordinates as would be found in a modern shore_i.nc file"
            )
            return coord_equals
        for count, (axes, ac) in enumerate(coor_groups.items()):
            ...
            if count > 0:
                if len(last_times) < len(self.ds[ac[TIME]]):
                    self.logger.info(
                        f"len(last_times) ({len(last_times)}) < len(self.ds[ac[TIME]]) ({len(self.ds[ac[TIME]])})"
                    )
                    num_pad = len(self.ds[ac[TIME]]) - len(last_times)
                    self.logger.info(
                        f"Padding last_ coordinate arrays with {num_pad} zero(s) to match (taking a chance) the self.ds coordinate arrays"
                    )
                    last_times = np.pad(
                        last_times, [(0, num_pad)], mode="constant", constant_values=0
                    )
                    last_depths = np.pad(
                        last_depths, [(0, num_pad)], mode="constant", constant_values=0
                    )
                    last_latitudes = np.pad(
                        last_latitudes,
                        [(0, num_pad)],
                        mode="constant",
                        constant_values=0,
                    )
                    last_longitudes = np.pad(
                        last_longitudes,
                        [(0, num_pad)],
                        mode="constant",
                        constant_values=0,
                ...
                if len(last_times) > len(self.ds[ac[TIME]]):
                    ...
                    self.logger.warn(
                        f"Not Padding self.ds arrays - not able to attempt a fix"
                    )
                    continue
                ...
                coord_equals[axes] = np.logical_and(
                    np.logical_and(times_equal, depths_equal),
    ...
    def get_load_structure(self):
        ...
        for pname in self.include_names:
            ...
            try:
                ...
                self.logger.debug(
                    f"Number likely in auxCoords rather than a coordinate name, convert to string for group_name"
                )
                group_name = ""
    ...
    def _find_dup_coords(self, ips, meass, coords_equal):
        for index, (ip, meas) in enumerate(zip(ips, meass)):
            if meas:
                try:
                    measurement = Measurement.objects.using(self.dbAlias).filter(
                        depth=meas.depth, geom=meas.geom, instantpoint=ip
                    )
                    self.logger.info(
                        f"Adding index {index} to coords_equal for meas = {meas}"
                    )
    ...
    def _read_coords_from_ds(self, tindx, ac, multidim_trajectory=False):
        ...
        try:
            if isinstance(self.ds[ac[DEPTH]], pydap.model.GridType):
                depths = self.ds[ac[DEPTH]][ac[DEPTH]][
                    tindx[0] : tindx[-1] : self.stride
                ]
            else:
            ...
            if ac[DEPTH] in self.ds:
                ...
                if isinstance(ac[DEPTH], (int, float)):
                    if multidim_trajectory:
                        self.logger.info(
                            "Overridden in auxCoords: ac[DEPTH] = {ac[DEPTH]}, setting depths to [{ac[DEPTH]}] * len(times)"
                        )
                        depths = [ac[DEPTH]] * len(times)
                    else:
                        self.logger.info(
                            "Overridden in auxCoords: ac[DEPTH] = {ac[DEPTH]}, setting depths to [{ac[DEPTH]}]"
                        )
                        depths = [ac[DEPTH]]

        if isinstance(self.ds[ac[LATITUDE]], pydap.model.GridType):
            latitudes = self.ds[ac[LATITUDE]][ac[LATITUDE]][
                tindx[0] : tindx[-1] : self.stride
            ]
        ...
        if isinstance(self.ds[ac[LONGITUDE]], pydap.model.GridType):
            longitudes = self.ds[ac[LONGITUDE]][ac[LONGITUDE]][
                tindx[0] : tindx[-1] : self.stride
            ]
        elif multidim_trajectory:
            # TODO: Deal with (as yet unseen) case where multiple trajectories exist in a netCDF file
            longitudes = self.ds[ac[LONGITUDE]][0][0][
                tindx[0] : tindx[-1] : self.stride
            ]
        else:
    ...
    def _load_coords_from_dsg_ds(
        self,
        tindx,
        ac,
        pnames,
        axes,
        coords_equal=np.array([]),
        ...
        """
        mtimes, depths, latitudes, longitudes = self._read_coords_from_ds(
            tindx, ac, multidim_trajectory=multidim_trajectory
        )
        self.logger.debug(f"Getting good_coords for {pnames}...")
        mtimes, depths, latitudes, longitudes, dup_times = zip(
            *self.good_coords(
        ...
        try:
            meass, mask = self._bulk_load_coordinates(
                self._ips(mtimes),
                self._meass(depths, longitudes, latitudes),
                dup_times,
                ac,
                axes,
            )
            ...
            self.logger.info(
                f"{e}: Trying _bulk_load_coordinates() again after _find_dup_coords()"
            )
            coords_equal = self._find_dup_coords(
                self._ips(mtimes),
                self._meass(depths, longitudes, latitudes),
                coords_equal,
            )
    ...
    def _load_coords_from_instr_ds(self, tindx, ac):
        ...
        try:
            ...
            self.logger.warn(
                f"Stride of {self.stride} likely greater than range of data: {tindx[0]}:{tindx[-1]}"
            )
        ...
        for mt in mtimes:
            try:
                ip, secs_diff = get_closest_instantpoint(
                    self.associatedActivityName, mt, self.dbAlias
                )
            except ClosestTimeNotFoundException as e:
                self.logger.error(
                    "Could not find corresponding measurment for LOPC data measured at %s",
                    mt,
                )
            else:
                if secs_diff > noload_secs_diff:
                    noload_count += 1
                    self.logger.debug(
                        f"{noload_count:3d}. LOPC data at {mt.strftime('%Y-%m-%d %H:%M:%S')} not loaded - more than "
                ...
                if secs_diff > warn_secs_diff:
                    warn_count += 1
                    self.logger.debug(
                        f"{warn_count:3d}. LOPC data at {mt.strftime('%Y-%m-%d %H:%M:%S')} more than "
                        f"{warn_secs_diff} secs away from existing measurement: {secs_diff}"
                    )

                meass.append(
                    Measurement.objects.using(self.dbAlias).get(instantpoint=ip)
                )

        self.logger.warn(
            f"{noload_count} of {len(times)} original LOPC measurements not loaded because they "
            f"were more than {noload_secs_diff} seconds away from an existing measurement"
        )
        self.logger.warn(
            f"{warn_count} of {len(meass)} collected LOPC measurements were more than "
    ...
    def _mask_data(self, vd, vm):
        ...
        if good_count == 0:
            self.logger.warning(
                f"No good data yielded. Coordinate values in {self.url} are likely bad."
            )
    ...
    def _meass_from_activity(self, add_to_activity, tindx, ac):
        ...
        """
        meass = (
            Measurement.objects.using(self.dbAlias)
            .filter(instantpoint__activity=add_to_activity)
        ...
        for count, (meas, mt, de, la, lo) in enumerate(
            zip(meass, *self._read_coords_from_ds(tindx, ac))
        ):
            ...
            if not np.isclose(meas.geom.y, la):
                la_msg = (
                    f"Existing latitude ({meas.geom.y}) != la ({la}) at index {count}"
                )
                self.logger.debug(la_msg)
                unequal_la += 1
            if not np.isclose(meas.geom.x, lo):
                lo_msg = (
                    f"Existing longitude ({meas.geom.x}) != lo ({lo}) at index {count}"
                )
        ...
        if unequal_ti:
            self.logger.error(
                f"Encountered {unequal_ti} unequal_ti when adding data from {self.url} to Activity {add_to_activity}"
            )
    ...
    def load_trajectory(self, add_to_activity=None):
        ...
        for axis_count, (k, pnames) in enumerate(load_groups.items()):
            ...
            try:
                ...
                self.logger.warn(
                    f"Failed to getTimeBegEndIndices() for axes {k} from {self.url}"
                )
                continue
            ...
            for i, pname in enumerate(pnames):
                ...
                if i == 0:
                    ...
                    if DEPTH not in ac:
                        self.logger.warn(
                            f"{self.param_by_key[pname]} does not have {DEPTH} in {ac}. Skipping."
                        )
                        continue
                    if ac[DEPTH] not in self.ds and isinstance(ac[DEPTH], (int, float)):
                        ...
                        self.logger.info(
                            f"{self.param_by_key[pname]} does not have {DEPTH} in {self.url}."
                        )
                        self.logger.info(
                            f"ac[DEPTH] = {ac[DEPTH]}. Assume that this depth coordinate was provided in auxCoords"
                        )
                        self.logger.info(f"Loading coordinates for axes {k}")
                        meass, dup_times, mask = self._load_coords_from_dsg_ds(
                            tindx,
                            ac,
                            pnames,
                            k,
                            multidim_trajectory=multidim_trajectory,
                        )
                    elif (
                        ac[DEPTH] in self.ds
                        and ac[LATITUDE] in self.ds
                        and ac[LONGITUDE] in self.ds
                    ):
                        try:
                            ...
                            if coords_equal_hash == {}:
                                if add_to_activity:
                                    meass, dup_times, mask = self._meass_from_activity(
                                        add_to_activity, tindx, ac
                                    )
                                else:
                                    meass, dup_times, mask = (
                                        self._load_coords_from_dsg_ds(
                                            tindx, ac, pnames, k
                                        )
                                    )
                            else:
                                meass, dup_times, mask = self._load_coords_from_dsg_ds(
                                    tindx, ac, pnames, k, coords_equal_hash[k]
                                )
                            ...
                            self.logger.debug(str(e))
                            self.logger.warn(
                                f"No good coordinates for {pname} - skipping it"
                            )
                            continue
                            ...
                            self.logger.debug(str(e))
                            self.logger.warn(
                                f"OverflowError when converting coordinates for {pname} - skipping it"
                            )
                            return total_loaded
                    else:
                        # Expect instrument (time-coordinate-only) dataset
                        self.logger.warn(
                            f"{pname} has no {ac[DEPTH]} coordinate - processing as time-coordinate-only, e.g. LOPC"
                        )
                    ...
                    if k in coords_equal_hash:
                        if coords_equal_hash[k].all():
                            ...
                            self.logger.warning(
                                f"Parameter {pname} does not share the same coordinates of previously loaded Parameters, skipping for now."
                            )
                            self.logger.debug(
                                f"coords_equal_hash[{k}] = {coords_equal_hash[k]}"
                            )
                            continue
                ...
                try:
                    if isinstance(self.ds[pname], pydap.model.GridType):
                        ...
                        values = self.ds[pname][pname].data[
                            tindx[0] : tindx[-1] : self.stride
                        ]
                    elif multidim_trajectory:
                        self.logger.info(
                            f"(multidim) loading {pname} from multidimensional trajectory file"
                        )
                        constraint_string = f"using python slice: ds['{pname}'][0][0][{tindx[0]}:{tindx[-1]}:{self.stride}]"
                        # TODO: Deal with (as yet unseen) case where multiple trajectories exist in a netCDF file
                        values = self.ds[pname].data[0][0][
                            tindx[0] : tindx[-1] : self.stride
                        ]
                    else:
                ...
                if mask:
                    ...
                    if not values:
                        self.logger.warning(
                            f"Coordinates likely bad - check them here:"
                        )
                        self.logger.warning(
                            f"Depth data: {self.url}.ascii?{ac[DEPTH]}[{tindx[0]}:{self.stride}:{tindx[-1] - 1}]"
                        )
                        self.logger.warning(
                            f"Latitude data: {self.url}.ascii?{ac[LATITUDE]}[{tindx[0]}:{self.stride}:{tindx[-1] - 1}]"
                        )
                        self.logger.warning(
                            f"Longitude data: {self.url}.ascii?{ac[LONGITUDE]}[{tindx[0]}:{self.stride}:{tindx[-1] - 1}]"
                        )
                        return total_loaded

                self.logger.info(
                    f"Time data: {self.url}.ascii?{ac[TIME]}[{tindx[0]}:{self.stride}:{tindx[-1] - 1}]"
                )
                if hasattr(values[0], "__iter__"):
                    # For data like LOPC data - expect all values to be non-nan, load array and the sum of it
                    self.param_by_key[
                        pname
                    ].description = "Sum of counts saved in datavalue, spectrum of counts saved in dataarray"
                    self.param_by_key[pname].save(using=self.dbAlias)
                    mps = (
                        MeasuredParameter(
                            measurement=me,
                            parameter=self.param_by_key[pname],
                            dataarray=list(va),
                            datavalue=sum(va),
                        )
                    ...
                    mps = (
                        ...
                        for me, va, dt in zip(meass, values, dup_times)
                        if not dt
                    )

                # All items but meass are generators, so we can call len() on it
                self.logger.info(
                    f"Bulk loading {len(meass)} {self.param_by_key[pname]} datavalues into MeasuredParameter {constraint_string} with batch_size = {BATCH_SIZE}"
                )
                mps = self._measuredparameter_with_measurement(meass, mps)
                mps = MeasuredParameter.objects.using(self.dbAlias).bulk_create(
                    mps, batch_size=BATCH_SIZE
                )
            ...
            for i, pname in enumerate(pnames):
                if i == 0:
                    ...
                    if ac[TIME] != list(self.ds[firstp].keys())[1]:
                        # Gratuitous check
                        self.logger.warn(
                            "Auxillary time coordinate '{ac[TIME]}' != first COARDS"
                    ...
                    tindx = self.getTimeBegEndIndices(
                        self.ds[list(self.ds[firstp].keys())[1]]
                    )
                    try:
                        times = self.ds[list(self.ds[firstp].maps.keys())[0]].data[
                            tindx[0] : tindx[-1] : self.stride
                        ]
                    except ValueError as e:
                        # Likely 'not enough values to unpack' because of self.stride exceeding range
                        self.logger.warn(
                            f"{e}. Stride value of {self.stride} is likely too high."
                        )
                        self.logger.warn(f"Skipping all parameters in coor_group {ac}")
                        continue
                    time_units = self.ds[
                        list(self.ds[firstp].maps.keys())[0]
                    ].units.lower()

                    if time_units == "true julian day":  # pragma: no cover
                        times, time_units = self._convert_EPIC_times(times, tindx)

                    time_units = time_units.replace(
                        "utc", "UTC"
                    )  # coards requires UTC in uppercase
                    if (
                        self.ds[list(self.ds[firstp].maps.keys())[0]].units
                    ...
                    try:
                        depths = self.ds[
                            list(self.ds[firstp].maps.keys())[1]
                        ].data[
                            :
                        ]  # TODO lookup more precise depth from conversion from pressure
                    except IndexError:
                        self.logger.warn(
                            f"Variable {firstp} has less than 2 coordinates: {self.ds[pname].keys()}"
                        )
                        depths = np.array([])
                    ...
                    if len(depths) == 1 and "depth" not in ac:
                        try:
                            self.logger.info(
                                "Attempting to set nominal depth from EPIC Convention sensor_depth variable attribute"
                            )
                            depths = np.array(
                                [self.ds[firstp].attributes["sensor_depth"]]
                            )
                        except KeyError:
                            self.logger.info(
                                "Variable %s does not have a sensor_depth attribute",
                                firstp,
                            )
                    elif not depths.any():
                        self.logger.warn(
                            "Depth coordinate not found at index [2]. Looking for nominal position from EPIC Convention global attributes."
                        )
                        try:
                            depths = np.array(
                                [
                                    float(
                                        self.ds.attributes["NC_GLOBAL"][
                                            "nominal_instrument_depth"
                                        ]
                                    )
                                ]
                            )
                            ...
                            self.logger.warn(
                                "EPIC nominal position not found in global attributes. Assigning from variables (and maybe variable attribute)."
                            )
                            if "depth" in self.ds:
                            ...
                            if "nominal_instrument_depth" in self.ds[firstp].attributes:
                                nomDepths = self.ds[firstp].attributes[
                                    "nominal_instrument_depth"
                                ]
                            if "lat" in self.ds:
                    ...
                    if nomDepths and nomLat and nomLon:
                        ...
                        self.logger.info(
                            "Nominal position assigned from EPIC Convention global attributes"
                        )
                        nomDepths = depths
                    elif depths.any():
                        self.logger.info(
                            "Nominal depth assigned from EPIC Convention variable attributes"
                        )
                        nomDepths = depths
                    ...
                    if shape_length == 4:
                        self.logger.info(
                            "%s has shape of 4, assume that singleton dimensions are used for nominal latitude and longitude",
                            firstp,
                        )
                        ...
                        if ac["latitude"] in self.ds[ac["latitude"]]:
                            ...
                            latitudes = float(
                                self.ds[list(self.ds[firstp].maps.keys())[2]].data[0]
                            )
                        ...
                        if ac["longitude"] in self.ds[ac["longitude"]]:
                            ...
                            longitudes = float(
                                self.ds[list(self.ds[firstp].maps.keys())[3]].data[0]
                            )

                    elif (
                        shape_length == 3
                        and "EPIC"
                        in self.ds.attributes["NC_GLOBAL"]["Conventions"].upper()
                    ):  # pragma: no cover
                        ...
                        latitudes = float(
                            self.ds[list(self.ds[firstp].maps.keys())[1]].data[0]
                        )  # TODO lookup more precise gps lat via coordinates pointing to a vector
                        longitudes = float(
                            self.ds[list(self.ds[firstp].maps.keys())[2]].data[0]
                        )  # TODO lookup more precise gps lon via coordinates pointing to a vector
                        depths = nomDepths
                    elif shape_length == 2:
                        self.logger.info(
                            "%s has shape of 2, assuming no latitude and longitude singletime"
                            " dimensions. Using nominal location read from auxillary coordinates",
                            firstp,
                        )
                        longitudes = nomLon
                        latitudes = nomLat
                        ...
                        self.logger.info(
                            "%s has shape of 1, assuming no latitude, longitude, and"
                            " depth singletime dimensions. Using nominal location read"
                            " from auxially coordinates",
                            firstp,
                        )
                        longitudes = nomLon
                        latitudes = nomLat
                        ...
                        raise Exception(
                            "{} has shape of {}. Can handle only shapes of 2, and 4".format(
                                firstp, shape_length
                            )
                        )

                    if hasattr(latitudes, "__iter__") and hasattr(
                        longitudes, "__iter__"
                    ):
                        ...
                        for i, (lo, la) in enumerate(zip(longitudes, latitudes)):
                            if (
                                lo == self.ds[ac["longitude"]].attributes["_FillValue"]
                                or lo
                                == self.ds[ac["longitude"]].attributes["missing_value"]
                                or la
                                == self.ds[ac["latitude"]].attributes["_FillValue"]
                                or la
                                == self.ds[ac["latitude"]].attributes["missing_value"]
                            ):
                                self.logger.debug(
                                    f"Not using missing or fill value at index {i}: lo, la = {lo}, {la}"
                                )
                            else:
                        ...
                        if abs(latitudes) > 90:
                            ...
                            self.logger.info(
                                "%s appears to not have COARDS ordering of coordinate dimensions, swapping them",
                                firstp,
                            )
                            tmp_var = latitudes
                        ...
                        self.logger.debug(
                            f"Making points list from {(longitudes, latitudes)} for each {len(list(mtimes))} mtimes"
                        )
                        points = [
                            Point(float(longitudes), float(latitudes))
                            for i in range(len(list(mtimes)))
                        ]
                    ...
                    ips = (
                        InstantPoint(activity=self.activity, timevalue=mt)
                        for mt in mtimes
                    )
                    try:
                        self.logger.info(
                            f"Calling bulk_create() for InstantPoints in ips generator for firstp = {firstp} with batch_size = {BATCH_SIZE}"
                        )
                        ips = InstantPoint.objects.using(self.dbAlias).bulk_create(
                            ips, batch_size=BATCH_SIZE
                        )
                    except (IntegrityError, psycopg2.IntegrityError) as e:
                        self.logger.info(
                            f"Time axis '{ac[TIME]}' likely has timevalues already loaded from an axis in {time_axes_loaded}"
                        )
                        self.logger.info(
                            f"Getting matching InstantPoints from the database, creating new ones not yet there."
                        )
                        ips_new = []
                        num_created = 0
                        for ip in (
                            ...
                            ip_db, created = InstantPoint.objects.using(
                                self.dbAlias
                            ).get_or_create(
                        ...
                        ips = ips_new
                        self.logger.info(
                            f"Got {len(ips) - num_created} InstantPoints from the database, created {num_created} new ones"
                        )

                        if not ips:
                            self.logger.error(
                                f"Unable to load load InstantPoints for axis {ac[TIME]}. Exiting."
                            )
                            self.logger.exception(
                                f"Maybe you should delete Activity '{self.activity.name}' first?"
                            )
                            sys.exit(-1)
                    ...
                    if nomDepths.any() and nom_point:
                        nls = []
                        for nd in nomDepths:
                            nl, _ = NominalLocation.objects.using(
                                self.dbAlias
                            ).get_or_create(
                    ...
                    try:
                        self.logger.info(
                            f"Calling bulk_create() for {len(meass)} Measurements with batch_size = {BATCH_SIZE}"
                        )
                        meass = Measurement.objects.using(self.dbAlias).bulk_create(
                            meass, batch_size=BATCH_SIZE
                        )
                    except (IntegrityError, psycopg2.IntegrityError) as e:
                        self.logger.info(
                            f"Depth axis '{ac[DEPTH]}' likely has depths already loaded from an axis in {depth_axes_loaded}"
                        )
                        self.logger.info(
                            f"Getting matching Measurements from the database, creating new ones not yet there."
                        )
                        meass_new = []
                        num_created = 0
                        for meas in meass:
                            meas_db, created = Measurement.objects.using(
                                self.dbAlias
                            ).get_or_create(
                                instantpoint=meas.instantpoint,
                                depth=meas.depth,
                                geom=meas.geom,
                                nominallocation=meas.nominallocation,
                            )
                            if created:
                                num_created += 1
                        ...
                        meass = meass_new
                        self.logger.info(
                            f"Got {len(meass) - num_created} Measurements from the database, created {num_created} new ones"
                        )

                        if not meass:
                            self.logger.error(
                                f"Unable to load load Measurements for axis {ac[DEPTH]}. Exiting."
                            )
                            self.logger.exception(
                                f"Maybe you should delete Activity '{self.activity.name}' first?"
                            )
                ...
                if len(values.shape) == 1:
                    self.logger.info(
                        "len(values.shape) = 1; likely EPIC timeseries data - reshaping to add a 'depth' dimension"
                    )
                ...
                mps = (
                    MeasuredParameter(
                        measurement=me, parameter=self.param_by_key[pname], datavalue=va
                    )
                    for me, va in zip(meass, values)
                )
    ...
    def _bulk_load_coordinates(self, ips, meass, dup_times, ac, axes):
        ...
        try:
            self.ips = InstantPoint.objects.using(self.dbAlias).bulk_create(
                ips_to_load, batch_size=BATCH_SIZE
            )
            ...
            if hasattr(self, "ips"):
                self.logger.info(
                    f"Duplicate time values for axes {axes}. Reusing previously loaded time values for {ac['time']}"
                )
            else:
                self.logger.error(f"{e}")
                self.logger.error(
                    f"It's likely that the {ac['time']} variable in {self.url} has a duplicate value"
                )
        ...
        self.logger.info(
            f"Calling bulk_create() for Measurements in meass generator with batch_size = {BATCH_SIZE}"
        )
        meass = Measurement.objects.using(self.dbAlias).bulk_create(
            meass, batch_size=BATCH_SIZE
        )
    ...
    def _delete_bad_datavalues(self, pname):
        num, _ = (
            MeasuredParameter.objects.using(self.dbAlias)
            .filter(parameter__name=pname, datavalue=np.nan)
            .delete()
        )
        if num:
        ...
        num, _ = (
            MeasuredParameter.objects.using(self.dbAlias)
            .filter(parameter__name=pname, datavalue=np.inf)
            .delete()
        )
        if num:
    ...
    def _post_process_updates(self, mps_loaded, featureType="", add_to_activity=None):
        ...
        if not hasattr(self, "activity") and hasattr(self, "associatedActivityName"):
            self.activity = Activity.objects.using(self.dbAlias).get(
                name=self.associatedActivityName
            )
        ...
        try:
            path = LineString([p[0] for p in linestringPoints]).simplify(
                tolerance=0.001
            )
            ...
            if len(path) == 2:
                ...
                if path[0][0] == path[1][0] and path[0][1] == path[1][1]:
                    self.logger.info(
                        "And the 2 points are identical. Saving the first point of this"
                        " path as a point as the featureType is also %s.",
                        featureType,
                    )
                ...
                lon = set(
                    [
                        p.x
                        for p in NominalLocation.objects.using(self.dbAlias)
                ...
                if lon and lat:
                    if len(lon) != 1 or len(lat) != 1:
                        self.logger.error(
                            f"For activity={self.activity} length of nominal latitudes and longitudes != 1"
                        )
                    else:
        ...
        if hasattr(self, "add_to_activity"):
            act_to_update = Activity.objects.using(self.dbAlias).get(
                id=self.add_to_activity.id
            )
            load_comment = (
                f"{act_to_update.comment} - Loaded variables {varList} from {self.url}"
            )
        ...
        if hasattr(self, "add_to_activity") or hasattr(self, "associatedActivityName"):
            num_updated = (
                Activity.objects.using(self.dbAlias)
                .filter(id=act_to_update.id)
                .update(
                    comment=load_comment,
                    num_measuredparameters=mps_loaded
                    + act_to_update.num_measuredparameters,
                )
            )
        else:
            num_updated = (
        ...
        if featureType == TRAJECTORY:
            if hasattr(self, "critSimpleDepthTime"):
                ...
                self.insertSimpleDepthTimeSeries(
                    critSimpleDepthTime=self.critSimpleDepthTime
                )
            else:
                self.insertSimpleDepthTimeSeries()
            ...
            self.insertSimpleDepthTimeSeriesByNominalDepth(
                trajectoryProfileDepths=self.timeDepthProfiles
            )
    ...
    def process_trajectory_values_from_generator(self, data_generator):
        ...
        for row in data_generator():
            ...
            mp = MeasuredParameter(
                measurement=meas, parameter=self.param_by_key[key], datavalue=value
            )
    ...
    def process_data(self, featureType="", add_to_activity=None):
        ...
        if getattr(self, "command_line_args", False):
            if self.command_line_args.append:
                self.dataStartDatetime = (
                    InstantPoint.objects.using(self.dbAlias)
        ...
        try:
            if featureType == TRAJECTORY:
                ...
                self.logger.warn(
                    f"Loader for featureType {featureType} has not yet been implemented"
                )
            else:
                raise Exception(
                    f"Global attribute 'featureType' is not one of '{TRAJECTORY}',"
            ...
            self.logger.error(
                f"If you need to load data that has been appended to the URL then delete its Activity before loading."
            )
        ...
        if mps_loaded:
            ...
            MeasuredParameter.objects.using(self.dbAlias).filter(
                datavalue=None, dataarray=None
            ).delete()
            ...
            for parameter in self.parameter_counts.copy().keys():
                ...
                if mp_count == 0:
                    self.logger.info(
                        f"Deleting Parameter because it has no valid data: {parameter}"
                    )
                    try:
            ...
            path = self._post_process_updates(
                mps_loaded, featureType, add_to_activity=add_to_activity
            )
...
class Dorado_Loader(Trajectory_Loader):
    ...
    def addResources(self):
        ...
        survey = (
            self.url.split("/")[-1].split(".nc")[0].split("_decim")[0]
        )  # Works for both .nc and _decim.nc files
        ...
        resourceType, _ = ResourceType.objects.db_manager(self.dbAlias).get_or_create(
            name="quick_look",
            description="Quick Look plot of data from this AUV survey",
        )
        for ql in [
            "2column",
            "biolume",
            ...
            self.logger.debug(
                "Getting or Creating Resource with name = %s, url = %s", ql, url
            )
        ...
        # kml, odv, mat
        kmlResourceType, _ = ResourceType.objects.db_manager(
            self.dbAlias
        ).get_or_create(
            name="kml",
            description="Keyhole Markup Language file of data from this AUV survey",
        )
        odvResourceType, _ = ResourceType.objects.db_manager(
            self.dbAlias
        ).get_or_create(name="odv", description="Ocean Data View spreadsheet text file")
        matResourceType, _ = ResourceType.objects.db_manager(
            self.dbAlias
        ).get_or_create(name="mat", description="Matlab data file")
        for res in ["kml", "odv", "odvGulper", "mat", "mat_gridded"]:
            ...
            self.logger.debug(
                "Getting or Creating Resource with name = %s, url = %s", res, url
            )
            resource, _ = Resource.objects.db_manager(self.dbAlias).get_or_create(
...
class Lrauv_Loader(Trajectory_Loader):
    ...
    def addResources(self):
        ...
        if self.contourUrl and self.timezone:  # pragma: no cover
            ...
            # Contour plots
            self.logger.debug("Getting or Creating ResourceType quick_look...")
            resourceType, _ = ResourceType.objects.db_manager(
                self.dbAlias
            ).get_or_create(
                name="quick_look",
                description="Quick Look plot of data from this AUV survey",
            ...
            self.logger.debug(
                "Getting or Creating Resource with name = log, url = %s", outurl
            )
            resource, _ = Resource.objects.db_manager(self.dbAlias).get_or_create(
            ...
            startDateTimeLocal = startDateTimeLocal.replace(
            ...
            endDateTimeLocal = endDateTimeLocal.replace(
            ...
            outurl = (
                ...
                + self.platformName
                + "_log_"
                + startDateTimeUTC.strftime("%Y%m%dT%H%M%S")
                + "_"
                + endDateTimeUTC.strftime("%Y%m%dT%H%M%S")
                + ".png"
            )
            self.logger.debug(
                "Getting or Creating Resource with name = 24hr, url = %s", outurl
            )
            resource, _ = Resource.objects.db_manager(self.dbAlias).get_or_create(
...
class BED_Trajectory_Loader(Trajectory_Loader):
    ...
    def addResources(self):  # pragma: no cover
        ...
        self.logger.debug(
            "Getting or Creating Resource with framegrab = self.framegrab"
        )
        link_text = "framegrab"
...
def runTrajectoryLoader(
    ...
    if plotTimeSeriesDepth is not None:
        ...
        loader.plotTimeSeriesDepth = dict.fromkeys(
            parmList + [ALTITUDE, SIGMAT, SPICE], plotTimeSeriesDepth
        )
...
def runBEDTrajectoryLoader(
    ...
    if plotTimeSeriesDepth:
        ...
        loader.plotTimeSeriesDepth = dict.fromkeys(
            parmList + ["altitude"], plotTimeSeriesDepth
        )
...
def _loadLOPC(
    ...
    if "Dorado389" in url:
        ...
        return
    lopc_url = (
        "http://dods.mbari.org/opendap/data/ssdsdata/ssds/generated/netcdf/"
        "files/ssds.shore.mbari.org/auvctd/missionlogs/{}/{}/{}.{}.{}/"
        "lopc.nc"
    ).format(yr, yr + yd, yr, yd, mn)
    ...
    if plotTimeSeriesDepth is not None:
        lopc_loader.plotTimeSeriesDepth = dict.fromkeys(
            lopc_loader.include_names, plotTimeSeriesDepth
        )
...
def _load_plankton_proxies(
    ...
    pp_url = (
        "http://odss.mbari.org/thredds/dodsC/Other/routine/Products/Dorado/"
        "netcdf_proxies/{}/Dorado_{}_{}_{}_proxies.nc"
    ).format(yr, yr, yd, mn)
    ...
    try:
        pp_loader = Trajectory_Loader(
            url=pp_url,
            campaignName=cName,
            campaignDescription=cDesc,
            dbAlias=dbAlias,
            activityName=pp_aName,
            activitytypeName=aTypeName,
            platformName=pName,
            platformColor=pColor,
            platformTypeName=pTypeName,
    ...
    if plotTimeSeriesDepth is not None:
        pp_loader.plotTimeSeriesDepth = dict.fromkeys(
            pp_loader.include_names, plotTimeSeriesDepth
        )

    # Auxillary coordinates are the same for all include_names
    pp_loader.auxCoords = {}
    for v in pp_loader.include_names:
        pp_loader.auxCoords[v] = {
            "time": "time",
            "latitude": "latitude",
            "longitude": "longitude",
            "depth": "depth",
...
def runDoradoLoader(
    ...
    if mps_loaded:
        if "Dorado389" in url and (
            "sepCountList" in loader.include_names
            or "mepCountList" in loader.include_names
        ):
    ...
    if parmList:
        ...
        for p in parmList:
            if p.find(".") == -1:
                ...
                loader.logger.warn(
                    "Parameter %s not included. CANNOT HAVE PARAMETER NAMES WITH PERIODS. Period.",
                    p,
                )
...
def runGliderLoader(
    ...
    if pTypeName == "waveglider":
        ...
        loader.auxCoords["temperature"] = {
            "time": "time",
            "latitude": "lat",
            "longitude": "lon",
            "depth": "depth",
        }
        loader.auxCoords["salinity"] = {
            "time": "time",
            "latitude": "lat",
            "longitude": "lon",
            "depth": "depth",
        }
        loader.auxCoords["density"] = {
            "time": "time",
            "latitude": "lat",
            "longitude": "lon",
            "depth": "depth",
        }
        loader.auxCoords["fluorescence"] = {
            "time": "time",
            "latitude": "lat",
            "longitude": "lon",
            "depth": "depth",
        }
        loader.auxCoords["phycoerythrin"] = {
            "time": "time",
            "latitude": "lat",
            "longitude": "lon",
            "depth": "depth",
        }
        loader.auxCoords["cdom"] = {
            "time": "time",
            "latitude": "lat",
            "longitude": "lon",
            "depth": "depth",
        }
        loader.auxCoords["oxygen"] = {
            "time": "time",
            "latitude": "lat",
            "longitude": "lon",
            "depth": "depth",
        }
        loader.auxCoords["optical_backscatter470nm"] = {
            "time": "time",
            "latitude": "lat",
            "longitude": "lon",
            "depth": "depth",
        }
        loader.auxCoords["optical_backscatter532nm"] = {
            "time": "time",
            "latitude": "lat",
            "longitude": "lon",
            "depth": "depth",
        }
        loader.auxCoords["optical_backscatter660nm"] = {
            "time": "time",
            "latitude": "lat",
            "longitude": "lon",
            "depth": "depth",
        }
        loader.auxCoords["optical_backscatter700nm"] = {
            "time": "time",
            "latitude": "lat",
            "longitude": "lon",
            "depth": "depth",
    ...
    if url.endswith("_CMSTV.nc"):
        ...
        for v in loader.include_names:
            if v in [
                ...
                loader.logger.warn(
                    "Do not have an auxCoords assignment for variable %s in url %s",
                    v,
                    url,
                )
