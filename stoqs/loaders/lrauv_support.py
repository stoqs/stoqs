...
from utils.STOQSQManager import DESCRIPTION
from utils.STOQSQManager import LABEL
from utils.STOQSQManager import LRAUV_MISSION
...
from stoqs.models import ResourceResource
from stoqs.models import ResourceType
from stoqs.models import SimpleDepthTime
...
class MissionLoader(STOQS_Loader):
    ...
    def _missions_from_json(self, platform_name, url):
        ...
        try:
            mission_starts = [
                Mission(d["unixTime"] / 1000.0, d["text"])
                for d in td_log_important
                if STARTED_MISSION in d["text"]
            ]
        except KeyError:
    ...
    def _make_starts_ends_names(self, mission_starts, orig_activity, db_alias):
        ...
        for mc in range(len(mission_starts) - 1, -1, -1):
            ...
            if len(name) > 255:
                self.logger.warn(
                    f"Mission named parsed from TethysDash is too big with length of {len(name)}"
                )
    ...
    def _create_mission_activities(
        self, db_alias, orig_activity, starts, ends, names, syslog_url
    ):
        ...
        activitytype, _ = ActivityType.objects.using(db_alias).get_or_create(
            name=LRAUV_MISSION
        )
        ...
        for name, mission_activity in ma.items():
            ...
            try:
                act = Activity.objects.using(db_alias).get(
                    name=name,
    ...
    def _update_mission_activity(
        ...
        for name in set(activity_by_mission.keys()):
            ...
            self.logger.info(
                f"mindepth = {mindepths[name]}, maxdepth = {maxdepths[name]}, num_measuredparameters = {num_measuredparameters[name]}"
            )

            act = Activity.objects.using(db_alias).get(
                name=name, activitytype=activitytype, platform=orig_activity.platform
            )
            try:
    ...
    def load_missions(
        self, platform_name, activity_name, url, db_alias, save_attributes=False
    ):
        ...
        syslog_url = "{}/syslog".format(
            "/".join(url.replace("opendap/", "").split("/")[:-1])
        )
        self.logger.info(
            f"Getting mission start information from the TethysDash API that's also in {syslog_url}"
        )
        ...
        starts, ends, names = self._make_starts_ends_names(
            mission_starts, orig_activity, db_alias
        )

        rt, _ = ResourceType.objects.using(db_alias).get_or_create(
            name=LRAUV_MISSION,
        ...
        if save_attributes:
            ...
            rdt, _ = ResourceType.objects.using(db_alias).get_or_create(
                name=LABEL, description="metadata"
            )
            lrauv_mission_res, _ = Resource.objects.using(db_alias).get_or_create(
                name=LRAUV_MISSION,
        ...
        for start, end, name in zip(starts, ends, names):
            ...
            meas_qs = Measurement.objects.using(db_alias).filter(
                instantpoint__activity=orig_activity,
            ...
            if save_attributes:
                self.logger.info(
                    f"LRAUV mission '{name}' in Activity {orig_activity}: start={start}, end={end}, duration={end - start}"
                )
                self.logger.info(
                    f"Associating with MeasuredParameters for Attribute selection in the UI"
                )
                ...
                res, _ = Resource.objects.using(db_alias).get_or_create(
                    name=LRAUV_MISSION, value=name, resourcetype=rt
                )
                # Associate with Resource that describes the source
                ResourceResource.objects.using(db_alias).get_or_create(
                    fromresource=res, toresource=lrauv_mission_res
                )
                # Create collections of MeasuredParameterResources for each Measurement in the Mission
                for count, meas in enumerate(meas_qs):
                    if not count % 100:
                        self.logger.debug(
                            f"{count:5d}: timevalue = {meas.instantpoint.timevalue}"
                        )
                    for mp in MeasuredParameter.objects.using(db_alias).filter(
                        measurement=meas
                    ):
                        num_measuredparameters[name] += 1
                        mpr, _ = MeasuredParameterResource.objects.using(
                            db_alias
                        ).get_or_create(
                            measuredparameter=mp, resource=res, activity=orig_activity
                        )

            # Associate copies of SimpleDepthTime with the mission Activity for overview data viz
            self.logger.info(
                f"Associating with SimpleDepthTime copies with Activity {name}"
            )
            last_sdt = None
            for count, sdt in enumerate(
                SimpleDepthTime.objects.using(db_alias).filter(
                ...
                if not count % 100:
                    self.logger.debug(
                        f"{count:5d}: timevalue = {sdt.instantpoint.timevalue}"
                    )
                sdt.pk = None
            ...
            if last_sdt:
                if name != last_name:
                    ...
                    sdt_break = SimpleDepthTime(
                        instantpoint=last_sdt.instantpoint,
                        depth=np.nan,
