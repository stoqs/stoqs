...
class PlanktonPump:
    ...
    def _pumping_start_time(self, a_name, v):
        ...
        samples = self._niskin_samples(a_name, v)
        last_bottle_dt = list(
            samples.aggregate(Max("instantpoint__timevalue")).values()
        )[0]
    ...
    def _pumping_instantpoint(self, a_name, v):
        ...
        samples = self._niskin_samples(a_name, v)
        ip = samples.get(
            name=max(self.pump_bottles[v["Relative Depth"]][0])
        ).instantpoint
    ...
    def _db_join(self, sm_hash):
        ...
        """
        self.logger.info(
            "Joining subsample information with bottle data from the"
            " database using duration = %d",
            self.args.duration,
        )
    ...
    def _create_activity_instantpoint_platform(
        self, r, duration_minutes, planktonpump_name, point
    ):
        ...
        act, created = Activity.objects.using(self.args.database).get_or_create(
            ...
            comment="Plankton pump done in conjunction with CTD cast %s"
            % r.get("Cast"),
            platform=platform,
            startdate=timevalue,
        ...
        with open(self.args.load_file) as f:
            for r in csv.DictReader(f):
                ...
                act, ip = self._create_activity_instantpoint_platform(
                    ...
                    planktonpump_name=self.pump_bottles[r.get("RelativeDepth")][1],
                    point=point,
                )
    ...
    def process_command_line(self):
        ...
        examples += (
            "  - cl.parent_nettow_file=2013_SIMZ_PlanktonPump_ParentSamples.csv\n"
        )
        examples += (
            "  - Add 'SIMZ_2013_PPump_STOQS_tidy_v2.csv' to cl.subsample_csv_files\n"
        )
        ...
        if self.args.subsampleFile:
            if not self.args.csv_file:
                parser.error(
                    "Must include --csv_file argument with --subsampleFile option"
                )
