...
for year in range(2010, 2011):
    text = BeautifulSoup(urllib.request.urlopen(base_url % {"year": year})).findAll(
        text=True
    )
    urllist += [
        (base_url + file) % {"year": year}
        for file in text
        if file.endswith("_decim.nc")
    ]
...
for url in urllist:
    ...
    bl = dl.Base_Loader(
        survey_name, platform=m.Platform.objects.get(code="gulper"), url=url, stride=1
    )
