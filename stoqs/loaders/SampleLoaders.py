...
from stoqs.models import ActivityType
from stoqs.models import AnalysisMethod
from stoqs.models import Campaign
...
from stoqs.models import ParameterResource
from stoqs.models import Platform
from stoqs.models import PlatformType
from stoqs.models import Resource
from stoqs.models import ResourceType
from stoqs.models import Sample
from stoqs.models import SampledParameter
from stoqs.models import SamplePurpose
from stoqs.models import SampleRelationship
from stoqs.models import SampleResource
from stoqs.models import SampleType
...
SIPPER_NUM_ERR = re.compile(
    "Sample (?P<sipper_num>\d+), \(?err_code=(?P<sipper_err>\d+)\)?"
)
...
lsr_volume_re_paused = (
    r".*[Ss]ampl.+\s+(?P<volume_num>[-+]?\d*\.\d+)(?P<volume_units>[a-z]{2})"
)
...
CONSOLIDATED_MSG = re.compile(
    r"ESP Cartridge (?P<cartridge_number>\d+): S_FILTERING: start: (?P<start_dt>\S+) end: (?P<end_dt>\S+)"
)
...
beg_syslog_re = (
    iso_time
    + r",(?P<log_esec>\d*\.\d+)\s\[(?P<log_component>[^\]]+)\]\((?P<log_level>[^\)]+)\):\s(?P<log_message>[^\n]+)"
)
...
def get_closest_instantpoint(aName, tv, dbAlias):
    ...
    while tol < 86400:  # Fail if not found within 24 hours
        qs = (
            InstantPoint.objects.using(dbAlias)
            .filter(
                activity__name__contains=aName,
...
class ParentSamplesLoader(STOQS_Loader):
    ...
    def load_gulps(self, activityName, auv_file, dbAlias):
        ...
        try:
            ...
            self.logger.warn(
                "Failed to find Activity with name like %s.  Skipping GulperLoad.",
                activityName,
            )
            return
        except MultipleObjectsReturned:
            self.logger.warn(
                "Multiple objects returned for name__contains = %s.  Selecting one by random and continuing...",
                activityName,
            )
            activity = Activity.objects.using(dbAlias).filter(
                name__contains=activityName
            )[0]

        try:
            if NOWATER in (
                ActivityResource.objects.using(dbAlias)
                .get(
                    activity=activity,
                ...
                self.logger.warn(
                    f"Found '{NOWATER}' text in title of {activity.name}, not adding Gulpers"
                )
                return
        except ActivityResource.DoesNotExist:
            self.logger.warn(
                f"Could not find 'title' nc_global attribute in ActivityResource for Activity {activity}"
            )
        ...
        (gulper_type, created) = SampleType.objects.using(dbAlias).get_or_create(
            name=GULPER
        )
        ...
        with requests.get(gulperUrl, stream=True) as r:
            if r.status_code != 200:
                self.logger.warn(
                    "Cannot read %s, r.status_code = %s", gulperUrl, r.status_code
                )
                return
            ...
            for row in reader:
                ...
                try:
                    timevalue = datetime(int(yyyy), 1, 1) + timedelta(
                        days=(float(row[r"YearDay [day]"]) - 1)
                    )
                except TypeError as e:
                    self.logger.error(
                        "%s.  Skipping this Sample - you may want to fix the input file",
                        e,
                    )
                    continue
                try:
                    ip, seconds_diff = get_closest_instantpoint(
                        activityName, timevalue, dbAlias
                    )
                    point = Point(
                        float(row[r"Lon (degrees_east)"]) - 360.0,
                        float(row[r"Lat (degrees_north)"]),
                    )
                    stuple = Sample.objects.using(dbAlias).get_or_create(
                        name=row[r"Bottle Number [count]"],
                        depth=row[r"DEPTH [m]"],
                        geom=point,
                        instantpoint=ip,
                    ...
                    rtuple = Resource.objects.using(dbAlias).get_or_create(
                        name="Seconds away from InstantPoint", value=seconds_diff
                    )

                    # 2nd item of tuples will be True or False dependending on whether the object was created or gotten
                    self.logger.info(
                        "Loaded Sample %s with Resource: %s", stuple, rtuple
                    )
                except ClosestTimeNotFoundException:
                    self.logger.warn(
                        "ClosestTimeNotFoundException: A match for %s not found for %s",
                        timevalue,
                        activity,
                    )

    def load_gulper_activities(
        self, activity_name, auv_file, db_alias, url, platform_color, sec_bnds: int = 1
    ):
        ...
        for sample_name, sample_esecs in bottle_times.items():
            ...
            gulper_names[sample_name] = SampleInfo(
                sample_esecs - sec_bnds, sample_esecs + sec_bnds, 0.8, summary
            )

        pt, _ = PlatformType.objects.using(db_alias).get_or_create(name="auv")
        platform, _ = Platform.objects.using(db_alias).get_or_create(
            name="dorado_Gulper", platformtype=pt, color=platform_color
        )
    ...
    def _get_lrauv_esp_sample_platform(
        self, db_alias, lrauv_platform, sample_type, esp_device=None
    ):
        ...
        if esp_device:
            p_name = (
                f"{lrauv_platform.name}_{sample_type.name.replace('ESP', esp_device)}"
            )
        else:
            p_name = f"{lrauv_platform.name}_{sample_type}"
        pt, _ = PlatformType.objects.using(db_alias).get_or_create(name="auv")
        platform, _ = Platform.objects.using(db_alias).get_or_create(
            name=p_name, platformtype=pt, color=lrauv_platform.color
        )
        return platform

    def _create_activity_instantpoint_platform(
        self,
        db_alias,
        platform_name,
        activity_name,
        ...
        """
        campaign = Campaign.objects.using(db_alias).filter(
            activity__name=activity_name
        )[0]
        lrauv_platform = Platform.objects.using(db_alias).filter(
            activity__name=activity_name
        )[0]
        platform = self._get_lrauv_esp_sample_platform(
            db_alias, lrauv_platform, sample_type, esp_device
        )
        ...
        ip_qs = (
            InstantPoint.objects.using(db_alias)
            .filter(
                activity__name=activity_name, timevalue__gte=sdt, timevalue__lte=edt
            )
            .order_by("timevalue")
        )
        if not ip_qs:
            self.logger.warn(
                f"No InstantPoints for Sample '{sample_name}' between {sdt} and {edt}"
            )
            if db_alias.endswith("_t"):
                self.logger.warn(
                    f"Perhaps doing a high stride test load? - skipping Sample '{sample_name}'"
                )
            else:
                self.logger.warn(
                    f"*** No timevalue data available for Sample '{sample_name}' between {sdt} and {edt}"
                )
                self.logger.warn(
                    f"*** Not loading Sample '{sample_name}' because there is no corresponding measurement or location data."
                )

            return None, None, None, None, None

        m_qs = Measurement.objects.using(db_alias).filter(
            instantpoint__activity__name=activity_name,
            instantpoint__timevalue__gte=sdt,
            instantpoint__timevalue__lte=edt,
        )
        qs = m_qs.aggregate(Min("depth"), Max("depth"), Avg("depth"))
        self.logger.info(
            f"Between {sdt} and {edt} depth__min={qs['depth__min']}, depth__max={qs['depth__max']}"
        )
        duration = edt - sdt
        max_hours = 2
        ...
        if duration > timedelta(hours=max_hours):
            self.logger.warn(
                f"Time duration of Sample '{sample_name}' is longer than {max_hours} hours: duration = {duration}"
            )
        ...
        if esp_device:
            ...
            self.logger.warning(
                f"esp_device not set, even after checking all syslogs in dlist_dir: {'/'.join(url.split('/')[:10])}"
            )
        ...
        self.logger.info(
            f"Creating {len(m_qs)} Measurements for a time series of this Sample Activity"
        )
        for me in m_qs:
            measurement = self.createMeasurement(
                mtime=me.instantpoint.timevalue,
                depth=me.depth,
                lon=me.geom.x,
                lat=me.geom.y,
            )
        ...
        if len(m_qs.values_list("geom", flat=True)) > 1:
            point = LineString(
                [p for p in m_qs.values_list("geom", flat=True)]
            ).centroid
            maptrack = LineString(
                [p for p in m_qs.values_list("geom", flat=True)]
            ...
            maptrack = LineString(
                [
                    m_qs.values_list("geom", flat=True)[0],
                    m_qs.values_list("geom", flat=True)[0],
                ]
            )
        self.logger.debug(
            f"Creating a single point for this Sample with timevalue = {sample_tv} at location {point}"
        )
        sample_ip, _ = InstantPoint.objects.using(db_alias).get_or_create(
            activity=sample_act, timevalue=sample_tv
        )
        depth = qs["depth__avg"]
        ...
        self.logger.info(
            f"Associating Resources for UI display of Parameters tab for sample_act = {sample_act}"
        )
        ncg_res_type, _ = ResourceType.objects.using(self.dbAlias).get_or_create(
            name="nc_global"
        )
        ui_res_type, _ = ResourceType.objects.using(self.dbAlias).get_or_create(
            name="ui_instruction"
        )
        ft_res, _ = Resource.objects.using(self.dbAlias).get_or_create(
            name="featureType", value="trajectory", resourcetype=ncg_res_type
        )
        ptsd_res, _ = Resource.objects.using(self.dbAlias).get_or_create(
            name="plotTimeSeriesDepth", value=0, resourcetype=ui_res_type
        )
        ar, _ = ActivityResource.objects.using(self.dbAlias).get_or_create(
            activity=sample_act, resource=ft_res
        )
        self.logger.info(f"ActivityResource.resource = {ar.resource}")
        ar, _ = ActivityResource.objects.using(self.dbAlias).get_or_create(
            activity=sample_act, resource=ptsd_res
        )
        self.logger.info(f"ActivityResource.resource = {ar.resource}")
        for parm in parameter_counts:
            pr, _ = ParameterResource.objects.using(self.dbAlias).get_or_create(
                parameter=parm, resource=ptsd_res
            )
            self.logger.info(
                f"parm = {parm}, ParameterResource.resource = {pr.resource}"
            )

        return sample_act, sample_ip, point, depth, maptrack

    def _read_syslog(self, url, levels=("IMPORTANT",), components=("ESPComponent",)):
        syslog_url = "{}/syslog".format(
            "/".join(url.replace("thredds/dodsC/LRAUV/", "data/lrauv/").split("/")[:-1])
        )
        ...
        if "realtime" in syslog_url:
            self.logger.info(
                f"String 'realtime' found in {syslog_url} - not attempting to read it"
            )
        ...
        with requests.get(syslog_url, stream=True) as resp:
            ...
            for line in (lb.decode(errors="ignore") for lb in resp.iter_lines()):
                ...
                if lm:
                    ...
                    if (
                        lm.groupdict().get("log_component") in ("ESPComponent",)
                        and lm.groupdict().get("log_level") in levels
                    ):
                        ...
                        if lm.groupdict().get("log_level") in levels:
                            ...
                            log_rows.append(
                                {
                                    "unixTime": float(lm.groupdict().get("log_esec"))
                                    * 1000,
                                    "text": lm.groupdict().get("log_message"),
                                }
                            )

                    elif (
                        lm.groupdict().get("log_component") == "ESPComponent"
                        and lm.groupdict().get("log_level") == "INFO"
                    ):
                        ...
                        esp_m = re.match(
                            "console:(ESP\S+) login", lm.groupdict().get("log_message")
                        )
                        if esp_m:
                        ...
                        if message:
                            log_rows.append(
                                {"unixTime": float(prev_esec) * 1000, "text": message}
                            )
                            prev_message = ""
    ...
    def _read_tethysdash(self, platform_name, url):
        ...
        try:
            if int(et[0:4]) > datetime.now().year:
                self.logger.warn(
                    f"Not looking for Samples for url = {url} as the to date is > {datetime.now().year}"
                )
        ...
        with requests.get(td_url) as resp:
            if resp.status_code != 200:
                self.logger.error(
                    "Cannot read %s, resp.status_code = %s", td_url, resp.status_code
                )
                return
    ...
    def _esps_from_json_consolidated(
        self, platform_name, url, db_alias, use_syslog=True
    ):
        ...
        for rec in log_important:
            ...
            if mcm := re.search(CONSOLIDATED_MSG, rec["text"]):
                ...
                start_esec = datetime.strptime(
                    mcm.groupdict().get("start_dt")[:-5], "%Y-%m-%dT%H:%M:%S"
                ).timestamp()
                end_esec = datetime.strptime(
                    mcm.groupdict().get("end_dt")[:-5], "%Y-%m-%dT%H:%M:%S"
                ).timestamp()
                if end_esec > start_esec:
                    ...
                    esp_log_summaries.append(
                        Log(rec["unixTime"] / 1000.0, rec["text"] + summary_text)
                    )
                else:
                    self.logger.warning(
                        f"Not saving Sample {rec['text']} as the end is before the start"
                    )
    ...
    def _esps_from_json(self, platform_name, url, db_alias, use_syslog=True):
        ...
        try:
            esp_s_filtering = [
                Log(d["unixTime"] / 1000.0, d["text"])
                for d in log_important
                if FILTERING in d["text"]
            ]
        except KeyError:
            self.logger.debug(
                f"No '{no_num_sampling_start_re}' messages found in syslog for {url}"
            )
        try:
            esp_s_stopping = [
                Log(d["unixTime"] / 1000.0, d["text"])
                for d in log_important
                if PROCESSING in d["text"]
            ]
        except KeyError:
            self.logger.debug(
                f"No '{no_num_sampling_end_re}' messages found in syslog for {url}"
            )
        try:
            esp_log_summaries = [
                Log(d["unixTime"] / 1000.0, d["text"])
                for d in log_important
                if LOGSUMMARY in d["text"]
            ]
        except KeyError:
            self.logger.debug(
                f"No '{lsr_num_messages_re}' messages found in syslog for {url}"
            )
        try:
            esp_log_criticals = [
                Log(d["unixTime"] / 1000.0, d["text"]) for d in log_critical
            ]
        except KeyError:
        ...
        if esp_s_filtering and esp_s_stopping and esp_log_summaries:
            self.logger.info(
                f"Parsed {len(esp_log_summaries)} Samples (esp_log_summaries) from syslog for {url}"
            )
            if (
                "stoqs_ecohab_april2023" in db_alias
                and "brizo/missionlogs/2023/20230421_20230427/20230425T183900" in url
            ):
                self.logger.info(
                    f"Will MacGyver a fix to add Cartridge 15 for its missing Log Summary Report - a special fix for stoqs_ecohab_april2023"
                )
                self.logger.info(
                    f"Add 30 seconds to [sample #3]'s stopping (S_PROCESSING) time to fudge a LSR with times Cartridge 15"
                )
                esp_log_summaries.append(
                    Log(
                        esp_s_stopping[1].esec + 30,
                        "[sample #3] ESP log summary report (2 messages):\n@15:47:39.71 Selecting Cartridge 15\n@16:38:51.27 Sampled  1000.0ml\n",
                    )
                )
            ...
            if "stoqs_canon_may2018" in db_alias:
                self.logger.info(
                    f"Will assign Cartridge numbers for the Campaign to the Samples - a special fix for stoqs_canon_may2018"
                )
                numbers = range(
                    self.esp_cartridge_number - len(esp_s_filtering),
                    self.esp_cartridge_number,
                )
                for i, filtering, stopping in zip(
                    numbers, esp_s_filtering, esp_s_stopping
                ):
                    self.logger.info(
                        f"Cartridge {i} assigned to Sample that started filtering at {filtering.esec} and stopped at {stopping.esec}"
                    )
                    esp_log_summaries.append(Log(filtering.esec, f"Cartridge {i}"))
                    self.esp_cartridge_number -= 1
            else:
                self.logger.info(
                    f"Will assign sequence numbers per log file to the Samples"
                )
                for i, filtering, stopping in zip(
                    range(len(esp_s_filtering), 0, -1), esp_s_filtering, esp_s_stopping
                ):
                    self.logger.info(
                        f"Sequence {i} assigned to Sample that started filtering at {filtering.esec} and stopped at {stopping.esec}"
                    )
                    esp_log_summaries.append(Log(filtering.esec, f"Sequence {i}"))
            self.logger.info(
                f"Parsed {len(esp_s_filtering)} Samples from syslog for {url} with no LOGSUMMARY reports"
            )
        else:
    ...
    def _check_leaked_cartridge(self, platform_name, summary):
        ...
        while index < len(summary.text):
            ...
            if occurrences > 1:
                self.logger.info(
                    f"Repeated 'Selecting Cartridge' message found in {platform_name}'s summary.text: {summary.text}"
                )
                if (
                    "Cmd::SpareCartridge" in summary.text
                    or "Cartridge::Sampler::Leak" in summary.text
                ):
                    # Assumes that there are multiple 'Selecting Cartridge' messages in summary.text: the last is the one used, the others leaked
                    self.logger.info(
                        f"Found 'Cmd::SpareCartridge' message, using last Cartridge number for sample name"
                    )
                    return re.findall(
                        lsr_cartridge_number_re, summary.text, re.MULTILINE
                    )
    ...
    def _filter_stop_nums(self, filterings, stoppings, summaries):
        ...
        for filtering, stopping, summary in zip(filterings, stoppings, summaries):
            ...
            ms = (
                re.match(sampling_start_re, filtering.text)
                or re.match(no_num_sampling_start_re, filtering.text)
                or re.match(sample_prefix, filtering.text)
            )
            me = (
                re.match(sampling_end_re, stopping.text)
                or re.match(no_num_sampling_end_re, stopping.text)
                or re.match(sample_prefix, stopping.text)
            )
    ...
    def _validate_summaries(
        self, platform_name, filterings, stoppings, summaries, criticals
    ):
        ...
        if not (len(filterings) == len(stoppings) == len(summaries)):
            self.logger.warn(
                f"Mismatch in the number of ESP Sampling messages: {len(filterings)=}, {len(stoppings)=}, {len(summaries)=}"
            )

            filter_nums, stop_nums = self._filter_stop_nums(
                filterings, stoppings, summaries
            )

            # 0. Report case with more filterings than summaries (summary likely in the following syslog)
            if len(summaries) < len(filterings):
                self.logger.info(
                    f"{len(summaries)=} < {len(filterings)=}. Summary for {filterings[-1]} likely in the following syslog."
                )
            ...
            # 1. Correct case where we have more summaries than filterings
                ...
                self.logger.info(
                    "One more summary than filtering in this syslog. There may be matching ones in previous syslogs."
                )
                if self.prev_filterings:
                    self.logger.info(
                        f"Prepending {self.prev_filterings[-1]=} to {filterings=}"
                    )
                    filterings = [self.prev_filterings[-1]] + filterings
                else:
                    self.logger.info(
                        "No prev_filterings exist. Here are the ESP messages from this syslog:"
                    )
            ...
            if len(summaries) > len(filterings):
                ...
                to_del = []
                self.logger.info(
                    f"{len(summaries)=} > {len(filterings)=}. Checking that numbers in filter_nums match '{sampling_start_re}'"
                )
                for index, summary in enumerate(summaries):
                    ...
                    if lsr_seq_num.groupdict().get("seq_num") not in filter_nums:
                        self.logger.warn(
                            f"Sample seq_num ({lsr_seq_num.groupdict().get('seq_num')}) not found in filter_nums: {filter_nums}"
                        )
                        self.logger.warn(
                            f"Likely an error for this cartridge: {summary.text}"
                        )
                        crit_err = ""
                        for lc in criticals:
                            lc_seq_num = re.search(
                                lsr_seq_num_re, lc.text, re.MULTILINE
                            )
                            if lsr_seq_num.groupdict().get(
                                "seq_num"
                            ) == lc_seq_num.groupdict().get("seq_num"):
                                crit_err = lc.text
                                break
                        try:
                            lsr_cartridge_number = (
                                re.search(
                                    lsr_cartridge_number_re, summary.text, re.MULTILINE
                                )
                                .groupdict()
                                .get("cartridge_number")
                            )
                        except AttributeError as e:
                            # Likely: AttributeError: 'NoneType' object has no attribute 'groupdict'
                            self.logger.warn(
                                f"{e}: Could not find cartridge_number in summary.text"
                            )
                        else:
                            self.logger.info(
                                f"Not loading Cartridge {lsr_cartridge_number} at index {index}"
                            )
                            self.logger.info(
                                f"Disposition of {platform_name} Cartridge {lsr_cartridge_number}: Not saving, no corresponding"
                        ...
                        while index < len(summary.text):
                            ...
                            if occurrences > 1:
                                ...
                                self.logger.info(
                                    f"Disposition of {platform_name} Cartridge {lsr_cartridge_number}: Not saving, repeated entry"
            ...
            # 2. Report case where we have more summaries than stoppings
                ...
                self.logger.info(
                    f"len(summaries) > len(stoppings). Checking that numbers in stop_nums match '{sampling_end_re}'"
                )
                to_del = []
                for index, summary in enumerate(summaries):
                    lsr_seq_num = re.search(lsr_seq_num_re, summary.text, re.MULTILINE)
                    if lsr_seq_num.groupdict().get("seq_num") not in stop_nums:
                        for lc in criticals:
                            ...
                            if lsr_seq_num.groupdict().get(
                                ...
                                if cart_num_text := re.search(
                                    lsr_cartridge_number_re, summary.text, re.MULTILINE
                                ):
                                    lsr_cartridge_number = (
                                        cart_num_text.groupdict().get(
                                            "cartridge_number"
                                        )
                                    )
                                    self.logger.info(
                                    ...
                                    self.logger.info(
                                        f"Disposition of {platform_name} [sample #{lsr_seq_num.groupdict().get('seq_num')}]:"
            ...
            if len(mult_carts) > 1:
                ...
                summaries[index] = Log(
                    ...
                    summary.text.replace(
                        f"Cartridge {mult_carts[0]}", f"Cartridge {mult_carts[-1]}"
                    ),
                )
                self.logger.info(
                    f"Replaced first (leaked={mult_carts[:-1]}) Cartridge number(s) with last (spare_used={mult_carts[-1]})"
        ...
        for filtering, stopping, summary in zip(filterings, stoppings, summaries):
            try:
                ...
                self.logger.warning(
                    f"Likely ESP state messages without 'sample #' text"
                )
        ...
        for num, count in num_dict.items():
            if count > 1:
                self.logger.warning(
                    f"[sample #{num}] found more that once ({count}) in syslog"
                )
                ...
                for index, summary in enumerate(summaries):
                    ...
                    if lsr_seq_num.groupdict().get("seq_num") == num:
                        self.logger.info(
                            f"index {index:3}: summary.text = {summary.text}"
                        )
                        if len(summary.text) < max_summ_len:
                            self.logger.info(
                                f"Will delete shorter summary.text with index = {index}"
                            )
                            to_del.append(index)

        for index in reversed(to_del):
            self.logger.info(
                f"Deleting index {index} from filtering, stoppings, and summaries lists"
            )
        ...
        for summary in summaries:
            try:
                lsr_cartridge_number = (
                    re.search(lsr_cartridge_number_re, summary.text, re.MULTILINE)
                    .groupdict()
                    .get("cartridge_number")
                )
            ...
            try:
                self.logger.info(
                    f"Disposition of {platform_name} Cartridge {lsr_cartridge_number}: Passed validation, marked for saving"
                    f" [sample #{lsr_seq_num.groupdict().get('seq_num')}]"
                )
            except AttributeError:
                self.logger.warn(
                    f"Likely lsr_seq_num ({lsr_seq_num}) is None and this log is before 'sample #' was implemented completely"
                )
                self.logger.info(
                    f"Disposition of {platform_name} Cartridge {lsr_cartridge_number}: Passed validation, marked for saving"
                )
    ...
    def _get_lsr_volume(self, summary) -> float:
        ...
        if not lsr_volume:
            ...
            self.logger.debug(
                f"Attempting with lsr_volume_re_paused regex: {lsr_volume_re_paused}"
            )
        ...
        if lsr_volume:
            if lsr_volume.groupdict().get("volume_units") == "ml":
                ...
                self.logger.debug(
                    f"Parsed from log summary report volume = {volume} ml"
                )

        return volume

    def _match_seq_to_cartridge(
        self,
        filterings,
        stoppings,
        summaries,
        ...
        for filtering, stopping, summary in zip(filterings, stoppings, summaries):
            ...
            lsr_seq_num = re.search(lsr_seq_num_re, summary.text, re.MULTILINE)
            lsr_lsr_num_messages = re.search(
                lsr_num_messages_re, summary.text, re.MULTILINE
            )
            if consolidated_msg:
                lsr_cartridge_number = (
                    re.search(CONSOLIDATED_MSG, summary.text)
                    .groupdict()
                    .get("cartridge_number")
                )
            ...
            lsr_esp_error_msg = re.search(
                lsr_esp_error_msg_re, summary.text, re.MULTILINE
            )
            ...
            try:
                if before_seq_num_implemented:
                    self.logger.info(
                        f"This log is before seq_num was implemented - not checking for match"
                    )
                elif consolidated_msg:
                    self.logger.info(
                        f"This log uses the consolidated message - not checking for match"
                    )
                elif not (
                    ms.groupdict().get("seq_num")
                    == me.groupdict().get("seq_num")
                    == lsr_seq_num.groupdict().get("seq_num")
                ):
                    self.logger.warn(
                        f"Sample numbers do not match for '{filtering.text}', '{stopping.text}', and '{summary.text}'"
                    )
            except AttributeError:
                if filtering and stopping and not lsr_seq_num:
                    sample_name = summary.text
                    self.logger.info(
                        f"No ESP log summary report: Assigning sample_name = {sample_name}"
                    )
                else:
                    self.logger.warn(
                        f"Sample numbers do not match for '{filtering.text}', '{stopping.text}', and '{summary.text}'"
                ...
                try:
                    ...
                    if lsr_seq_num:
                        self.logger.info(
                            f"sample # = {lsr_seq_num.groupdict().get('seq_num')}, sample_name = {sample_name}, volume = {volume} ml"
                        )
                    else:
                        self.logger.info(
                            f"(No 'sample #' match) sample_name = {sample_name}"
                        )
                except AttributeError:
                    ...
                    self.logger.info(
                        f"No ESP Cartridge number found: Assigning sample_name = {sample_name}"
                    )
            ...
            if lsr_esp_error_msg:
                if lsr_esp_error_msg.groupdict().get("esp_error_message"):
                    ...
                    merr = re.match(
                        ".+actually (\d+)([a-z]+)",
                        lsr_esp_error_msg.groupdict().get("esp_error_message"),
                    )
                    if merr:
                        if merr.group(2) == "ul":
                            self.logger.info(
                                f"Saving 'actually' volume following from {lsr_esp_error_msg.groupdict().get('esp_error_message')}"
                            )
                            volume = float(merr.group(1)) * 1.0e-3
                    else:
                        ...
                        self.logger.info(
                            f"Error encountered: {lsr_esp_error_msg.groupdict().get('esp_error_message')}"
                        )

            sample_names[sample_name] = SampleInfo(
                filtering.esec, stopping.esec, volume, summary.text
            )
    ...
    def _sippers_from_json(self, platform_name, url, use_syslog=True):
        ...
        SIPPERING = (
            "ELMO will go to position"  # Is an 'INFO' message, use log_info messages
        )
        if use_syslog:
            log_info, _ = self._read_syslog(
                url, components=("CANONSampler",), levels=("INFO",)
            )
            log_important, _ = self._read_syslog(
                url, components=("CANONSampler",), levels=("IMPORTANT",)
            )
            log_critical, _ = self._read_syslog(
                url, components=("CANONSampler",), levels=("CRITICAL",)
            )
        else:
            log_important = self._read_tethysdash(platform_name, url)
        ...
        try:
            sample_num_errs = [
                Log(d["unixTime"] / 1000.0, d["text"])
                for d in log_info
                if re.match(SIPPER_NUM_ERR, d["text"])
            ]
        except KeyError:
        ...
        if samplings_at and sample_num_errs:
            self.logger.info(
                f"Parsed {len(samplings_at)} '{SIPPERING}' strings and {len(sample_num_errs)} Sample numbers from {url}'s syslog"
            )
        else:
    ...
    def _match_sippers(self, samplings_at, sample_num_errs):
        ...
        if len(samplings_at) != len(sample_num_errs):
            self.logger.warn(
                f"len(samplings_at) [{len(samplings_at)}] != len(sample_num_errs) [{len(sample_num_errs)}]"
            )
            self.logger.warn(
                "A Sipper logging error might have occurred -- check syslog"
            )
        ...
        for sample_at, sample_num_err in zip(samplings_at, sample_num_errs):
            ...
            sipper_names[sample_name] = SampleInfo(
                sample_at.esec, sample_num_err.esec, volume, summary
            )
    ...
    def _save_samples(
        ...
        for sample_name, sample in samples.items():
            self.logger.info(
                f"Calling _create_activity_instantpoint_platform() for sample_name={sample_name}"
            )
            try:
                act, ip, point, depth, maptrack = (
                    self._create_activity_instantpoint_platform(
                        db_alias,
                        platform_name,
                        activity_name,
                        sampletype,
            ...
            try:
                ...
                self.logger.info(
                    f"It's possible that the sample_name is too long: {sample_name}"
                )
                raise
            ...
            self.logger.info(
                f"Saving {sampletype.name} Sample '{sample_name}': {sample.summary}, volume = {sample.volume} ml"
            )
            act.save(using=db_alias)
            self.logger.debug(
                f"Updated Activity with point={point} and maptrack={maptrack}"
            )

            # Associate Resource (log summary report text) with Sample
            res, _ = Resource.objects.using(db_alias).get_or_create(
                name=log_text, value=sample.summary
            )
            SampleResource.objects.using(db_alias).get_or_create(
                sample=samp, resource=res
            )
            ...
            if sampletype.name.startswith("ESP"):
                if esp_device:
                    res, _ = Resource.objects.using(db_alias).get_or_create(
                        name="ESP_device_name", value=esp_device
                    )
                    SampleResource.objects.using(db_alias).get_or_create(
                        sample=samp, resource=res
                    )
                    ...
                    self.logger.warn(
                        f"SampleType is {sampletype.name}, but no ESP device name found in syslog for {platform_name} {samp}"
                    )

    def load_lrauv_samples(
        self, platform_name, activity_name, url, db_alias, use_consolidated_msg=True
    ):
        """
        url looks like 'http://dods.mbari.org/opendap/data/lrauv/tethys/missionlogs/2018/20180906_20180917/20180908T084424/201809080844_201809112341_2S_scieng.nc'
        """
        self.logger.info(
            f"Parsing ESP sample messages from /mbari/LRAUV/{'/'.join(url.split('/')[6:-1])}/syslog"
        )
        if use_consolidated_msg:
            ...
            (
                ...
                init_criticals,
                esp_device,
            ) = self._esps_from_json_consolidated(platform_name, url, db_alias)
        else:
            (
                init_filterings,
                init_stoppings,
        ...
        if not init_filterings and not init_stoppings and not init_summaries:
            ...
            self.logger.info(
                f"No consolidated ESP messages found, using legacy method to match filterings and stoppings"
            )
            (
                init_filterings,
                init_stoppings,
                ...
                init_criticals,
                esp_device,
            ) = self._esps_from_json(platform_name, url, db_alias)
            filterings, stoppings, summaries = self._validate_summaries(
                platform_name,
                init_filterings,
                init_stoppings,
                init_summaries,
            ...
            consolidated_msg = True
            self.logger.info(
                f"Consolidated ESP messages found, using Cartridge number, start, and end time listed there"
            )
            filterings, stoppings, summaries = (
                init_filterings,
                init_stoppings,
        ...
        if filterings and stoppings and summaries:
            esp_names = self._match_seq_to_cartridge(
                filterings,
                stoppings,
                summaries,
        ...
        if esp_names:
            (esp_archive_type, created) = SampleType.objects.using(
                db_alias
            ).get_or_create(name=ESP_FILTERING)
        ...
        if sipper_names:
            (sipper_type, created) = SampleType.objects.using(db_alias).get_or_create(
                name=SIPPER
            )
...
class SeabirdLoader(STOQS_Loader):
    ...
    def load_data(self, lat, lon, depth, mtime, parmNameValues):
        ...
        try:
            measurement = self.createMeasurement(
                mtime=mtime, depth=depth, lat=lat, lon=lon
            )
            ...
            self.logger.debug(
                "longitude = %s, latitude = %s, mtime = %s, depth = %s",
                lon,
                lat,
                mtime,
                depth,
        ...
        for pn, value in parmNameValues:
            ...
            try:
                mp, _ = MeasuredParameter.objects.using(self.dbAlias).get_or_create(
                    measurement=measurement,
                ...
                self.logger.exception(
                    "Bad value (id=%(id)s) for %(pn)s = %(value)s",
                    {"pn": pn, "value": value, "id": mp.pk},
                )
            else:
                loaded += 1
                self.logger.debug(
    ...
    def load_btl(self, lat, lon, depth, timevalue, bottleName):
        ...
        # Get the Activity from the Database
        try:
            activity = Activity.objects.using(self.dbAlias).get(
            ...
            activity = Activity.objects.using(self.dbAlias).filter(
                name__contains=self.activityName
            )[0]

        # Get or create SampleType for Niskin
        (sample_type, created) = SampleType.objects.using(self.dbAlias).get_or_create(
            name=NISKIN
        )
        self.logger.debug("sampletype %s, created = %s", sample_type, created)
        # Get or create SamplePurpose for Niskin
        (sample_purpose, created) = SamplePurpose.objects.using(
            self.dbAlias
        ).get_or_create(name="StandardDepth")
        ...
        try:
            ...
            point = Point(lon, lat)
            Sample.objects.using(self.dbAlias).get_or_create(
                name=bottleName,
    ...
    def process_btl_file(self, fh, year, lat, lon, btlUrl):
        ...
        for line in fh:
            ...
            if not line.startswith("#") and not line.startswith("*"):
                ...
                if m:
                    ...
                    if _debug:
                        self.logger.debug("Spaces to dashes: line = %s", line)
        ...
        try:
            ...
            self.logger.warn(
                f"Not able to read time from bottle file for {self.activityName}"
            )
            return
        ...
        try:
            ...
            self.logger.warn(
                "Failed to find Activity with name like %s.  Expected that downcast was data before loading bottles.",
                self.activityName,
            )
            ...
            self.logger.error(
                "Multiple objects returned for name__contains = %s."
                "This should not happen.  Fix the database and the reason for this.",
                self.activityName,
            )
            raise SingleActivityNotFound(
                "Multiple objects returned for name__contains = %s" % self.activityName
            )
        ...
        for r in csv.DictReader(open(tmpFile), delimiter=" ", skipinitialspace=True):
            ...
            for name in list(parmDict.keys()):
                self.logger.debug(
                    "name = %s, parmDict[name].attributes = %s",
                    name,
                    parmDict[name].attributes,
                )
                try:
                    parmNameValues.append(
                        (name, float(r[parmDict[name].attributes["colname"]]))
                    )
        ...
        if fileList:
            ...
            for link in linkList:
                ...
                if bfile.endswith(".btl"):
                    ...
                    if (
                        bfile.split("/")[-1].split(".")[0] + ".nc"
                        not in seabirdFileList
                    ):
                        self.logger.warn(
                            "Skipping %s as it is in not in seabirdFileList = %s",
                            bfile,
                            seabirdFileList,
                        )
                        continue
...
class SubSamplesLoader(STOQS_Loader):
    ...
    def load_subsample(self, parentSample, row):
        ...
        if not parameter_value:
            raise SubSampleLoadError(
                "Must have a row['Parameter Value'] to load subsample"
            )

        (sampleType, created) = SampleType.objects.using(self.dbAlias).get_or_create(
            name="subsample"
        )
        (samplePurpose, created) = SamplePurpose.objects.using(
            self.dbAlias
        ).get_or_create(name=row["Sample Type"])
        ...
        if not vol:
            if row.get("Sample Volume [m^3]"):
                vol = (
                    float(row.get("Sample Volume [m^3]")) * 1.0e6
                )  # A million ml per cubic meter
        if not vol:
            self.logger.warn(
                "Sample Volume [mL] or Sample Volume [m^3] is not specified."
        ...
        if parameter_name.find(" ") != -1:
            spaceRemoveMsg = (
                "row['Parameter Name'] = %s contains a space. Replacing"
        ...
        if "(" in parameter_name or ")" in parameter_name:
            parenRemoveMsg = (
                "row['Parameter Name'] = %s contains ( or ). Removing"
        ...
        (parameter, created) = Parameter.objects.using(self.dbAlias).get_or_create(
            name=parameter_name, units=parameter_units
        )
        ...
        if row["Analysis Method"]:
            (analysisMethod, created) = AnalysisMethod.objects.using(
                self.dbAlias
    ...
    def delete_subsample(self, parentSample, row):
        ...
        if not samples:
            self.logger.debug(
                "No samples returned from query of parentSample = %s and row = %s",
                parentSample,
                row,
            )
            return
        ...
        if len(samples) == 1:
            self.logger.debug(
                "Deleting subsample %s from database %s", samples[0], self.dbAlais
            )
            samples[0].delete(using=self.dbAlias)
        else:
            self.logger.warn(
                "More than one subsample returned for query of parentSample = %s and row = %s",
                parentSample,
                row,
            )
        ...
        for r in csv.DictReader(open(fileName, encoding="latin-1")):
            ...
            try:
                ...
                parentSample = Sample.objects.using(self.dbAlias).filter(
                    instantpoint__activity__name__icontains=aName, name=sample_name
                )[0]
            except IndexError:
                try:
                    ...
                    self.logger.error(
                        "Parent Sample not found for Cruise (Activity Name) = %s, Bottle Number = %s",
                        aName,
                        r["Bottle Number"],
                    )
                    continue
                ...
                try:
                    ...
                    self.logger.warn(
                        "Parent Sample not found for Activity %s, name %s. Skipping.",
                        aName,
                        sample_name,
                    )
                ...
                if r["Bottle Number"] == "NetTow":
                    try:
                        ...
                        sample_name = "1"
                        parentSample = (
                            Sample.objects.using(self.dbAlias)
                            .select_related("instantpoint__activity")
                            .filter(
                                instantpoint__activity__name__icontains=aName
                                + "_NetTow1",
                            )[0]
                        ...
                        self.logger.warn(
                            "Parent Sample not found for Activity %s. Skipping.", aName
                        )
                        continue
                else:
                    raise e
            ...
            if unloadFlag:
                ...
                if parameter and subCount and parentSample not in loadedParentSamples:
                    ...
                    self.logger.info(
                        "%d subsamples loaded of %s from %s",
                        subCount,
                        parameter.name,
                    ...
                    self.logger.info(
                        "Loading subsamples of parentSample (activity, bottle/name) = (%s, %s)",
                        aName,
                        sample_name,
                    )
                    subCount = 0
    ...
    def postProcess(self):
        ...
        """
        for row in (
            SampledParameter.objects.using(self.dbAlias)
            .values("sample__instantpoint__activity__pk")
            .distinct()
        ):
