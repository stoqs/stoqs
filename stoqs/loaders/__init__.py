...
from utils.utils import spiciness
...
class LoadScript(object):
    ...
    def __init__(
        ...
        for dbType in ("", "_t", "_o", "_s10"):
            if dbType == "":
                exampleString = (
                    exampleString
                    + "  %s       \t# Load full resolution data into %s\n"
                ...
                exampleString = exampleString + "  %s -%s 10\t# Load data into %s\n" % (
                    sys.argv[0],
                    dbType[1],
                    self.base_dbAlias + dbType,
                )
            else:
                exampleString = (
                    exampleString
                    + "  %s -%s    \t# Load data into %s\n"
        ...
        if not self.args.campaignName:
            if self.args.optimal_stride:
                ...
                if self.args.stride == 1:
                    ...
                    self.campaignName = (
                        self.base_campaignName
                        + " with uniform stride of %d" % self.args.stride
                    )
        else:
    ...
    def addTerrainResources(self):
        ...
        self.logger.debug(
            "Looking in database %s for Campaign name = %s",
            self.dbAlias,
            self.campaignName,
        )
        try:
            campaign = m.Campaign.objects.using(self.dbAlias).get(
                name=self.campaignName
            )
        except m.Campaign.DoesNotExist:
            self.logger.error(
                f"Could not find Campaign record in {self.dbAlias} at end of load. Perhaps no data was loaded?"
            )
            sys.exit(-1)
        ...
        for url, viewpoint in list(self.x3dTerrains.items()):
            ...
            for name, value in list(viewpoint.items()):
                ...
                self.logger.info(
                    "Resource uristring=%s, name=%s, value=%s", url, name, value
                )
    ...
    def _build_image_atlases(self):
        ...
        for simulation in self.simulations.keys():
            ...
            data_range = [
                float(n) for n in self.simulations[simulation]["data_range"].split()
            ]
            scaled_range = [
                int(n) for n in self.simulations[simulation]["scaled_range"].split()
            ]
            simulation_dir = os.path.join(
                settings.MEDIA_ROOT,
                "simulations",
                self.simulations[simulation]["directory"],
            )
            ...
            for itime in range(ds.dims["time"]):
                ...
                for iy in range(ds.dims["y"]):
                    self.logger.debug(
                        f"{itime = } {iy = } Max value = {getattr(ds, variable).isel(time=itime, y=iy).values.max()}"
                    )
                    ...
                    self.logger.debug(
                        f"{pgm_name = } scaled_data range = [{scaled_data.min()} {scaled_data.max()}]"
                    )
                # Assemble this timestep to an ImageMagick montage
                adjusted_time = int(ds.time[itime].astype(int) / 1e9) + time_adjustment
                image_atlas_file = os.path.join(
                    simulation_dir, f"sim_{adjusted_time}.png"
                )
                os.system(f"cd {simulation_dir} && ls -1 *.pgm> slices")
                os.system(
                    f"cd {simulation_dir} && montage @slices -geometry +0+0 -tile {tile_dims} {image_atlas_file}"
                )
                os.system(f"cd {simulation_dir} && rm {' '.join(slices)} slices")
                self.logger.info(
                    f"{itime:03d} of {ds.dims['time']}: {image_atlas_file = }"
                )
    ...
    def addSimulationResources(self, build_image_atlases=False):
        ...
        if build_image_atlases:
            ...
            self.logger.info(
                "build_image_atlases=True, this needs to be done only once on a server"
            )
            self._build_image_atlases()

        resourceType, _ = m.ResourceType.objects.using(self.dbAlias).get_or_create(
            name="x3dsimulation",
            description="X3D Simulation information for Spatial 3D visualization",
        )
        ...
        m.Resource.objects.using(self.dbAlias).filter(
    ...
    def addPlaybackResources(self, x3dmodelurl, aName):
        ...
        self.logger.info("Adding to ResourceType: %s", resourceType)
        self.logger.debug(
            "Looking in database %s for Activity name = %s", self.dbAlias, aName
        )
        ...
        resource, _ = m.Resource.objects.using(self.dbAlias).get_or_create(
            ...
            name=X3D_MODEL,
            value="Output from bed2x3d.py - uristring to be included in GeoLocation node",
            resourcetype=resourceType,
        )
    ...
    def addPlatformResources(
        self, x3dmodelurl, pName, value="X3D model", nominaldepth=0.0, scalefactor=1.0
    ):
        ...
        self.logger.info("Adding to ResourceType: %s", resourceType)
        self.logger.debug(
            "Looking in database %s for Platform name = %s", self.dbAlias, pName
        )
        try:
            platform = m.Platform.objects.using(self.dbAlias).get(name=pName)
        except ObjectDoesNotExist:
            self.logger.debug(
                f"Platform {pName} not found in database {self.dbAlias}. Can't add Resources."
            )
            return
...
class STOQS_Loader(object):
    ...
    global_ignored_names = (
        ...
        "DEPTH",
        "depth",
    )  # A list (tuple) of parameters that should not be imported as parameters
    global_dbAlias = ""
    ...
    def getPlatform(self, name, type_name):
        ...
        if re.search("[^a-zA-Z0-9_-]", name):
            raise Exception(
                'Platform name = "%s" is not allowed.  Name can contain only letters, numbers, "_", and "-"'
                % name
            )

        self.logger.debug(
            "Opening %s to read platform names for matching to the MBARI tracking database",
            paURL,
        )
        ...
        try:
            ...
            self.logger.warn(
                f"Unable to read platform names from the MBARI tracking database: {paURL}"
            )

        if not platformName:
            platformName = name
            self.logger.debug(
                "Platform name %s not found in tracking database.  Creating new platform anyway.",
                platformName,
            )
        ...
        # Create PlatformType
        self.logger.debug(
            "calling using('%s').get_or-create() on PlatformType for platformTypeName = %s",
            self.dbAlias,
            self.platformTypeName,
        )
        platformType, created = m.PlatformType.objects.using(
            self.dbAlias
        ).get_or_create(name=self.platformTypeName)
        if created:
            self.logger.debug(
                "Created platformType.name %s in database %s",
                platformType.name,
                self.dbAlias,
            )
        else:
            self.logger.debug(
                "Retrieved platformType.name %s from database %s",
                platformType.name,
                self.dbAlias,
            )

        # Create Platform, allowing color to be updated by later load, eliminating potential duplicate names
        platform, created = m.Platform.objects.using(self.dbAlias).get_or_create(
            name=platformName, platformtype=platformType
        )
        if created:
            self.logger.info(
                "Created platform %s in database %s", platformName, self.dbAlias
            )
        else:
            self.logger.info(
                "Retrieved platform %s from database %s", platformName, self.dbAlias
            )
    ...
    def add_parameters(self, ds, append_units_to_name=True):
        ...
        for variable in set(self.include_names) & set(self.ds.keys()):
            ...
            parameter_name, parameter_units = self.parameter_name(
                variable, append_units_to_name
            )
            ...
            try:
                ...
                self.logger.debug(
                    "Parameter not found in local cache. Getting from database."
                )
                vattr = ds[variable].attributes
                self.parameter_dict[parameter_name], created = (
                    m.Parameter.objects.using(self.dbAlias).get_or_create(
                        name=parameter_name,
                ...
                if created:
                    self.logger.debug(
                        f"Added parameter {parameter_name} from {self.url} to database {self.dbAlias}"
                    )
    ...
    def createCampaign(self):
        ...
        try:
            ...
            if self.campaign.name != self.campaignName:
                self.logger.warn(
                    "Supplied Campaign name of %s does not match name = %s "
                    "in database %s",
                    self.campaignName,
                    self.campaign.name,
                    self.dbAlias,
            ...
            self.campaign = m.Campaign(
                name=self.campaignName, description=self.campaignDescription
            )
    ...
    def remove_appended_activities(self):
        loaded_date = None
        for dt_str in (
            m.Resource.objects.using(self.dbAlias)
            .filter(name="load_date_gmt")
        ...
        if not loaded_date:
            self.logger.info(
                "--remove_appended_activities requested, but loaded_date not found in %s.  Perhaps another load is in process.",
                self.dbAlias,
            )
            return

        for act in m.Activity.objects.using(self.dbAlias).filter(
            enddate__gt=loaded_date
        ):
    ...
    def createActivity(self):
        ...
        self.logger.debug(
            "Creating Activity with startDate = %s and endDate = %s",
            self.startDatetime,
            self.endDatetime,
        )
        comment = "Loaded on %s with these include_names: %s" % (
            datetime.now(),
        ...
        if self.activitytypeName is not None:
            self.activityType, created = m.ActivityType.objects.using(
                self.dbAlias
        ...
        try:
            ...
            self.activity, created = m.Activity.objects.using(
                self.dbAlias
            ).get_or_create(
                name__contains=self.activityName,
                platform=self.platform,
        ...
        if created:
            ...
            self.logger.info(
                "Created activity %s in database %s with startDate=%s, endDate = %s",
                self.activity.name,
                self.dbAlias,
                self.activity.startdate,
                self.activity.enddate,
            )
        else:
            self.logger.info(
                "Retrieved activity %s from database %s",
                self.activityName,
                self.dbAlias,
            )
    ...
    def _add_nc_global_attrs(self, all_names, append_units_to_name=True):
        ...
        self.logger.debug(
            "Getting or Creating Resource with name = %s, value = %s",
            "opendap_url",
            self.url,
        )
        resource, _ = m.Resource.objects.using(self.dbAlias).get_or_create(
        ...
        ars = (
            m.ActivityResource.objects.using(self.dbAlias)
            .filter(
                activity=self.activity,
                resource__resourcetype=resourceType,
        ...
        for ar in ars:
            if ar.resource.value != mp_ft_res.value:
                ...
                ars = (
                    ...
                    .update(resource=mp_ft_res)
                )
                self.logger.warn(
                    "Over-riding featureType from NC_GLOBAL (%s) with monkey-patched value = %s",
                    ar.resource.value,
                    mp_ft_res.value,
                )

        self.logger.info(
            "Adding attributes of all the variables from the original NetCDF file"
        )
        for v in all_names:
            ...
            try:
                ...
                if v != ALTITUDE:
                    self.logger.debug(
                        "include_name %s is not in %s - skipping", v, self.url
                    )
            except AttributeError as e:
                # Just skip over loaders that don't have the plotTimeSeriesDepth attribute
                self.logger.warn(
                    "%s for include_name %s in %s. Skipping", e, v, self.url
                )
            except ParameterNotFound as e:
                self.logger.warn("Could not get Parameter for v = %s: %s", v, e)

    def _add_plot_timeseries_depth_resources(
        self, all_names, append_units_to_name=True
    ):
        self.logger.info(
            "Adding plotTimeSeriesDepth Resource for Parameters we want plotted in Parameter tab"
        )
        for v in all_names:
            if hasattr(self, "plotTimeSeriesDepth"):
                if self.plotTimeSeriesDepth.get(v, None) is not None:
                    self.logger.debug("v = %s", v)
                    try:
                        uiResType, _ = m.ResourceType.objects.using(
                            self.dbAlias
                        ).get_or_create(name="ui_instruction")
                        resource, _ = m.Resource.objects.using(
                            self.dbAlias
                        ).get_or_create(
                            name="plotTimeSeriesDepth",
                            value=self.plotTimeSeriesDepth[v],
                            resourcetype=uiResType,
                        )
                    except ParameterNotFound as e:
                        self.logger.warn(
                            "Could not get_or_create uiResType or resource for v = %s: %s",
                            v,
                            e,
                        )
                    try:
                        ...
                        self.logger.debug(
                            "Could not add plotTimeSeriesDepth ParameterResource for v = %s: %s",
                            v,
                            e,
                        )
                    try:
                        ...
                        self.logger.warn(
                            "Could not add plotTimeSeriesDepth PlatformResource for v = %s: %s",
                            v,
                            e,
                        )
    ...
    def addResources(self):
        ...
        if hasattr(self, "add_to_activity"):
            if self.add_to_activity.name.split(" (")[0] not in self.url:
                self.logger.info(
                    f"Not adding NC_GLOBAL attributes from {self.url} as we are adding to Activity {self.add_to_activity.name}"
                )
                return
        if hasattr(self, "associatedActivityName"):
            if self.associatedActivityName.split(" (")[0] not in self.url:
                self.logger.info(
                    f"Not adding NC_GLOBAL attributes from {self.url} as we are associated with Activity {self.associatedActivityName}"
                )
                return
        if hasattr(self, "ds"):
    ...
    def getParameterByName(self, name):
        ...
        if name not in self.parameter_dict:
            ...
            if self.standard_names.get(name) is not None:
                ...
                try:
                    ...
                    self.parameter_dict[name] = m.Parameter.objects.using(
                        self.dbAlias
                    ).get(standard_name=self.standard_names[name][0])
                except ObjectDoesNotExist:
                    pass
                except IndexError:
                    pass
        ...
        if name not in self.parameter_dict:
            ...
            try:
                self.logger.debug(
                    "trying to get '%s' from database %s...", name, self.dbAlias
                )
                self.parameter_dict[name] = m.Parameter.objects.using(self.dbAlias).get(
                    name=name
                )
                self.logger.debug(
                    "self.parameter_dict[name].name = %s",
                    self.parameter_dict[name].name,
                )
            except ObjectDoesNotExist:
                ...
                raise ParameterNotFound(
                    "Parameter %s not found in the cache nor the database" % (name,)
                )
        # Finally, since we haven't had an error, we MUST have a parameter for this name.  Return it.

        self.logger.debug(
            "Returning self.parameter_dict[name].units = %s",
            self.parameter_dict[name].units,
        )
        try:
    ...
    def createMeasurement(
        self, mtime, depth, lat, lon, nomDepth=None, nomLat=None, nomLong=None
    ):
        """
        ...
        if depth < minDepth or depth > maxDepth:
            raise SkipRecord(
                "Bad depth: %s (depth must be between %s and %s)"
        ...
        if lat < -90 or lat > 90:
            raise SkipRecord(
                "Bad lat: %s (latitude must be between %s and %s)" % (lat, -90, 90)
            )
        if lon < -720 or lon > 720:
            raise SkipRecord(
                "Bad lon: %s (longitude must be between %s and %s)" % (lon, -720, 720)
            )

        ip, _ = m.InstantPoint.objects.using(self.dbAlias).get_or_create(
        ...
        if not (nomDepth is None and nomLat is None and nomLong is None):
            self.logger.debug(
                "nomDepth = %s nomLat = %s nomLong = %s", nomDepth, nomLat, nomLong
            )
        ...
        try:
            measurement, _ = m.Measurement.objects.using(self.dbAlias).get_or_create(
                instantpoint=ip, nominallocation=nl, depth=repr(depth), geom=point
            )
            ...
            self.logger.error(
                "Cannot save measurement mtime = %s, lon = %s, lat = %s, depth = %s",
                mtime,
                repr(lon),
                repr(lat),
                repr(depth),
    ...
    def good_coords(
        self, pnames, mtimes, depths, latitudes, longitudes, coords_equal=np.array([])
    ):
        ...
        for string in known_dup_or_decr_time_sources:
            if string in self.url:
                self.logger.info(
                    f"Setting known_dup_or_decr_time_problem for known_dup_or_decr_time_source: {string}"
                )
        ...
        previous_times = []
        for i, (mt, de, la, lo) in enumerate(
            zip(mtimes, depths, latitudes, longitudes)
        ):
            ...
            if self.is_coordinate_bad(pnames[0], mt, de, la, lo):
                self.logger.debug(
                    f"Marked coordinate bad: i = {i}, mt = {mt}, de = {de}, la = {la}, lo = {lo}"
                )
                mt = None
                de = None
                la = None
            ...
            if known_dup_or_decr_time_problem:
                ...
                if mt in previous_times:
                    self.logger.warn(
                        f"Will not load data from duplicate time coordinate: {mt} at index {i}"
                    )
                    dup_time = True
                elif mt and i > 0 and previous_times:
                    if mt <= previous_times[-1]:
                        self.logger.warn(
                            f"Will not load data from decreasing time coordinate: {mt} at index {i}"
                        )
                        decr_time = True
        ...
        for v in self.include_names:
            ...
            try:
                if len(self.ds[v].shape) == 1:
                    ...
                    anyValidData = not np.isnan(
                        np.array(self.ds[v].data[:][0][0])
                    ).all()
                    ...
                    self.logger.error(
                        "Parameter %s shape length of %s not handled",
                        v,
                        len(self.ds[v].shape),
                    )
                    raise Exception(
                        f"Parameter {v} with shape {len(self.ds[v].shape)} length of not handled"
                    )
            except KeyError:
                self.logger.debug(
                    "Parameter %s not in %s. Skipping.", v, list(self.ds.keys())
                )
                if v.find(".") != -1:
                    raise Exception(
                        "Parameter names must not contain periods - cannot load data. Paramater %s violates CF conventions."
                        % v
                    )
            except ValueError:
                pass
        ...
        try:
            for var in list(self.ds.keys()):
                ...
                if "standard_name" in self.ds[var].attributes:
                    ...
                    self.standard_names[var] = (
                        None  # Indicate those without a standard name
                    )
        except AttributeError as e:
            self.logger.warn(e)
    ...
    def update_ap_stats(dbAlias, activity, parameters, sampledFlag=False):
        ...
        for p in list(parameters.keys()):
            if sampledFlag:
                data = (
                    m.SampledParameter.objects.using(dbAlias)
                    .filter(parameter=p, sample__instantpoint__activity=activity)
                    .values_list("datavalue", flat=True)
                )
            else:
                data = (
                    m.MeasuredParameter.objects.using(dbAlias)
                    .filter(parameter=p, measurement__instantpoint__activity=activity)
                    .values_list("datavalue", flat=True)
                )
            ...
            if len(data) == 0:
                ...
                data_array = (
                    m.MeasuredParameter.objects.using(dbAlias)
                    .filter(
                        parameter=p, measurement__instantpoint__activity__name=activity
                    )
    ...
    @classmethod
    def update_activityparameter_stats(
        cls, dbAlias, activity, parameters, sampledFlag=False
    ):
    ...
    def updateActivityParameterStats(self, act_to_update=None, sampledFlag=False):
        ...
        try:
            self.update_activityparameter_stats(
                self.dbAlias, act_to_update, self.parameter_counts, sampledFlag
            )
            ...
            self.logger.warn(
                "IntegrityError(%s): Cannot create ActivityParameter and "
                "updated statistics for Activity %s.",
                (e, act_to_update),
            )

        self.logger.info(
            "Updated statistics for act_to_update.name = %s", act_to_update.name
        )
    ...
    def insertSimpleDepthTimeSeries(self, critSimpleDepthTime=10):
        ...
        """
        vlqs = (
            m.Measurement.objects.using(self.dbAlias)
        ...
        self.logger.info(
            "Number of points in original depth time series = %d", len(line)
        )
        ...
        self.logger.info(
            "Number of points in simplified depth time series = %d", len(simple_line)
        )
        ...
        for t, d, k in simple_line:
            try:
                ...
                self.logger.warn(
                    "InstantPoint with id = %d does not exist; from point at index k = %d",
                    pklookup[k],
                    k,
                )
    ...
    def saveBottomDepth(self):
        ...
        def _innerSaveBottomDepth(self):
            ...
            """
            mpQS = (
                m.MeasuredParameter.objects.using(self.dbAlias)
            ...
            for mp in mpQS:
                ...
                if counter % 10000 == 0:
                    self.logger.info(
                        "%d of %d mp.measurement.bottomdepth records saved",
                        counter,
                        count,
                    )
    ...
    def insertSimpleBottomDepthTimeSeries(self, critSimpleBottomDepthTime=10):
        ...
        def _innerInsertSimpleBottomDepthTimeSeries(self, critSimpleBottomDepthTime=10):
            ...
            tbdQS = (
                m.MeasuredParameter.objects.using(self.dbAlias)
                .filter(measurement__instantpoint__activity=self.activity)
                .values(
                    "measurement__instantpoint__timevalue",
                    "measurement__bottomdepth",
            ...
            if count > maxRecords:
                ...
                self.logger.info(
                    "Striding tbdQS by %d to be kind to simplify_points()", stride
                )
            ...
            for tbd in tbdQS:
                ...
                if i % stride == 0:
                    counter += 1
                    ems = 1000 * date2num(
                        tbd["measurement__instantpoint__timevalue"],
                    ...
                    if tbd["measurement__bottomdepth"]:
                        ...
                        if counter % 10000 == 0:
                            self.logger.info(
                                "%d of %d points read", counter, int(count / stride)
                            )
            ...
            if line:
                try:
                    # Original simplify_points code modified: the index from the original line is added as 3rd item in the return
                    self.logger.info(
                        "Calling simplify_points with len(line) = %d", len(line)
                    )
            ...
            self.logger.info(
                "Number of points in simplified depth time series = %d",
                len(simple_line),
            )
            self.logger.debug("simple_line = %s", simple_line)
            ...
            self.logger.info(
                "Inserted %d values into SimpleBottomDepthTime", len(simple_line)
            )

        return _innerInsertSimpleBottomDepthTimeSeries(self, critSimpleBottomDepthTime)

    def insertSimpleDepthTimeSeriesByNominalDepth(
        self, critSimpleDepthTime=10, trajectoryProfileDepths=None
    ):
        """
        ...
        for i, nl in enumerate(
            m.NominalLocation.objects.using(self.dbAlias).filter(activity=self.activity)
        ):
            nomDepth = nl.depth
            ...
            ndlqs = (
                m.Measurement.objects.using(self.dbAlias)
                .filter(instantpoint__activity=self.activity, nominallocation=nl)
                .values_list("instantpoint__timevalue", "depth", "instantpoint__pk")
            ...
            if trajectoryProfileDepths:
                self.logger.info(
                    "Loading time varying depths in SimpleDepthTime for nomDepth=%s",
                    nomDepth,
                )
                ...
                self.logger.debug(
                    "Loading depths in SimpleDepthTime for nomDepth=%s", nomDepth
                )
            ...
            if len(simple_line) != 2:
                self.logger.warn(
                    "len(simple_line) != 2. If appending data, then all points will be added to SimpleDepthTime"
                )
            ...
            if self.dataStartDatetime and len(simple_line) == 2:
                ...
                try:
                    ...
                    m.SimpleDepthTime.objects.using(self.dbAlias).get_or_create(
                        activity=self.activity,
                        nominallocation=nl,
                        instantpoint=ip,
                        depth=d0,
                        epochmilliseconds=t0,
                    )
                except ObjectDoesNotExist:
                    self.logger.warn(
                        "InstantPoint with id = %d does not exist; from point at index k = %d",
                    ...
                    self.logger.warn(e)
                    firstPoints = m.SimpleDepthTime.objects.using(self.dbAlias).filter(
                        activity=self.activity,
                        nominallocation=nl,
                        instantpoint=ip,
                        depth=d0,
                ...
                lastPoints = m.SimpleDepthTime.objects.using(self.dbAlias).filter(
                    ...
                    nominallocation=nl,
                    depth=d1,
                    epochmilliseconds__gt=t0,
                )
                for lp in lastPoints:
                    self.logger.debug(
                        "Deleting SimpleDepthTime point with epochmilliseconds=%s",
                        lp.epochmilliseconds,
                    )
                ...
                for t, d, k in simple_line:
                    ...
                    try:
                        ip = m.InstantPoint.objects.using(self.dbAlias).get(
                            id=pklookup[k]
                        )
                        self.logger.debug("ip = %s", ip)
                        m.SimpleDepthTime.objects.using(self.dbAlias).create(
            ...
            self.logger.debug(
                "Inserted %d values into SimpleDepthTime for nomDepth = %f",
                len(simple_line),
                nomDepth,
            )
    ...
    def updateActivityMinMaxDepth(self, act_to_update):
        ...
        m.Activity.objects.using(self.dbAlias).filter(id=act_to_update.id).update(
            mindepth=m_qs["depth__min"], maxdepth=m_qs["depth__max"]
        )
    ...
    def updateCampaignStartEnd(self):
        ...
        try:
            if self.campaign:
                ip_qs = m.InstantPoint.objects.using(self.dbAlias).aggregate(
                    Max("timevalue"), Min("timevalue")
                )
                m.Campaign.objects.using(self.dbAlias).filter(
                    id=self.campaign.id
                ).update(
                    startdate=ip_qs["timevalue__min"], enddate=ip_qs["timevalue__max"]
                )
        except AttributeError as e:
            self.logger.warn(e)
    ...
    def assignParameterGroup(self, groupName=MEASUREDINSITU):
        ...
        """
        g, _ = m.ParameterGroup.objects.using(self.dbAlias).get_or_create(
            name=groupName
        )
        for p in self.parameter_counts:
            pgps = m.ParameterGroupParameter.objects.using(self.dbAlias).filter(
                parameter=p, parametergroup=g
            )
            if not pgps:
                ...
                try:
                    ...
                    self.logger.warn(
                        "%s: Cannot create ParameterGroupParameter name = %s for parameter.name = %s. Skipping.",
                        e,
                        groupName,
                        p.name,
                    )
    ...
    def _best_ts_parameter_names(self, ms, salinity_standard_name):
        ...
        temp_mp = (
            m.MeasuredParameter.objects.using(self.dbAlias)
            .filter(
                measurement__in=ms, parameter__standard_name="sea_water_temperature"
            )
        ...
        if hasattr(self.ds, "attributes"):
            if "NC_GLOBAL" in self.ds.attributes:
                if "comment" in self.ds.attributes["NC_GLOBAL"]:
                    self.logger.info(
                        f"netCDF comment = {self.ds.attributes['NC_GLOBAL']['comment']}"
                    )
                    if best_ctd := best_ctd_is_re.search(
                        self.ds.attributes["NC_GLOBAL"]["comment"]
                    ):
                        self.logger.info(
                            f"Looking for Best CTD in temp_mp list: {temp_mp}"
                        )
                        self.logger.info(
                            f"Looking for Best CTD in sal_mp list: {sal_mp}"
                        )
        ...
        if not sal_pn:
            ...
            sal_pn = sal_mp[0]
        self.logger.info(
            "Using Parameters '%s' and '%s' to compute sigmat and spice",
            temp_pn,
            sal_pn,
        )
    ...
    def _calculate_sigmat_mps(
        self, temps, salts, mids, depths, lats, p_sigmat, salinity_standard_name
    ):
        ...
        for meas, sigmat in zip(meass, sigmats):
            sigmat_mp = m.MeasuredParameter(
                measurement=meas, parameter=p_sigmat, datavalue=sigmat
            )
    ...
    def _calculate_spice_mps(self, temps, salts, mids, p_spice, salinity_standard_name):
        ...
        for meas, spice in zip(meass, spices):
            spice_mp = m.MeasuredParameter(
                measurement=meas, parameter=p_spice, datavalue=spice
            )
    ...
    def _get_sea_water_parameters(self):
        ...
        """
        sea_water_temperature_parms = [
            p
            for p in self.parameter_dict.values()
            if p.standard_name == "sea_water_temperature"
        ]
        try:
        ...
        if len(sea_water_temperature_parms) > 1:
            self.logger.info(
                f"Found more than one Parameter in {self.url} with standard_name == 'sea_water_temperature'"
            )
            ...
            for p in sea_water_temperature_parms:
                if "_" not in p.name:
                    ...
                    break
            self.logger.info(
                f"Using {sea_water_temperature_parm} for sea_water_temperature"
            )
        ...
        sea_water_salinity_parms = [
            p
            for p in self.parameter_dict.values()
            if p.standard_name == salinity_standard_name
        ]
        ...
        if len(sea_water_salinity_parms) > 1:
            self.logger.info(
                f"Found more than one Parameter in {self.url} with standard_name == 'sea_water_salinity'"
            )
        ...
        return (
            sea_water_temperature_parm,
            sea_water_salinity_parm,
            salinity_standard_name,
        )
    ...
    def addSigmaTandSpice(self, activity=None):
        ...
        try:
            (
                ...
                sea_water_salinity_parm,
                salinity_standard_name,
            ) = self._get_sea_water_parameters()
        except NameError as e:
            self.logger.info(f"{e}")
            self.logger.info(
                "No sea_water_temperature and sea_water_salinity Parameters. Not adding SigmaT and Spice."
            )
            return
        ...
        if not ms:
            self.logger.info(
                "No sea_water_temperature and sea_water_salinity measurements. Not adding SigmaT and Spice."
            )
            return
        ...
        if "spice" in self.include_names:
            p_spice, _ = m.Parameter.objects.using(self.dbAlias).get_or_create(
                name="stoqs_spice", defaults={"long_name": SPICINESS}
            )
        else:
            p_spice, _ = m.Parameter.objects.using(self.dbAlias).get_or_create(
                name=SPICE, defaults={"long_name": SPICINESS}
            )
        # Update with descriptions, being kind to legacy databases
        p_sigmat.description = (
            "Calculated in STOQS loader from Measured Parameters having standard_names"
        ...
        self.logger.info(
            f"Calculating {self.parameter_counts[p_sigmat]} sigmat & spice MeasuredParameters"
        )
        ...
        sigmat_mps = self._calculate_sigmat_mps(
            temps, salts, mids, depths, lats, p_sigmat, salinity_standard_name
        )
        spice_mps = self._calculate_spice_mps(
            temps, salts, mids, p_spice, salinity_standard_name
        )

        self.logger.info(
            f"Bulk loading {self.parameter_counts[p_sigmat]} sigmat MeasuredParameters"
        )
        m.MeasuredParameter.objects.using(self.dbAlias).bulk_create(sigmat_mps)
        self.logger.info(
            f"Bulk loading {self.parameter_counts[p_spice]} spice MeasuredParameters"
        )
    ...
    def addAltitude(self, activity=None):
        ...
        if self.grdTerrain:
            try:
                ...
                self.logger.error(
                    f"Cannot add {ALTITUDE}. Make sure file {os.path.abspath(self.grdTerrain)} is present."
                )
                self.logger.error(
                    f"cd stoqs/loaders && wget https://stoqs.mbari.org/terrain/{os.path.basename(self.grdTerrain)}"
                )
                ...
                try:
                    ...
                    try:
                        ...
                        self.logger.error(
                            f"Cannot read range metadata from {self.grdTerrain}. Not able to load"
        ...
        xyFileName = NamedTemporaryFile(
            dir="/dev/shm", prefix="STOQS_LatLon_", suffix=".txt"
        ).name
        ...
        if activity:
            ms = ms.filter(instantpoint__activity=activity)
        ms = (
            ms.order_by("instantpoint__activity__id", "instantpoint__timevalue")
            .values("id", "geom", "depth")
            .distinct()
        )
        mList = []
        depthList = []
        ...
        bdepthFileName = NamedTemporaryFile(
            dir="/dev/shm", prefix="STOQS_BDepth", suffix=".txt"
        ).name
        if cmd_exists("grdtrack"):
            cmd = "grdtrack %s -V -G%s > %s" % (
                xyFileName,
                self.grdTerrain,
                bdepthFileName,
            )
        ...
        if self.totalRecords > 1e6:
            self.logger.info(
                "This is lame... Sleeping 5 seconds to give time for system call to finish writing to %s",
                bdepthFileName,
            )
            time.sleep(5)
        if self.totalRecords > 1e7:
            self.logger.info(
                "Sleeping another 300 seconds to give time for system call to"
                " finish writing to %s for more than 10 million records",
                bdepthFileName,
            )
            time.sleep(300)
        ...
        try:
            p_alt, _ = m.Parameter.objects.using(self.dbAlias).get_or_create(
                ...
                long_name="Altitude",
                description=(
                    "Calculated in STOQS loader by using GMT's grdtrack(1) program on the Platform's"
        ...
        meass = (
            m.Measurement.objects.using(self.dbAlias)
            .filter(id__in=mList)
            .order_by("instantpoint__activity__id", "instantpoint__timevalue")
        )
        count = 0
        alt_mps = []
            ...
            try:
                with transaction.atomic():
                    for line, meas in zip(altFH, meass):
                        ...
                        mp_alt = m.MeasuredParameter(
                            datavalue=alt, measurement=meas, parameter=p_alt
                        )
        ...
        if inputFileCount != count:
            self.logger.warn(
                "Counts are not equal! inputFileCount = %s, count from grdtrack output = %s",
                inputFileCount,
                count,
            )
