...
query_parms = {
    ...
    "parameterplot": (
        "parameterplotid",  # Plot radio button selection
        "platformplotname",
    ),  # - client knows platform name
    "parametercontourplot": (
        "parametercontourplotid",  # Plot contour radio button selection
...
def _buildMapFile(request, qm, options):
    ...
    if "mappath" in request.session:
        logger.debug(
            "Reusing request.session['mappath'] = %s", request.session["mappath"]
        )
    else:
        request.session["mappath"] = (
            __name__
            + "_"
        ...
        logger.debug(
            "Setting new request.session['mappath'] = %s", request.session["mappath"]
        )
    ...
    for plats in platform_layer_types:
        for p in plats:
            ...
            if (
                ...
                and p[3].lower() != "timeseriesprofile"
                and p[3].lower() != "trajectoryprofile"
            ):
                continue
            item = Item()
            item.id = p[1]
            ...
            item.geo_query = qm.getActivityGeoQuery(
                Q(platform__name="%s" % p[0]), pointFlag=True
            )
            item_list.append(item)
    ...
    av = ActivityView(
        request, item_list, trajectory_union_layer_string, station_union_layer_string
    )
...
def queryData(request, fmt=None):
    ...
    for key, value in list(request.GET.items()):
        if key.endswith(
            ...
            try:
                pminmax = {
                    name: (
                        request.GET.getlist(name + "_MIN")[0],
                        request.GET.getlist(name + "_MAX")[0],
                    )
                }
            except:
                logger.exception(
                    "Could not get parameter values even though "
                    + key
                    + " ends with _MIN"
                )
    ...
    try:
        ...
        return HttpResponseBadRequest(
            'Bad request: Database "' + request.META["dbAlias"] + '" Does Not Exist'
        )
    try:
        start_time = time.time()
        options = json.dumps(qm.generateOptions(), cls=encoders.STOQSJSONEncoder)
        logger.info(
            f"generateOptions() took {1000 * (time.time() - start_time):6.1f} ms to build query/summary response"
        )
...
def queryMap(request, db_alias):
    ...
    options = json.dumps(qm.generateOptions(), cls=encoders.STOQSJSONEncoder)
    logger.info(
        f"generateOptions() took {1000 * (time.time() - start_time):7.1f} ms to build query/map response"
    )
    ##logger.debug('options = %s', pprint.pformat(options))
...
def queryUI(request, db_alias=None):
    """
    Build and return main query web page
    """

    # Set db_alias in request.META if provided
    if db_alias:
        request.META["dbAlias"] = db_alias
    ...
    formats = [
        ...
        (
            "kml",
            "Keyhole Markup Language - click on icon to view in Google Earth",
        ),
        (
            "sql",
            "Structured Query Language for PostgreSQL",
        ),
        (
            "matlab",
            "Matlab - Load data into Matlab structure array",
        ),
        (
            "json",
        ...
        (
            "tsv",
            "Tabbed Separated Values",
        ),
        (
            "html",
            "Hyper Text Markup Language table",
        ),
    ]
    ...
    return render(request, "stoqs/stoqsquery.html", context=config_settings)
