...
Fig.~\ref{fig:Trajectory} is a visualization of \textbf{trajectory} salinity from AUV
Dorado shown along with Net Tows and underway tracks from the R/V Rachel Carson.
...
If data have different units additional axes will be added to the plot.
...
from a mooring in Monterey Bay. The ``balance'' color map from the Ocean \cite{cmocean} tab in the Colormaps section.
...
\begin{figure*}[htbp]
\centering
\includegraphics[width=0.8\linewidth]{Isopycnal.png}
\caption{Chlorophyll fluoresence measurement from from a week-long set of missions by AUV Dorado selected for plotting on isopycnal level between 25.0 and 25.25.}
\label{fig:Isopycnal}
\end{figure*}
