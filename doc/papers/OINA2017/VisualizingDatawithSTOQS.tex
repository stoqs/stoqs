...
by oceanographic platforms. STOQS uses a geospatial database and a web-based user interface (UI) 
to allow scientists to explore large collections of data. The UI is optimized to provide a 
...
Open Geospatial Consortium (OGC) CF-NetCDF standard and Discrete Sampling Geometries
...
and subsetting happen on reading the data from disk. This problem is commonly
encountered by users when they start working with large collections of data 
using commonly available Matlab and Python routines -- they run out of memory. 
...
These CF-NetCDF files are then loaded into a STOQS database using and Extract Translate
Load process that depends on the files being served by an OPeNDAP server \cite{Cornillon03opendap:accessing}.
Requiring the data to be served by an OPeNDAP server before loading into STOQS has two positive outcomes:
1) OPeNDAP supports indexed access to NeTCDF data with a network protocol, and 2) Provenance tracking
is supported as the STOQS user is able to access the original source of the data.
...
\caption{Googl Cardboard viewing mode screen shot from a smart phone. 
...
measured at the same time. By viewing selected data in the Spatial section 
...
can be used some screen shots are shown.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=1\linewidth]{Trajectory.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=1\linewidth]{TimeseriesProfile.png}
...
constraints and another demontstrates analyzing spectral Laser Optical
Plankton Counter data from an AUV.
...
post-campaign for analysis of multidisciplinary data sets.
